...
ApplePS2Keyboard * ApplePS2Keyboard::probe(IOService * provider, SInt32 * score)
{
  ...
  IOLog("ApplePS2Keyboard: probe provider=%s success=%d\n",
...
bool ApplePS2Keyboard::start(IOService * provider)
{
  ...
  IOLog("ApplePS2Keyboard: start provider=%s\n",
        provider ? provider->getName() : "(null)");
  ...
  // Panthera's PS/2 path is still polling-based, so disable HID autorepeat
  // until key-up delivery is fully interrupt-driven and stable.
  AbsoluteTime_to_scalar(&_keyRepeat) = 0;
  AbsoluteTime_to_scalar(&_initialKeyRepeat) = 0;
  ...
  _device->installInterruptAction(this, &ApplePS2Keyboard::_interruptOccurredAction);
  ...
  // Panthera currently polls the PS/2 controller instead of wiring IRQ1.
  setCommandByte(kCB_TranslateMode,
  ...
  _device->installPowerControlAction(this, &ApplePS2Keyboard::_setDevicePowerStateAction);
  _powerControlHandlerInstalled = true;

  IOLog("ApplePS2Keyboard: keyboard enabled\n");
...
void ApplePS2Keyboard::_interruptOccurredAction(void * target, UInt8 scanCode)
{
  if (target)
  {
    ((ApplePS2Keyboard *)target)->interruptOccurred(scanCode);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void ApplePS2Keyboard::_setDevicePowerStateAction(void * target, UInt32 whatToDo)
{
  if (target)
  {
    ((ApplePS2Keyboard *)target)->setDevicePowerState(whatToDo);
  }
}
