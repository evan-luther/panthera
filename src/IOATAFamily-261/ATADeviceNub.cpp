...
#define PANTHERA_ATANUB_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
ATADeviceNub::ataDeviceNub( IOATAController* provider, ataUnitID unit, ataDeviceType devType)
{
	...
	if( ! nub )
	{
		PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB alloc failed unit=%u type=%u\n",
		                      (unsigned) unit, (unsigned) devType);
		return 0L;
	}
	
	if( !nub->init( provider, unit, devType) )
	{
			PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB init failed unit=%u type=%u\n",
			                      (unsigned) unit, (unsigned) devType);
			nub->release();
			return 0L;
	}

	PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB ready unit=%u type=%u\n",
	                      (unsigned) unit, (unsigned) devType);
	return nub;
...
{
	PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB init enter unit=%u type=%u provider=%s\n",
	                      (unsigned) unit,
	                      (unsigned) devType,
	...
	{
		PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB super::init failed unit=%u\n",
		                      (unsigned) unit);
		return false;
	}
	...
	if( !buffer )
	{
		PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB buffer alloc failed unit=%u\n",
		                      (unsigned) unit);
		return false;
	}
	...
	if( err )
	{
		PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB getDeviceID failed unit=%u err=0x%x\n",
		                      (unsigned) unit, (unsigned) err);
	...
	PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB getDeviceID ok unit=%u type=%u\n",
	                      (unsigned) unit, (unsigned) _deviceType);
...
ATADeviceNub::getDeviceID( void )
{
	...
	PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB identify unit=%u type=%u\n",
	                      (unsigned) _unitNumber, (unsigned) _deviceType);
	...
	// Panthera: early legacy PATA interrupt delivery is still incomplete.
	// IDENTIFY is a simple PIO read, so force the built-in polled path here
	// to get the device nub online before full IRQ handling is perfect.
	cmd->setFlags(mATAFlagIORead | mATAFlagUseNoIRQ);
	...
	PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB identify submit unit=%u err=0x%x\n",
	                      (unsigned) _unitNumber, (unsigned) err);
	if(!err)
	{
	...
	PANTHERA_ATANUB_TRACE("PANTHERA:ATANUB identify result unit=%u err=0x%x\n",
	                      (unsigned) _unitNumber, (unsigned) err);
