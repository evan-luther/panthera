...
#define PANTHERA_ATADEV_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
IOATADevice::matchPropertyTable(OSDictionary * table)
{
	...
	bool	result;
	OSObject *bundleId;
	OSString *bundleIdString;
	
	result = compareProperty ( table, "IOUnit" );

	bundleId = table ? table->getObject("CFBundleIdentifier") : 0;
	bundleIdString = OSDynamicCast(OSString, bundleId);
	if (bundleIdString &&
	    bundleIdString->isEqualTo("org.puredarwin.panthera.PantheraATAStorage")) {
		PANTHERA_ATADEV_TRACE("PANTHERA:ATADEV match noscore IOUnit=%d\n", result ? 1 : 0);
	}
...
IOATADevice::matchPropertyTable(OSDictionary * table, SInt32 * score )
{
	...
	if (bundleIdString &&
	    bundleIdString->isEqualTo("org.puredarwin.panthera.PantheraATAStorage")) {
		PANTHERA_ATADEV_TRACE("PANTHERA:ATADEV match score type=%d score=%ld\n",
		                      result ? 1 : 0,
		                      score ? (long) *score : -1L);
	}
