...
#define PANTHERA_PIIXROOT_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
static void registerClientApplier( IOService * service, void * context )
{
    if ( service )
    {
        PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT register client %s class=%s\n",
                                service->getName(), service->getMetaClass()->getClassName());
...
{
    PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT start provider=%s class=%s\n",
    ...
    if ( _nubs == 0 )
    {
        PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT no channel nubs created\n");
        return false;
    }

    PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT created %u channel nubs\n",
                            (unsigned) _nubs->getCount());
...
OSSet * AppleIntelPIIXATARoot::createATAChannelNubs( void )
{
    ...
    do {
        ...
        {
            PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT provider open failed viddid=0x%08x\n",
                                    (unsigned) _provider->configRead32(kIOPCIConfigVendorID));
            break;
        }

        PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT provider open ok viddid=0x%08x serial=%u\n",
                                (unsigned) _provider->configRead32(kIOPCIConfigVendorID),
                                getProperty(kSerialATAKey) == kOSBooleanTrue ? 1U : 0U);
        ...
        for ( UInt32 channelID = 0; channelID < 2; channelID++ )
        {
            ...
            PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT ch=%u mode=%u pi=0x%x irq=0x%x\n",
                                    (unsigned) channelID, (unsigned) channelMode,
                                    (unsigned) _provider->configRead8(kPIIX_PCI_PI),
                                    (unsigned) _provider->configRead8(kIOPCIConfigInterruptLine));
            ...
            if (channelInfo)
            {
                PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT ch=%u native ok\n", (unsigned) channelID);
            }
            if (channelInfo == 0)
            {
                channelInfo = createLegacyModeChannelInfo( channelID, channelMode );
                PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT ch=%u legacy %s\n",
                                        (unsigned) channelID, channelInfo ? "ok" : "failed");
            }
            if (channelInfo == 0)
                continue;
            ...
            if ( nub )
            {
                ...
                bool initOK = nub->init( this, channelInfo, dtEntry );
                bool attachOK = false;

                if ( initOK )
                {
                    attachOK = nub->attach( this );
                }

                PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT ch=%u init=%u attach=%u\n",
                                        (unsigned) channelID, initOK ? 1U : 0U, attachOK ? 1U : 0U);
                ...
                else
                {
                    ...
                    channelName[3] = (char) ('0' + channelID);
                                          ...
                                          UInt8  interruptVector )
{
    ...
    {
        PANTHERA_PIIXROOT_TRACE("PANTHERA:PIIXROOT createChannelInfo rejected ch=%u mode=%u cmd=0x%x ctr=0x%x irq=%u\n",
                                (unsigned) ataChannel, (unsigned) channelMode,
                                commandPort, controlPort, interruptVector);
...
IOReturn
AppleIntelPIIXATARoot::callPlatformFunction( const OSSymbol * functionName,
                                             ...
                                             void *           param4 )
{
    ...
    if (platform)
    {
        IOReturn result = platform->callPlatformFunction(functionName,
                                                         waitForFunction,
                                                         param1,
                                                         param2,
                                                         param3,
