...
#define PANTHERA_PIIXPATA_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
bool AppleIntelPIIXPATA::start( IOService * provider )
{
    ...
    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA start provider=%s class=%s\n",
    ...
    if ( _provider == 0 )
    {
        PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA provider cast failed\n");
        goto fail;
    }
    ...
    {
        PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA configurePCIDevice failed ch=%u\n",
                                (unsigned) _channel);
        goto fail;
    }
    ...
    {
        PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA super::start failed ch=%u\n",
                                (unsigned) _channel);
        goto fail;
    }
    superStarted = true;

    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA super::start ok ch=%u\n", (unsigned) _channel);
    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA devinfo ch=%u unit0=%u unit1=%u\n",
                            (unsigned) _channel,
                            (unsigned) _devInfo[0].type,
                            (unsigned) _devInfo[1].type);

    if (_devInfo[0].type == kUnknownATADeviceType &&
        _devInfo[1].type == kUnknownATADeviceType)
    {
        PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA no devices after scan ch=%u, bailing early\n",
                                (unsigned) _channel);
        goto fail;
    }
    ...
    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA interrupt source ok ch=%u irq=%u intsrc=%p workloop=%p\n",
                            (unsigned) _channel,
                            (unsigned) _provider->getInterruptVector(),
                            _intSrc, _workLoop);
	...
	*_bmStatusReg = kPIIX_IO_BMISX_IDEINTS;
    ...
    // drain any latched INTRQ so the line is low before first commanded I/O
    (void) (*_tfStatusCmdReg);
    OSSynchronizeIO();
    *_bmStatusReg = kPIIX_IO_BMISX_IDEINTS;

    // Panthera: skip PATA power-management / ACPI media-notify setup during
    // early bring-up so disk publication is not blocked on those paths.
    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA skipping PM/ACPI setup ch=%u\n",
                            (unsigned) _channel);

    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA publish-loop begin ch=%u unit0=%u unit1=%u\n",
                            (unsigned) _channel,
                            (unsigned) _devInfo[0].type,
                            (unsigned) _devInfo[1].type);
    ...
    {
        PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA publish-loop unit=%u type=%u\n",
                                (unsigned) i, (unsigned) _devInfo[i].type);
        if ( _devInfo[i].type != kUnknownATADeviceType )
        {
            ...
            if ( nub )
            {
                PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA nub unit=%u type=%u\n",
                                        (unsigned) i, (unsigned) _devInfo[i].type);
                ...
                if ( nub->attach( this ) )
                {
                    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA nub attach ok unit=%u\n",
                                            (unsigned) i);
                    _nub[i] = (IOATADevice *) nub;
                    _nub[i]->retain();
                    _nub[i]->registerService();
                    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA nub registerService unit=%u\n",
                                            (unsigned) i);
                }
                else
                {
                    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA nub attach failed unit=%u\n",
                                            (unsigned) i);
                }
                nub->release();
            }
            ...
            {
                PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA nub create failed unit=%u type=%u\n",
                                        (unsigned) i, (unsigned) _devInfo[i].type);
            }
        }
    }
...
fail:
    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA start failed ch=%u superStarted=%u\n",
                            (unsigned) _channel, superStarted ? 1 : 0);
                                             ...
                                             UInt32        channel )
{
    ...
    idetimOffset = (channel == kPIIX_CHANNEL_SECONDARY)
                 ? kPIIX_PCI_IDETIM_S
                 : kPIIX_PCI_IDETIM;
    idetim = device->configRead16( idetimOffset );
    progIf = device->configRead8( 0x09 );
    bmiba  = device->configRead32( kPIIX_PCI_BMIBA );

    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA cfg ch=%u idetim=0x%04x progif=0x%02x bmiba=0x%08x\n",
                            (unsigned) channel, (unsigned) idetim, (unsigned) progIf, (unsigned) bmiba);

    // QEMU's legacy IDE path can expose a usable controller with the decode
    // enable bit clear. Force it on so the rest of the driver can probe.
    if ( (idetim & kPIIX_PCI_IDETIM_IDE) == 0 )
    {
        UInt16 forced = idetim | kPIIX_PCI_IDETIM_IDE;
        PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA forcing %s IDE decode enable old=0x%04x new=0x%04x\n",
                                (channel == kPIIX_CHANNEL_PRIMARY) ? "primary" : "secondary",
                                (unsigned) idetim, (unsigned) forced);
        device->configWrite16( idetimOffset, forced );
        idetim = device->configRead16( idetimOffset );
        PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA forced idetim verify=0x%04x\n",
                                (unsigned) idetim);
...
UInt32 AppleIntelPIIXPATA::scanForDrives( void )
{
    ...
    *_tfAltSDevCReg = 0x0;
    ...
    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA scan enter ch=%u status=0x%02x cyl=%02x/%02x sc=%02x sn=%02x\n",
                            (unsigned) _channel,
                            (unsigned) *_tfStatusCmdReg,
                            (unsigned) *_tfCylLoReg,
                            (unsigned) *_tfCylHiReg,
                            (unsigned) *_tfSCountReg,
                            (unsigned) *_tfSectorNReg);
    ...
    *_tfSDHReg = 0x00;  // Initialize device selection to device 0.
    ...
    PANTHERA_PIIXPATA_TRACE("PANTHERA:PIIXPATA scan done ch=%u units=%u unit0=%u unit1=%u status=0x%02x\n",
                            (unsigned) _channel,
                            (unsigned) unitsFound,
                            (unsigned) _devInfo[0].type,
                            (unsigned) _devInfo[1].type,
...
IOReturn
AppleIntelPIIXPATA::handleInsertAction( OSObject * owner,
                    void *     /* arg0 */,
                    void *     /* arg1 */,
                    void *     /* arg2 */,
                    void *     /* arg3 */ )
{
    AppleIntelPIIXPATA * self = (AppleIntelPIIXPATA *) owner;
    return self ? self->handleInsert() : kIOReturnBadArgument;
}
	...
	if( messageType == kIOACPIMessageDeviceNotification )
	{
		...
		switch( messageVal )
		{
			case 0x81:
				...
				self->_cmdGate->runAction( &handleInsertAction );
			break;
