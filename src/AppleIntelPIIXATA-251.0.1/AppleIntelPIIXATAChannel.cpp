...
#define PANTHERA_PIIXCHAN_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
AppleIntelPIIXATAChannel::setupInterrupt( IOService * provider, UInt32 line )
{
    ...
    PANTHERA_PIIXCHAN_TRACE("PANTHERA:PIIXCHAN setupInterrupt provider=%s class=%s irq=%u ret=0x%x\n",
    ...
    {
        const OSSymbol * controllerName = OSSymbol::withCString("8259-pic");
        ...
        if (controllerName && controller && specifier && data)
        {
            controller->setObject(controllerName);
            specifier->setObject(data);
            setProperty(gIOInterruptControllersKey, controller);
            setProperty(gIOInterruptSpecifiersKey, specifier);
            PANTHERA_PIIXCHAN_TRACE("PANTHERA:PIIXCHAN setupInterrupt fallback irq=%u controller=%s\n",
                                    (unsigned) line,
                                    controllerName->getCStringNoCopy());
            data->release();
            specifier->release();
            controller->release();
                                     ...
                                     IORegistryEntry * dtEntry )
{
    ...
    PANTHERA_PIIXCHAN_TRACE("PANTHERA:PIIXCHAN pre-int ch=%u mode=%u cmd=0x%x ctr=0x%x irq=%u provider=%s\n",
                            (unsigned) _channelNum, (unsigned) _channelMode,
                            _cmdBlock, _ctrBlock, _irq,
                            provider ? provider->getName() : "<null>");
    ...
    PANTHERA_PIIXCHAN_TRACE("PANTHERA:PIIXCHAN init ch=%u mode=%u cmd=0x%x ctr=0x%x irq=%u\n",
                            (unsigned) _channelNum, (unsigned) _channelMode,
                            _cmdBlock, _ctrBlock, _irq);
                                                   ...
                                                   SInt32 *       score )
{
    ...
    bool match = (( getChannelMode() != kChannelModePATA &&  drvSATA ) ||
                  ( getChannelMode() == kChannelModePATA && !drvSATA ));

    PANTHERA_PIIXCHAN_TRACE("PANTHERA:PIIXCHAN match ch=%u mode=%u drvSATA=%u result=%u\n",
                            (unsigned) getChannelNumber(), (unsigned) getChannelMode(),
                            drvSATA ? 1 : 0, match ? 1 : 0);
