...
kern_return_t IOHIDEventService::Dispatch(const IORPC rpc)
{
    return super::Dispatch(rpc);
}
...
bool IOHIDEventService::start ( IOService * provider )
{
    ...
    bool explicitLegacyShim = PE_parse_boot_argn("hid-legacy-shim", &legacy_shim, sizeof (legacy_shim));
    if (!explicitLegacyShim) {
        legacy_shim = 0;
    }
    ...
    if (legacy_shim) {
        ...
        if (pantheraUSBHIDLogsEnabled() && !explicitLegacyShim) {
            _keyboardShim = kLegacyShimEnabled;
            IOLog("PANTHERA:IOHIDEventService forcing keyboard legacy shim provider=%s singleUser=%d\n",
    ...
    if (pantheraUSBHIDLogsEnabled()) {
        IOLog("PANTHERA:IOHIDEventService shim state provider=%s keyboard=%d legacyBootArg=%d singleUser=%d\n",
              provider ? provider->getName() : "<null>",
              _keyboardShim,
              explicitLegacyShim ? 1 : 0,
              isSingleUser() ? 1 : 0);
    }
                                ...
                                IOOptionBits    options)
{
    ...
    if (pantheraUSBHIDLogsEnabled()) {
        IOLog("PANTHERA:IOHIDEventService newKeyboardShim service=%s supportedModifiers=0x%x options=0x%x dispatcher=%d\n",
              getName(),
              (unsigned)supportedModifiers,
              (unsigned)options,
              isDispatcher ? 1 : 0);
    }
    ...
    if (pantheraUSBHIDLogsEnabled()) {
        IOLog("PANTHERA:IOHIDEventService newKeyboardShim started nub=%p provider=%s\n",
              keyboardNub,
              getName());
    }
...
void IOHIDEventService::dispatchEvent(IOHIDEvent * event, IOOptionBits options)
{
    ...
    if (event->getType() == kIOHIDEventTypeKeyboard &&
        ...
        // Panthera bring-up: defer display tickle integration until the HID
        // kernel path is otherwise alive.
    }
