...
void _runSyncReportAsyncTrampoline(thread_call_param_t param0, thread_call_param_t param1)
{
    if (param0) {
        ((IOHIDDevice *)param0)->runSyncReportAsync();
    }
}

void _setNextAsyncTimeoutTrampoline(OSObject *owner, IOTimerEventSource *sender)
{
    if (owner) {
        ((IOHIDDevice *)owner)->setNextAsyncTimeout();
    }
}
...
kern_return_t IOHIDDevice::Dispatch(const IORPC rpc)
{
    return IOService::Dispatch(rpc);
}
...
bool IOHIDDevice::start( IOService * provider )
{
    ...
    _asyncReportThread = thread_call_allocate_with_options(_runSyncReportAsyncTrampoline, this, THREAD_CALL_PRIORITY_KERNEL, THREAD_CALL_OPTIONS_ONCE);
    ...
    require_action((_asyncTimer = IOTimerEventSource::timerEventSource(this, _setNextAsyncTimeoutTrampoline)), exit, HIDDeviceLogError("IOTimerEventSource::timerEventSource failed"));
...
bool IOHIDDevice::handleOpen(IOService * client, IOOptionBits options, void * argument __unused)
{
    ...
    if (options & kIOHIDOptionsTypeSeizeDevice && !_seizedClient) {
        ...
        if (provider && (provider->metaCast("IOHIKeyboard") || provider->metaCast("IOHIPointing"))) {
            provider->message(kIOHIDSystemDeviceSeizeRequestMessage, this, (void *)true);
        }
    }
...
void IOHIDDevice::handleClose(IOService * client, IOOptionBits options __unused)
{
    ...
    if (_clientSet->containsObject(client)) {
        ...
        if (client == _seizedClient) {
            ...
            IOService *provider = getProvider();
            if (provider && (provider->metaCast("IOHIKeyboard") || provider->metaCast("IOHIPointing"))) {
                provider->message(kIOHIDSystemDeviceSeizeRequestMessage, this, (void *)false);
            }
        }
