...
kern_return_t IOHIDInterface::Dispatch(const IORPC rpc)
{
    return super::Dispatch(rpc);
}

void IOHIDInterface::ReportAvailable(uint64_t timestamp,
                                     uint32_t reportID,
                                     uint32_t reportLength,
                                     IOHIDReportType type,
                                     IOMemoryDescriptor *report,
                                     OSAction *action,
                                     OSDispatchMethod actionDispatch)
{
#pragma unused(timestamp, reportID, reportLength, type, report, action, actionDispatch)
}
