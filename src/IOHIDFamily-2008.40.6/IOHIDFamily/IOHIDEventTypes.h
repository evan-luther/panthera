...
/*
 * Panthera: newer IOHIDSystem sources reference a few event scalar types that
 * are not present in this older public header set. Provide the narrow typedefs
 * needed to compile the kernel-side HID slice.
 */
typedef double   IOHIDDouble;
typedef uint32_t IOHIDGenericGestureType;
typedef uint32_t IOHIDUnicodeEncodingType;
typedef uint32_t IOHIDEventTimestampType;
/*!
...
/*
 * Later HID sources use a dedicated game-controller event type that is absent
 * from the older public enum surface. Reuse the reserved slot so newer fields
 * have a stable base without perturbing the older event numbering.
 */
enum {
    kIOHIDEventTypeGameController = kIOHIDEventTypeReserved1
};
...
enum {
    ...
    kIOHIDEventOptionContinuousTime                         = 1<<5,
...
enum {
    kIOHIDEventTimestampTypeDefault                         = 0,
    kIOHIDEventTimestampTypeAbsolute                        = 0,
    kIOHIDEventTimestampTypeContinuous                      = 1
};
typedef uint32_t IOHIDEventTimestampType;

enum {
    kIOHIDEventFieldGameControllerDirectionPadUp            = IOHIDEventFieldBase(kIOHIDEventTypeGameController),
    ...
    kIOHIDEventFieldGameControllerThumbstickButtonLeft,
    kIOHIDEventFieldGameControllerThumbstickButtonRight,
    kIOHIDEventFieldGameControllerButtonL4,
    kIOHIDEventFieldGameControllerButtonR4,
    kIOHIDEventFieldGameControllerButtonL5,
    kIOHIDEventFieldGameControllerButtonR5,
    kIOHIDEventFieldGameControllerType
};
