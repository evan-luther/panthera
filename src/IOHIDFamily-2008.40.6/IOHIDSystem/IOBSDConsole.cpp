...

{
    ...
    if( ref) {
        ...
        if (audio != 0) {
            OSObject *out = newService->copyProperty("Out");
            OSNumber *outNumber = OSDynamicCast(OSNumber, out);
            if (outNumber) {
                if (outNumber->unsigned8BitValue() == 1) {
	...
	audio = 0;
        ...
        if( keyboard && self->attach( keyboard )) {
            if (pantheraUSBHIDLogsEnabled()) {
                IOLog("PANTHERA:IOBSDConsole keyboard published=%s\n", newService ? newService->getName() : "<null>");
            }
...
void IOBSDConsole::arbitrateForKeyboard( IOHIKeyboard * nub )
{
  bool opened;
  if (pantheraUSBHIDLogsEnabled()) {
    IOLog("PANTHERA:IOBSDConsole arbitrate keyboard=%s\n", nub ? nub->getName() : "<null>");
  }
  opened = nub->open(this, 0, 0,
		(KeyboardEventCallback)keyboardEvent, 
	        (KeyboardSpecialEventCallback) 0, 
	        (UpdateEventFlagsCallback)updateEventFlags);
  if (pantheraUSBHIDLogsEnabled()) {
    IOLog("PANTHERA:IOBSDConsole keyboard open result=%d keyboard=%s\n",
          opened ? 1 : 0,
          nub ? nub->getName() : "<null>");
  }
