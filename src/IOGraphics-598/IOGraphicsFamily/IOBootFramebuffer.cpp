                                      ...
                                      SInt32 *  score )
{
    ...
    do
    {
        IOLog("PANTHERA:IOBootFramebuffer probe enter provider=%s boot-display=%d\n",
              provider->getName(),
              (provider->getProperty("AAPL,boot-display") != 0));
        ...
        {
            IOLog("PANTHERA:IOBootFramebuffer probe console-miss err=%#x base=%#x\n",
                  err,
        ...
        IOLog("PANTHERA:IOBootFramebuffer probe provider=%s base=%#x width=%u height=%u depth=%u rowBytes=%u\n",
              provider->getName(),
              (unsigned int) bootDisplay.v_baseAddr,
        ...
        setProperty(kIOFBWidthKey, static_cast<UInt32>(bootDisplay.v_width), 32);
        setProperty(kIOFBHeightKey, static_cast<UInt32>(bootDisplay.v_height), 32);
        setProperty(kIOFBBitsPerPixelKey, static_cast<UInt32>(bootDisplay.v_depth), 32);
        setProperty(kIOFBBytesPerRowKey, static_cast<UInt32>(bootDisplay.v_rowBytes & 0x7fff), 32);
        setProperty(kIOFBMemorySizeKey,
                    static_cast<UInt32>((bootDisplay.v_rowBytes & 0x7fff) * bootDisplay.v_height),
                    32);
        IOLog("PANTHERA:IOBootFramebuffer published geometry width=%u height=%u depth=%u rowBytes=%u\n",
              (unsigned int) bootDisplay.v_width,
              (unsigned int) bootDisplay.v_height,
              (unsigned int) bootDisplay.v_depth,
              (unsigned int) (bootDisplay.v_rowBytes & 0x7fff));
        *score          = 0;
        ret = this;                     // Success
    }
    while (false);
