...
protected:
    ...
    // Referenced by the public IOFramebuffer.cpp, but absent from the public
    // header set.
    static IOReturn gTraceData(void *context, void *buffer, uint16_t *sizeP);
    static IOReturn agdcTraceData(void *context, void *buffer, uint16_t *sizeP);
    static void triggerEvent(uint_fast32_t event);
    static uint_fast32_t clearEvent(uint_fast32_t event);
