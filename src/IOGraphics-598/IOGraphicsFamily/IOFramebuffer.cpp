...
IOReturn IOFramebuffer::gTraceData(void *context, void *buffer, uint16_t *sizeP)
{
    (void) context;
    (void) buffer;
    if (sizeP) {
...
IOReturn IOFramebuffer::agdcTraceData(void *context, void *buffer, uint16_t *sizeP)
{
    (void) context;
    (void) buffer;
    if (sizeP) {
...
void IOFramebuffer::triggerEvent(uint_fast32_t event)
{
    atomic_fetch_or_explicit(&gIOFBGlobalEvents, event, memory_order_relaxed);
    startThread(false);
}

uint_fast32_t IOFramebuffer::clearEvent(uint_fast32_t event)
{
    return atomic_fetch_and_explicit(&gIOFBGlobalEvents,
                                     ~event,
...
enum
{
    kServerAckProtocolGraphicsTypesRev = IOGRAPHICSTYPES_REV
};
...
#define SYSASSERTGATED() do { \
    if (!gIOFBSystemWorkLoop->inGate()) panic_plain("%s not sys gated?", __FUNCTION__); \
} while(false)
#define SYSASSERTNOTGATED() do { \
    if (gIOFBSystemWorkLoop->inGate()) panic_plain("%s sys gated?", __FUNCTION__); \
} while(false)
...
#define FCASSERTGATED(fc) do { \
    if (!fc->fWl->inGate()) panic_plain("%s not controller gated?", __FUNCTION__); \
} while(false)
#define FCASSERTNOTGATED(fc) do { \
    if (fc->fWl->inGate()) panic_plain("%s controller gated?", __FUNCTION__); \
} while(false)
...
#define FBASSERTGATED(fb) do { \
    if (!FBWL(fb)->inGate()) panic_plain("%s not framebuffer gated?", __FUNCTION__); \
} while(false)
#define FBASSERTNOTGATED(fb) do { \
    if (FBWL(fb)->inGate()) panic_plain("%s framebuffer gated?", __FUNCTION__); \
} while(false)
...
static const char * PantheraPropString(IORegistryEntry * entry, const char * key)
{
    OSString * value = OSDynamicCast(OSString, entry->getProperty(key));
    return value ? value->getCStringNoCopy() : "<null>";
}

static IOService * PantheraEnsureDisplayProvider(void)
{
    ...
    if (!platform)
    {
        IOLog("PANTHERA:IOG no platform expert for display synth\n");
        return NULL;
    }

    if (platform->getConsoleInfo(&bootDisplay) || !bootDisplay.v_baseAddr)
    {
        IOLog("PANTHERA:IOG display synth skipped base=%#x width=%u height=%u depth=%u rowBytes=%u\n",
              (unsigned int) bootDisplay.v_baseAddr,
              (unsigned int) bootDisplay.v_width,
              (unsigned int) bootDisplay.v_height,
              (unsigned int) bootDisplay.v_depth,
              (unsigned int) bootDisplay.v_rowBytes);
        return NULL;
    }

    OSDictionary * dict = OSDictionary::withCapacity(1);
    OSString * ioName = dict ? OSString::withCString("display") : NULL;
    if (!dict || !ioName)
    {
        IOLog("PANTHERA:IOG display synth alloc failed\n");
        OSSafeReleaseNULL(ioName);
        OSSafeReleaseNULL(dict);
        return NULL;
    }

    dict->setObject("IOName", ioName);
    ioName->release();
    ...
    if (!nub)
    {
        IOLog("PANTHERA:IOG display synth createNub returned null\n");
        return NULL;
    }

    nub->setProperty("AAPL,boot-display", kOSBooleanTrue);
    IOLog("PANTHERA:IOG display synth created class=%s name=%s base=%#x width=%u height=%u depth=%u rowBytes=%u\n",
    ...
    {
        IOLog("PANTHERA:IOG display synth attach failed\n");
        nub->release();
        return NULL;
    }

    nub->registerService();
    IOLog("PANTHERA:IOG display synth registered boot-display=%d provider=%s\n",
          (nub->getProperty("AAPL,boot-display") != 0),
          (nub->getProvider() && nub->getProvider()->getName()) ? nub->getProvider()->getName() : "<null>");
    return nub;
}

static void PantheraInspectDisplayProvider(void)
{
    IORegistryEntry * platform = IORegistryEntry::fromPath("/IOPlatformExpert", gIOServicePlane);
    OSIterator * iter;
    OSObject * obj;
    bool found = false;
    ...
    if (!platform)
    {
        IOLog("PANTHERA:IOG /IOPlatformExpert missing in service plane\n");
        return;
    }

    iter = platform->getChildIterator(gIOServicePlane);
    if (!iter)
    {
        IOLog("PANTHERA:IOG /IOPlatformExpert has no child iterator\n");
        platform->release();
        return;
    }
    ...
    while ((obj = iter->getNextObject()))
    {
        ...
        const OSMetaClass * superMeta;

        if (!child)
            continue;

        meta = child->getMetaClass();
        superMeta = meta ? meta->getSuperClass() : 0;
        if (!child->getPath(path, &pathLen, gIOServicePlane))
            strlcpy(path, "<path-failed>", sizeof(path));

        IOLog("PANTHERA:IOG platform-child class=%s super=%s name=%s IOName=%s reg-name=%s boot-display=%d path=%s\n",
              meta ? meta->getClassName() : "<null>",
              superMeta ? superMeta->getClassName() : "<null>",
              child->getName() ? child->getName() : "<null>",
              PantheraPropString(child, "IOName"),
              PantheraPropString(child, "name"),
              (child->getProperty("AAPL,boot-display") != 0),
              path);

        if (child->getName() && (0 == strcmp("display", child->getName())))
            found = true;
    }

    if (!found)
    {
        IOLog("PANTHERA:IOG no display child under /IOPlatformExpert\n");
        IOService * created = PantheraEnsureDisplayProvider();
        if (created)
        {
            IOLog("PANTHERA:IOG display synth ready class=%s name=%s\n",
                  created->getMetaClass() ? created->getMetaClass()->getClassName() : "<null>",
                  created->getName() ? created->getName() : "<null>");
            created->release();
        }
    }
...
__private_extern__ "C" kern_return_t IOGraphicsFamilyModuleStart(kmod_info_t *ki, void *data)
{
    ...
    PantheraInspectDisplayProvider();
...
void IOFramebuffer::setupCursor(void)
{
    ...
    if (getProvider() && getProvider()->getProperty("AAPL,boot-display"))
    {
        setProperty(kIOFBWidthKey, __private->framebufferWidth, 32);
        setProperty(kIOFBHeightKey, __private->framebufferHeight, 32);
        setProperty(kIOFBBitsPerPixelKey, info->bitsPerPixel, 32);
        setProperty(kIOFBBytesPerRowKey, rowBytes, 32);
        IOLog("PANTHERA:IOFramebuffer geometry width=%u height=%u depth=%u rowBytes=%u\n",
              (unsigned int) __private->framebufferWidth,
              (unsigned int) __private->framebufferHeight,
              (unsigned int) info->bitsPerPixel,
...
bool IOFramebuffer::start( IOService * provider )
{
    ...
    if (provider && provider->getName() && (0 == strcmp("display", provider->getName())))
    {
        IOLog("PANTHERA:IOFramebuffer start enter provider=%s class=%s\n",
              provider->getName(),
              provider->getMetaClass() ? provider->getMetaClass()->getClassName() : "<null>");
    }
    ...
    if (provider && provider->getName() && (0 == strcmp("display", provider->getName())))
    {
        IOLog("PANTHERA:IOFramebuffer start registered provider=%s regID=%#qx\n",
              provider->getName(),
              __private ? __private->regID : 0);
        if (provider->getProperty("AAPL,boot-display") && !opened)
        {
            IOLog("PANTHERA:IOFramebuffer start auto-open provider=%s\n",
                  provider->getName());
            const IOReturn openErr = open();
            IOLog("PANTHERA:IOFramebuffer start auto-open result=%#x opened=%d online=%d\n",
                  openErr,
                  opened,
                  (__private ? __private->online : 0));
        }
    }
		...
		*/
		...
		else
		{
			...
			if (__private->saveLength)
			{
				...
				kern_return_t kr = 
#endif
				vm_map_wire_external( IOPageableMapForAddress( (vm_address_t) __private->saveFramebuffer ),
...
void IOFramebuffer::displaysOnline(bool nowOnline)
{
    ...
    if (getProvider() && getProvider()->getName()
        && (0 == strcmp("display", getProvider()->getName())))
    {
        IOLog("PANTHERA:IOFramebuffer displaysOnline now=%d was=%d\n",
              nowOnline,
              __private ? __private->displaysOnline : 0);
    }
...
IOReturn IOFramebuffer::open( void )
{
    ...
    if (getProvider() && getProvider()->getName()
        && (0 == strcmp("display", getProvider()->getName())))
    {
        IOLog("PANTHERA:IOFramebuffer open enter provider=%s opened=%d dead=%d\n",
              getProvider()->getName(),
              opened,
              dead);
    }
    ...
    do
    {
        ...
        if (getProvider() && getProvider()->getName()
            && (0 == strcmp("display", getProvider()->getName())))
        {
            IOLog("PANTHERA:IOFramebuffer open updateOnline=%d console=%d\n",
                  nowOnline,
                  isConsoleDevice());
        }
		if (nowOnline)
    ...
    if (getProvider() && getProvider()->getName()
        && (0 == strcmp("display", getProvider()->getName())))
    {
        IOLog("PANTHERA:IOFramebuffer open exit err=%#x opened=%d online=%d all=%u started=%u\n",
              err,
              opened,
              (__private ? __private->online : 0),
              gAllFramebuffers ? gAllFramebuffers->getCount() : 0,
              gStartedFramebuffers ? gStartedFramebuffers->getCount() : 0);
    }
...
IOReturn IOFramebuffer::diagnoseReport( void * param1, void * param2, void * param3, void * param4 )
{
    (void) param1;
    (void) param2;
    (void) param3;
