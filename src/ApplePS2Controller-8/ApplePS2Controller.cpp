...
static bool
panthera_seed_ps2_interrupts(IOService *provider)
{
  OSArray *controllers = 0;
  OSArray *specifiers = 0;
  const OSSymbol *controllerName = 0;
  bool success = false;
  ...
  if (provider->getProperty(gIOInterruptControllersKey) &&
      provider->getProperty(gIOInterruptSpecifiersKey)) {
    return true;
  }

  controllers = OSArray::withCapacity(16);
  specifiers = OSArray::withCapacity(16);
  controllerName = OSSymbol::withCString("8259-pic");
  if (!controllers || !specifiers || !controllerName) {
    goto done;
  }

  for (UInt32 vector = 0; vector < 16; vector++) {
    OSData *data = OSData::withBytes(&vector, sizeof(vector));
    if (!data) {
      goto done;
    }
    controllers->setObject(controllerName);
    specifiers->setObject(data);
    data->release();
  }

  provider->setProperty(gIOInterruptControllersKey, controllers);
  provider->setProperty(gIOInterruptSpecifiersKey, specifiers);
  success = true;

done:
  if (controllerName) controllerName->release();
  if (specifiers) specifiers->release();
...
bool ApplePS2Controller::init(OSDictionary * properties)
{
  ...
  _interruptRegisteredKeyboard = false;
  _interruptRegisteredMouse    = false;
...
bool ApplePS2Controller::start(IOService * provider)
{
 ...
 if (!super::start(provider))  return false;
  ...
  IOLog("ApplePS2Controller: start provider=%s\n",
        provider ? provider->getName() : "(null)");
  IOLog("ApplePS2Controller: interrupt props controllers=%p specifiers=%p\n",
        provider ? provider->getProperty(gIOInterruptControllersKey) : 0,
        provider ? provider->getProperty(gIOInterruptSpecifiersKey) : 0);
  ...
  if (!panthera_seed_ps2_interrupts(provider)) {
    IOLog("ApplePS2Controller: failed to seed interrupt properties\n");
    goto fail;
  }
  ...
  _interruptSourceMouse    = IOInterruptEventSource::interruptEventSource(
        this, &ApplePS2Controller::_interruptOccurredAction);
  ...
  _interruptSourceQueue    = IOInterruptEventSource::interruptEventSource(
        this, &ApplePS2Controller::_processRequestQueueAction);
  _pollSource              = IOTimerEventSource::timerEventSource(
        this, &ApplePS2Controller::_pollForDataAction);

  if ( !_workLoop                ||
       !_interruptSourceMouse    ||
       !_interruptSourceKeyboard ||
       !_interruptSourceQueue    ||
       !_pollSource )  goto fail;

  if ( _workLoop->addEventSource(_interruptSourceQueue) != kIOReturnSuccess )
    goto fail;
  if ( _workLoop->addEventSource(_pollSource) != kIOReturnSuccess )
    goto fail;
  ...
  IOLog("ApplePS2Controller: published keyboard and mouse nubs\n");
  return true; // success

fail:
  IOLog("ApplePS2Controller: start failed\n");
  stop(provider);
  return false;
}
                                                ...
                                                PS2InterruptAction action)
{
  ...
  if (deviceType == kDT_Keyboard && _interruptInstalledKeyboard == false)
  {
    ...
    _interruptRegisteredKeyboard = false;
    IOLog("ApplePS2Controller: keyboard using poll mode (IRQ1 disabled)\n");
    if (_pollSource) {
      _pollSource->cancelTimeout();
      _pollSource->setTimeoutUS(kPS2PollIntervalUS);
    }
  ...
  {
    IOReturn registerResult;
    IOReturn enableResult;
    target->retain();
    _interruptTargetMouse = target;
    _interruptActionMouse = action;
    _workLoop->addEventSource(_interruptSourceMouse);
    registerResult = getProvider()->registerInterrupt(kIRQ_Mouse, 0, interruptHandlerMouse);
    enableResult = getProvider()->enableInterrupt(kIRQ_Mouse);
    IOLog("ApplePS2Controller: mouse interrupt install register=0x%x enable=0x%x\n",
          registerResult, enableResult);
    _interruptRegisteredMouse = ((registerResult == kIOReturnSuccess) &&
                                 (enableResult == kIOReturnSuccess));
    if (!_interruptRegisteredMouse) {
      IOLog("ApplePS2Controller: mouse IRQ unavailable, enabling poll fallback\n");
    }
    if (_pollSource) {
      _pollSource->cancelTimeout();
      _pollSource->setTimeoutUS(kPS2PollIntervalUS);
...
void ApplePS2Controller::uninstallInterruptAction(PS2DeviceType deviceType)
{
  ...
  {
    if (_interruptRegisteredKeyboard) {
  ...
  if (_pollSource && !_interruptInstalledKeyboard && !_interruptInstalledMouse) {
    _pollSource->cancelTimeout();
  }
}
...
void ApplePS2Controller::_interruptOccurredAction(OSObject * owner, IOInterruptEventSource * sender, int count)
{
  if (owner)
  {
    ((ApplePS2Controller *)owner)->interruptOccurred(sender, count);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void ApplePS2Controller::_processRequestQueueAction(OSObject * owner, IOInterruptEventSource * sender, int count)
{
  if (owner)
  {
    ((ApplePS2Controller *)owner)->processRequestQueue(sender, count);
  }
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void ApplePS2Controller::_pollForDataAction(OSObject * owner, IOTimerEventSource * sender)
{
  if (owner)
  {
    ((ApplePS2Controller *)owner)->pollForData(sender);
  }
}
...
{                                                      // IOInterruptEventAction
  ...
  while (1)
  {
                                   ...
                                   (kOutputReady | kMouseData))
    {
      ...
      unlockController(state);
      dispatchDriverInterrupt(kDT_Mouse, data);
      lockController(&state);
    }

    // In poll mode there is no IRQ1 handler to pre-read keyboard bytes into
    // the internal queue, so consume raw keyboard data directly here.
    else if (((status = inb(kCommandPort)) & (kOutputReady | kMouseData)) ==
             kOutputReady)
    {
      UInt8 data = inb(kDataPort);
      unlockController(state);
      dispatchDriverInterrupt(kDT_Keyboard, data);
      lockController(&state);
    }
...
void ApplePS2Controller::pollForData(IOTimerEventSource *sender)
{
  if (!_interruptInstalledKeyboard && !_interruptInstalledMouse) {
    return;
  }

  interruptOccurred(0, 0);

  if (sender && (_interruptInstalledKeyboard || _interruptInstalledMouse)) {
    sender->setTimeoutUS(kPS2PollIntervalUS);
  }
}
                                            ...
                                            IOService *   policyMaker)
{
  ...
  if ( thread_call_enter1( _powerChangeThreadCall,
                           (void *) (uintptr_t) powerStateOrdinal ) == TRUE )
  {
    release();
  }
                                                  ...
                                                  void * arg2, void * arg3 )
{
  ...
  UInt32       powerState = (UInt32) (uintptr_t) arg0;
