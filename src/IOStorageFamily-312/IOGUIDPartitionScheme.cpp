...
#define PANTHERA_GPT_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
OSDefineMetaClassAndStructors(IOGUIDPartitionScheme, IOPartitionScheme);

namespace {
static const char *pantheraTranslateGPTContentHint(const char *partitionHint)
{
    if (partitionHint == 0) {
        return "";
    }

    if (strcmp(partitionHint, "48465300-0000-11AA-AA11-00306543ECAC") == 0) {
        return "Apple_HFS";
    }

    if (strcmp(partitionHint, "426F6F74-0000-11AA-AA11-00306543ECAC") == 0) {
        return "Apple_Boot";
    }
...
IOService * IOGUIDPartitionScheme::probe(IOService * provider, SInt32 * score)
{
    ...
    PANTHERA_GPT_TRACE("PANTHERA:GPT probe provider=%s score=%ld\n",
    ...
    PANTHERA_GPT_TRACE("PANTHERA:GPT probe scan partitions=%u score=%ld\n",
                       _partitions ? _partitions->getCount() : 0U,
                       score ? (long)*score : -1L);
...
bool IOGUIDPartitionScheme::start(IOService * provider)
{
    ...
    PANTHERA_GPT_TRACE("PANTHERA:GPT start partitions=%u\n",
                       _partitions ? _partitions->getCount() : 0U);
    ...
    while ( (partition = (IOMedia *) partitionIterator->getNextObject()) )
    {
        if ( partition->attach(this) )
        {
            ...
            PANTHERA_GPT_TRACE("PANTHERA:GPT publish name=%s content=%s whole=%d\n",
                               partition->getName(),
                               partition->getContent() ? partition->getContent() : "<null>",
                               partition->isWhole() ? 1 : 0);
        }
    }
...
OSSet * IOGUIDPartitionScheme::scan(SInt32 * score)
{
    ...
    PANTHERA_GPT_TRACE("PANTHERA:GPT scan enter media=%s blockSize=%llu size=%llu\n",
                       media ? media->getName() : "<null>",
                       mediaBlockSize,
                       media ? media->getSize() : 0ULL);

    // Determine whether this media is formatted.

    if ( media->isFormatted() == false ) {
        PANTHERA_GPT_TRACE("PANTHERA:GPT scan media not formatted\n");
        goto scanErr;
    }
    ...
    if ( (mediaBlockSize % sizeof(disk_blk0)) ) {
        PANTHERA_GPT_TRACE("PANTHERA:GPT scan bad block size=%llu\n", mediaBlockSize);
        goto scanErr;
    }
    ...
    if ( buffer == 0 ) {
        PANTHERA_GPT_TRACE("PANTHERA:GPT scan buffer alloc failed size=%llu\n", bufferSize);
        goto scanErr;
    }
    ...
    if ( partitions == 0 ) {
        PANTHERA_GPT_TRACE("PANTHERA:GPT scan partition set alloc failed\n");
        goto scanErr;
    }
    ...
    if ( mediaIsOpen == false ) {
        PANTHERA_GPT_TRACE("PANTHERA:GPT scan open failed\n");
        goto scanErr;
    }
    ...
    PANTHERA_GPT_TRACE("PANTHERA:GPT scan protective read status=0x%x\n", status);
                                                         ...
                                                         UInt32    partitionID )
{
    ...
    uuid_unparse( partition->ent_type,
                  partitionHint );
    translatedHint = pantheraTranslateGPTContentHint(partitionHint);
    ...
    if ( newMedia )
    {
         if ( newMedia->init(
                ...
                /* contentHint        */ translatedHint ) )
        {
            PANTHERA_GPT_TRACE("PANTHERA:GPT partition id=%u raw=%s translated=%s\n",
                               partitionID,
                               partitionHint,
                               translatedHint);
