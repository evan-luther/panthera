...
/* DriverKit IOUserBlockStorageDevice not available on Panthera */
class IOPerfControlClient;
class IOUserBlockStorageDevice : public IOBlockStorageDevice {
    OSDeclareDefaultStructors(IOUserBlockStorageDevice);
public:
    virtual IOReturn doEjectMedia(void) { return kIOReturnUnsupported; }
    ...
    virtual UInt32 doGetFormatCapacities(UInt64 *capacities, UInt32 capacitiesMaxCount) const {
        if (capacities && capacitiesMaxCount) {
            capacities[0] = 0;
        }
        return capacities && capacitiesMaxCount ? 1U : 0U;
    }
    virtual char *getVendorString(void) { return (char *)"Panthera"; }
    virtual char *getProductString(void) { return (char *)"UserBlockStub"; }
    virtual char *getRevisionString(void) { return (char *)"0"; }
    virtual char *getAdditionalDeviceInfoString(void) { return (char *)""; }
    ...
    IOPerfControlClient *getPerfControlClient() { return NULL; }
};
...
#define PANTHERA_BSDRV_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
bool IOBlockStorageDriver::start(IOService * provider)
{
    ...
    PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV start provider=%s class=%s\n",
    ...
    success = open(this);
    PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV open=%d\n", success ? 1 : 0);
    ...
    if (success)
    {
        ...
        PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV handleStart=%d\n", success ? 1 : 0);
    ...
    if (success)
    {
        ...
        registerService();
        PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV registerService\n");
    }
...
IOBlockStorageDriver::acceptNewMedia(void)
{
    ...
    PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV acceptNewMedia blocks=%llu blockSize=%llu bytes=%llu writeProtected=%d\n",
                         _maxBlockNumber,
                         _mediaBlockSize,
                         nbytes,
                         _writeProtected ? 1 : 0);
    ...
    PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV instantiateMedia result=0x%x name=%s\n",
                         result,
                         name);
    ...
    if (result == kIOReturnSuccess) {
        ...
        PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV media attach=%d\n", ok ? 1 : 0);
        if (ok) {
            ...
            PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV media registerService whole=%d content=%s\n",
                                 m->isWhole() ? 1 : 0,
                                 m->getContent() ? m->getContent() : "<null>");
        } else {
...
IOBlockStorageDriver::checkForMedia(void)
{
    ...
    PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV reportMediaState ret=0x%x current=%d changed=%d hasMedia=%d\n",
                         result,
                         currentState ? 1 : 0,
                         changed ? 1 : 0,
                         _mediaObject ? 1 : 0);
...
IOBlockStorageDriver::mediaStateHasChanged(IOMediaState state)
{
    ...
    if (state == kIOMediaStateOnline) {		/* media is now present */
        PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV mediaState online\n");
        ...
        PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV recordMediaParameters ret=0x%x blockSize=%llu maxBlock=%llu writeProtected=%d\n",
                             result,
                             _mediaBlockSize,
                             _maxBlockNumber,
                             _writeProtected ? 1 : 0);
        ...
        PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV acceptNewMedia ret=0x%x\n", result);
        if (result != kIOReturnSuccess) {
        ...
        PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV mediaState offline\n");
...
IOBlockStorageDriver::handleStart(IOService * provider)
{
    ...
    PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV reportRemovability ret=0x%x removable=%d\n",
                         result, _removable ? 1 : 0);
    if (result != kIOReturnSuccess) {
    ...
    if (_removable) {
        ...
        PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV reportEjectability ret=0x%x ejectable=%d\n",
                             result, _ejectable ? 1 : 0);
        if (result != kIOReturnSuccess) {
    ...
    PANTHERA_BSDRV_TRACE("PANTHERA:BSDRV checkForMedia ret=0x%x\n", result);
