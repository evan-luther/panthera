...
#define PANTHERA_FDISK_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
IOService * IOFDiskPartitionScheme::probe(IOService * provider, SInt32 * score)
{
    ...
    PANTHERA_FDISK_TRACE("PANTHERA:FDISK probe provider=%s score=%ld\n",
    ...
    PANTHERA_FDISK_TRACE("PANTHERA:FDISK probe scan partitions=%u score=%ld\n",
...
bool IOFDiskPartitionScheme::start(IOService * provider)
{
    ...
    PANTHERA_FDISK_TRACE("PANTHERA:FDISK start partitions=%u\n",
                         _partitions ? _partitions->getCount() : 0U);
    ...
    while ( (partition = (IOMedia *) partitionIterator->getNextObject()) )
    {
        if ( partition->attach(this) )
        {
            ...
            PANTHERA_FDISK_TRACE("PANTHERA:FDISK publish name=%s content=%s whole=%d\n",
                                 partition->getName(),
...
OSSet * IOFDiskPartitionScheme::scan(SInt32 * score)
{
    ...
    PANTHERA_FDISK_TRACE("PANTHERA:FDISK scan enter media=%s blockSize=%llu size=%llu\n",
    ...
    if ( media->isFormatted() == false ) {
        PANTHERA_FDISK_TRACE("PANTHERA:FDISK scan media not formatted\n");
        goto scanErr;
    }
    ...
    if ( (mediaBlockSize % sizeof(disk_blk0)) ) {
        PANTHERA_FDISK_TRACE("PANTHERA:FDISK scan bad block size=%llu\n", mediaBlockSize);
        goto scanErr;
    }
    ...
    if ( buffer == 0 ) {
        PANTHERA_FDISK_TRACE("PANTHERA:FDISK scan buffer alloc failed size=%llu\n", bufferSize);
        goto scanErr;
    }
    ...
    if ( partitions == 0 ) {
        PANTHERA_FDISK_TRACE("PANTHERA:FDISK scan partition set alloc failed\n");
        goto scanErr;
    }
    ...
    if ( mediaIsOpen == false ) {
        PANTHERA_FDISK_TRACE("PANTHERA:FDISK scan open failed\n");
        goto scanErr;
    }
    ...
    do
    {
        ...
        PANTHERA_FDISK_TRACE("PANTHERA:FDISK scan read block=%u status=0x%x\n",
                             fdiskBlock, status);
