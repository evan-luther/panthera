...
///protected:	User Client doesn't like some protected fields and methods.
    ...
    bool	reclaimTransmitDescriptors();
    ...
    inline void csrWrite16Slow( UInt16 offset, UInt16 value )
    {
        ...
        if ( fCSRIsIOPort ) {
            ...
            OSWriteLittleInt16( csrBase, offset, value );
            OSReadLittleInt16( csrBase, offset );
        }
	}/* end csrWrite16Slow */
    ...
    {
        return fCSRIsIOPort ? pciNub->ioRead32( offset, csrMap )
                            : OSReadLittleInt32( csrBase, offset );
    }

    inline UInt16 csrRead16( UInt16 offset )
    {
        return fCSRIsIOPort ? pciNub->ioRead16( offset, csrMap )
                            : OSReadLittleInt16( csrBase, offset );
    }

    inline UInt8 csrRead8( UInt16 offset )
    {
        return fCSRIsIOPort ? pciNub->ioRead8( offset, csrMap )
                            : *((UInt8*)csrBase + offset);
    }
