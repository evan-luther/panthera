...
static UInt16 panthera_read_be16(const UInt8 *p)
{
	return ((UInt16)p[0] << 8) | p[1];
}

static void panthera_log_ipv4_addr(const UInt8 *addr)
{
	IOLog("%u.%u.%u.%u", addr[0], addr[1], addr[2], addr[3]);
}

static void panthera_rtl_trace_packet(const char *dir, const UInt8 *frame, UInt32 len,
									  UInt8 index, UInt32 aux)
{
	if (!panthera_rtl_packet_trace_enabled() || frame == NULL || len < 14)
		return;

	UInt16 etherType = panthera_read_be16(frame + 12);

	if (etherType == 0x0806 && len >= 42)
	{
		const UInt8 *arp = frame + 14;
		UInt16 op = panthera_read_be16(arp + 6);

		IOLog("PANTHERA:RTL8139 pkt %s idx=%u len=%u aux=0x%x type=0x0806 arp op=%u spa=",
			  dir, index, len, aux, op);
		panthera_log_ipv4_addr(arp + 14);
		IOLog(" tpa=");
		panthera_log_ipv4_addr(arp + 24);
		IOLog("\n");
		return;
	}
	...
	if (etherType == 0x0800 && len >= 34)
	{
		...
		UInt8 ihl = (ip[0] & 0x0f) * 4;
		UInt8 proto = ip[9];

		if (ihl < 20 || len < (UInt32)(14 + ihl))
			return;

		IOLog("PANTHERA:RTL8139 pkt %s idx=%u len=%u aux=0x%x type=0x0800 ip proto=%u src=",
			  dir, index, len, aux, proto);
		panthera_log_ipv4_addr(ip + 12);
		IOLog(" dst=");
		panthera_log_ipv4_addr(ip + 16);

		if ((proto == 6 || proto == 17) && len >= (UInt32)(14 + ihl + 4))
		{
			const UInt8 *l4 = ip + ihl;
			IOLog(" sport=%u dport=%u", panthera_read_be16(l4), panthera_read_be16(l4 + 2));
		}
	...
	IOLog("PANTHERA:RTL8139 pkt %s idx=%u len=%u aux=0x%x type=0x%04x\n",
		  dir, index, len, aux, etherType);
}
...
bool RTL8139::reclaimTransmitDescriptors()
{
	UInt32	txStatus;
	UInt8	collisions;
    ...
    while ( fTxSendCount )
    {
		...
		ELG( fTxAckIndex, txStatus, ' Tx+', "RTL8139::reclaimTransmitDescriptors" );
        collisions = (txStatus & R_TSD_NCC) >> 24;

        if ( txStatus & R_TSD_TABT ) // transmit abort
        {
			ALRT( fTxAckIndex, txStatus, 'TxA-', "RTL8139::reclaimTransmitDescriptors - transmit abort" );
        ...
        {
			ALRT( fTxAckIndex, txStatus, 'TxU-', "RTL8139::reclaimTransmitDescriptors - transmit underrun" );
            BUMP_ETHER_TX_COUNTER( underruns );
        }

        if ( collisions )
        {
			ALRT( fTxAckIndex, txStatus, 'TxC-', "RTL8139::reclaimTransmitDescriptors - transmit collisions" );
	...
	return reclaimedAny;
}/* end reclaimTransmitDescriptors */

	//---------------------------------------------------------------------------

void RTL8139::transmitterInterrupt( bool *reclaimed )
{
	if ( reclaimTransmitDescriptors() )
		*reclaimed = true;
...
void RTL8139::receiverInterrupt( bool *queued )
{
    ...
    while ( true )
    {
		...
		if ( status == 0xFFFF || count >= 0xFFF0 )
		{
			...
			{
				IOLog("PANTHERA:RTL8139 rx break pending status=0x%x count=0x%x cm=0x%x cba=0x%x capr=0x%x rxoff=0x%x total=%u\n",
				      status, count, csrRead8(RTL_CM), csrRead16(RTL_CBA),
				      csrRead16(RTL_CAPR), fRxOffset, totalBytes);
			}
		...
		if ( totalBytes > 0x10000 )			// Do no more than 1 Ring at a time.
		{
			if (panthera_rtl_packet_trace_enabled())
			{
				IOLog("PANTHERA:RTL8139 rx break ring total=%u cm=0x%x cba=0x%x capr=0x%x rxoff=0x%x\n",
				      totalBytes, csrRead8(RTL_CM), csrRead16(RTL_CBA),
				      csrRead16(RTL_CAPR), fRxOffset);
			}
        ...
        else		/*** Good frame, pass it up:	***/
        {
            ...
            else
            {
				...
				panthera_rtl_trace_packet("rx", (const UInt8 *)mbuf_data(pkt), pktLen, 0xff, status);
	...
	if (panthera_rtl_packet_trace_enabled())
	{
		IOLog("PANTHERA:RTL8139 rx exit total=%u cm=0x%x cba=0x%x capr=0x%x rxoff=0x%x queued=%u\n",
		      totalBytes, csrRead8(RTL_CM), csrRead16(RTL_CBA),
		      csrRead16(RTL_CAPR), fRxOffset, queued && *queued ? 1U : 0U);
	}
	return;
...
void RTL8139::enableHardwareInterrupts()
{
	...
	///	fIMR = R_ISR_TMOUT | R_ISR_TOK | R_ISR_TER | R_ISR_RER | R_ISR_RXOVW | R_ISR_FOVW;
		fIMR = R_ISR_TMOUT | R_ISR_TOK | R_ISR_TER | R_ISR_ROK | R_ISR_RER | R_ISR_RXOVW | R_ISR_FOVW;
	}
	else	// 10 Mbps:
	{
		fIMR = R_ISR_ALL;
	...
	if (panthera_rtl_packet_trace_enabled())
	{
		IOLog("PANTHERA:RTL8139 enable interrupts speed100=%u imr=0x%x cm=0x%x rcr=0x%x rbstart=0x%x\n",
			  fSpeed100 ? 1U : 0U, fIMR, csrRead8(RTL_CM), csrRead32(RTL_RCR), fRxBufferPhys);
	}
...
void RTL8139::interruptOccurred( IOInterruptEventSource *src, int count )
{
	...
	bool rxPending = ((csrRead8(RTL_CM) & R_CM_BUFE) == 0);

	if (panthera_rtl_packet_trace_enabled() &&
		((isr & (R_ISR_ROK | R_ISR_RER | R_ISR_RXOVW | R_ISR_FOVW)) ||
		 (fSpeed100 && rxPending)))
	{
		IOLog("PANTHERA:RTL8139 intr isr=0x%x cm=0x%x cba=0x%x capr=0x%x rxoff=0x%x head=0x%x\n",
			  isr, csrRead8(RTL_CM), csrRead16(RTL_CBA), csrRead16(RTL_CAPR), fRxOffset,
			  OSReadLittleInt16(fpRxBuffer, fRxOffset) << 16 |
			  OSReadLittleInt16(fpRxBuffer, fRxOffset + sizeof(UInt16)));
	}
	...
	rxPending = ((csrRead8(RTL_CM) & R_CM_BUFE) == 0);
...
UInt32 RTL8139::outputPacket( mbuf_t pkt, void *param )
{
    ...
    UInt8				*pDest;
    IODebuggerLockState lockState;
	static UInt32		pantheraOutputTraceCount = 0;
	...
	if ( kOwnedByChip == fTxBufOwnership[ fTxSendIndex ] )
		reclaimTransmitDescriptors();

	tsd = csrRead32( RTL_TSD0 + fTxSendIndex * sizeof( UInt32 ) );
	pktLen = mbuf_pkthdr_len( pkt );

	if ( panthera_rtl_packet_trace_enabled() && pantheraOutputTraceCount < 64 )
	{
		IOLog("PANTHERA:RTL8139 output begin idx=%u len=%ld tsd=0x%x owner=%u\n",
			  fTxSendIndex, pktLen, tsd, fTxBufOwnership[ fTxSendIndex ]);
		pantheraOutputTraceCount++;
	}
...
///	if ( tsd & (R_TSD_CRS | R_TSD_TABT | R_TSD_OWC | R_TSD_CDH | R_TSD_TUN) )
	...
	UInt32 targetLen = (pktLen > 0) ? (UInt32) pktLen : 0;

    for ( mn = pkt; mn && copied < targetLen; mn = mbuf_next( mn ) )
    {
		UInt32 segmentLen = (UInt32) mbuf_len( mn );
		UInt32 remaining = targetLen - copied;

		if ( panthera_rtl_packet_trace_enabled() && pantheraOutputTraceCount < 96 )
		{
			IOLog("PANTHERA:RTL8139 output segment idx=%u seg=%u len=%u remaining=%u next=%p\n",
				  fTxSendIndex, segmentCount, segmentLen, remaining, mbuf_next( mn ));
			pantheraOutputTraceCount++;
		}
		...
		if ( segmentLen != 0 )
		{
			bcopy( mbuf_data( mn ), pDest, segmentLen );
			pDest += segmentLen;
			copied += segmentLen;
		}
		...
		if ( segmentCount >= 32 )
		{
			IOLog("PANTHERA:RTL8139 output abort idx=%u copied=%u target=%u excessive_segments\n",
				  fTxSendIndex, copied, targetLen);
			IODebuggerUnlock( lockState );
			freePacket( pkt );
			BUMP_NET_COUNTER( outputErrors );
	...
	if ( copied != targetLen )
	{
		IOLog("PANTHERA:RTL8139 output abort idx=%u copied=%u target=%u incomplete_chain\n",
			  fTxSendIndex, copied, targetLen);
		IODebuggerUnlock( lockState );
		freePacket( pkt );
		BUMP_NET_COUNTER( outputErrors );
	...
	panthera_rtl_trace_packet("tx", fpTxRxBuffers + TX_BUF_SIZE * fTxSendIndex,
							  (UInt32)pktLen, fTxSendIndex, tsd);
...
#pragma mark -
#pragma mark --- Put EEPROM contents in IORegistry ---
#pragma mark -
