...
#define PANTHERA_RTL8139_PHY_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))

static bool panthera_rtl_interrupts_disabled()
{
	...
	{
		int value = 0;
		disabled = (PE_parse_boot_argn("panthera_rtlpoll", &value, sizeof(value)) && value != 0);
		checked = 1;
	}
...
void RTL8139::phyReportLinkStatus()
{
    ...
    {
        PANTHERA_RTL8139_PHY_TRACE("PANTHERA:RTL8139 phy link change status=0x%x last=0x%x force=%u txq=%p\n",
                                   phyStatus, phyStatusLast, forceLinkChange ? 1U : 0U,
                                   fTransmitQueue);
        ...
        if ( phyStatus & MII_STATUS_LINK_STATUS )
        {
            ...
            if (!panthera_rtl_interrupts_disabled())
            {
                PANTHERA_RTL8139_PHY_TRACE("PANTHERA:RTL8139 phy link up enabling interrupts txq=%p\n",
                                           fTransmitQueue);
                enableHardwareInterrupts();
            }
            else
            {
                PANTHERA_RTL8139_PHY_TRACE("PANTHERA:RTL8139 phy link up deferring interrupts txq=%p\n",
                                           fTransmitQueue);
            }
            PANTHERA_RTL8139_PHY_TRACE("PANTHERA:RTL8139 phy link up medium=%u speed=%u starting_txq=%p\n",
                                       activeMediumIndex, fSpeed100 ? 100000000U : 10000000U,
                                       fTransmitQueue);
        ...
        else							//  Link down:
        {
            ...
            PANTHERA_RTL8139_PHY_TRACE("PANTHERA:RTL8139 phy link down stopping_txq=%p\n",
                                       fTransmitQueue);
			fTransmitQueue->stop();
