...
static void
panthera_launchd_trace(const char *s)
{
	size_t len = 0;
...
main(int argc, char *const *argv)
{
	...
	panthera_launchd_trace("launchd: main enter\n");
	...
	panthera_launchd_trace("launchd: after getopt\n");

	if (getpid() != 1 && getppid() != 1) {
		panthera_launchd_trace("launchd: rejecting direct exec\n");
		fprintf(stderr, "%s: This program is not meant to be run directly.\n", getprogname());
		exit(EXIT_FAILURE);
	}
	panthera_launchd_trace("launchd: after pid check\n");

	launchd_runtime_init();
	panthera_launchd_trace("launchd: after runtime_init\n");

	if (NULL == getenv("PATH")) {
		setenv("PATH", _PATH_STDPATH, 1);
	}
	panthera_launchd_trace("launchd: after PATH init\n");

	if (pid1_magic) {
		/* Panthera: minimal PID 1 init — skip audit, syslog, signal handlers */
		...
		panthera_launchd_trace("*** launchd[1] has started up. ***\n");
	} else {
		panthera_launchd_trace("launchd: entering per-user branch\n");
		launchd_uid = getuid();
	...
	/* Skip monitor_networking_state — sockets not ready for PID 1 bring-up */
	panthera_launchd_trace("launchd: after monitor_networking_state\n");
	jobmgr_init(sflag);
	panthera_launchd_trace("launchd: after jobmgr_init\n");

#if PANTHERA
	/* Load LaunchDaemon plists directly. Dispatch is deferred until the
	 * runtime kqueue is active so services can safely bootstrap_check_in(). */
	if (pid1_magic) {
		panthera_load_jobs();
	}
#endif
	...
	/*
	 * Full runtime signal setup reapplies the ignored-signal table;
	 * the early PID-1 SIGUSR1 disposition closes the exec-to-runtime initialization window.
	 */
	launchd_runtime_init2();
#if PANTHERA
	if (pid1_magic) {
		panthera_dispatch_loaded_jobs();
	}
#endif
	panthera_launchd_trace("launchd: entering runtime\n");
	launchd_runtime();
}
