...
#define PANTHERA_APCI_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
OSDefineMetaClassAndStructors(AppleI386PCI, IOPCIHostBridge)
...
bool AppleI386PCI::start( IOService * provider )
{
    ...
    PANTHERA_APCI_TRACE("PANTHERA:APCI start entered provider=%s\n",
    ...
    PANTHERA_APCI_TRACE("PANTHERA:APCI start provider=%s maxBus=%u cm1=%u cm2=%u bios16=%u\n",
                        provider ? provider->getName() : "<null>",
                        maxBusNum,
                        configMethod1,
                        configMethod2,
...
bool AppleI386PCI::configure( IOService * provider )
{
    ...
    PANTHERA_APCI_TRACE("PANTHERA:APCI configure provider=%s io=0x10000 memBase=0x80000000 memSize=0x7f000000\n",
                        provider ? provider->getName() : "<null>");
...
IOReturn AppleI386PCI::setLinkSpeed( tIOPCILinkSpeed, bool )
{
    return kIOReturnUnsupported;
}

IOReturn AppleI386PCI::getLinkSpeed( tIOPCILinkSpeed * linkSpeed )
{
    if (linkSpeed) {
