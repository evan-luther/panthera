...
{	
	...
	protected:
		...
		struct V3ExpansionData { 
            ...
            bool					_wakingFromStandby;					// t when waking from S4 standby
			UInt32					_pantheraBootstrapHolds;
		};
		...
		virtual uint64_t		GetErrata64Bits(UInt16 vendorID, UInt16 deviceID, UInt16 revisionID) { return 0; }
	...
	public:
		...
		IOReturn						PantheraDeviceRequestWithTimeouts(IOUSBDevRequest *request, IOUSBCompletion *completion, USBDeviceAddress address, UInt8 epNum, UInt32 noDataTimeout, UInt32 completionTimeout);
		IOReturn						PantheraDeviceRequestDescWithTimeouts(IOUSBDevRequestDesc *request, IOUSBCompletion *completion, USBDeviceAddress address, UInt8 epNum, UInt32 noDataTimeout, UInt32 completionTimeout);
		void							PantheraBeginDeviceBootstrap(void);
		void							PantheraEndDeviceBootstrap(void);
		bool							PantheraDeviceBootstrapActive(void) const { return _v3ExpansionData && (_v3ExpansionData->_pantheraBootstrapHolds != 0); }
