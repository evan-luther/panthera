...
static IOUSBCommand *
CreateUSBCommandForController(IOUSBController *controller)
{
    IOUSBCommand *command = IOUSBCommand::NewCommand();
    IOUSBControllerV2 *controllerV2 = OSDynamicCast(IOUSBControllerV2, controller);
    ...
    if (controllerV2)
    {
        IODMACommand *dmaCommand = controllerV2->GetNewDMACommand();
        if (!dmaCommand)
        {
            USBError(1, "%s[%p]::CreateUSBCommandForController - missing DMA command", controller->getName(), controller);
            command->release();
            return NULL;
        }
        command->SetDMACommand(dmaCommand);
    }

    return command;
}

static IOUSBIsocCommand *
CreateUSBIsocCommandForController(IOUSBController *controller)
{
    IOUSBIsocCommand *command = IOUSBIsocCommand::NewCommand();
    IOUSBControllerV2 *controllerV2 = OSDynamicCast(IOUSBControllerV2, controller);
    ...
    if (controllerV2)
    {
        ...
        if (!dmaCommand)
        {
            USBError(1, "%s[%p]::CreateUSBIsocCommandForController - missing DMA command", controller->getName(), controller);
            command->release();
            return NULL;
        }
        command->SetDMACommand(dmaCommand);
...
UInt32                  gEHCIBandwidthLogLevel = 7;
UInt32                  gEHCIRLvalue = 15;
...
IOUSBController::start( IOService * provider )
{
	...
	{
	    IOUSBCommand *command = CreateUSBCommandForController(this);
	    if (command)
	...
	{
	    IOUSBIsocCommand *icommand = CreateUSBIsocCommandForController(this);
	    if (icommand)
...
{
    IOUSBControllerV3 *controllerV3 = OSDynamicCast(IOUSBControllerV3, this);
    ...
    IOLog("PANTHERA:IOUSBController CreateDevice begin addr=%u speed=%u power=%u v3=%d onWL=%d\n",
          (unsigned)deviceAddress,
          (unsigned)speed,
          (unsigned)powerAvailable,
          controllerV3 ? 1 : 0,
          (_workLoop && _workLoop->onThread()) ? 1 : 0);
    
    do 
    {
        if (controllerV3)
            controllerV3->PantheraBeginDeviceBootstrap();
        ...
        if (!newDevice->init(deviceAddress, powerAvailable, speed, maxPacketSize))
        {
            ...
            IOLog("PANTHERA:IOUSBController CreateDevice init failed addr=%u\n", (unsigned)deviceAddress);
            break;
        }
        ...
        if (!newDevice->attach(this))
        {
            ...
            IOLog("PANTHERA:IOUSBController CreateDevice attach failed addr=%u\n", (unsigned)deviceAddress);
            break;
        }
        
        IOLog("PANTHERA:IOUSBController CreateDevice start call addr=%u onWL=%d\n",
              (unsigned)deviceAddress,
              (_workLoop && _workLoop->onThread()) ? 1 : 0);
        ...
        if (!newDevice->start(this))
        {
            ...
            IOLog("PANTHERA:IOUSBController CreateDevice start failed addr=%u\n", (unsigned)deviceAddress);
        ...
        IOLog("PANTHERA:IOUSBController CreateDevice success addr=%u\n", (unsigned)deviceAddress);

        if (controllerV3)
            controllerV3->PantheraEndDeviceBootstrap();
...
IOUSBController::DeviceRequest(IOUSBDevRequest *request, IOUSBCompletion *completion, USBDeviceAddress address, UInt8 ep)
{
    ...
    if (controllerV3)
    {
        IOLog("PANTHERA:IOUSBController DeviceRequest4->V3 addr=%u ep=%u bm=0x%x req=0x%x\n",
              (unsigned)address,
              (unsigned)ep,
              request ? (unsigned)request->bmRequestType : 0,
              request ? (unsigned)request->bRequest : 0);
        return controllerV3->PantheraDeviceRequestWithTimeouts(request, completion, address, ep,
                                                               ep ? 0 : kUSBDefaultControlNoDataTimeoutMS,
                                                               ep ? 0 : kUSBDefaultControlCompletionTimeoutMS);
    }
...
IOUSBController::DeviceRequest(IOUSBDevRequestDesc *request, IOUSBCompletion *	completion, USBDeviceAddress address, UInt8 ep)
{
    ...
    if (controllerV3)
    {
        IOLog("PANTHERA:IOUSBController DeviceRequestDesc4->V3 addr=%u ep=%u bm=0x%x req=0x%x\n",
              (unsigned)address,
              (unsigned)ep,
              request ? (unsigned)request->bmRequestType : 0,
              request ? (unsigned)request->bRequest : 0);
        return controllerV3->PantheraDeviceRequestDescWithTimeouts(request, completion, address, ep,
                                                                   ep ? 0 : kUSBDefaultControlNoDataTimeoutMS,
                                                                   ep ? 0 : kUSBDefaultControlCompletionTimeoutMS);
    }
                                            ...
                                            UInt32	bufferSizeRemaining)
{
    ...
    if (status != kIOReturnSuccess)
    {
	    ...
	    if (todo == 0)
	    {
	        ...
	        IOMemoryDescriptor *requestMemDesc = command->GetRequestMemoryDescriptor();
	        IOMemoryDescriptor *bufferMemDesc = command->GetBufferMemoryDescriptor();
	        UInt16 bytesTransferred = request->wLength - command->GetDataRemaining();
	        IOUSBCompletion clientCompletion = command->GetClientCompletion();
        ...
        if (bufferMemDesc)
        {
            if ((((request->bmRequestType >> kUSBRqDirnShift) & kUSBRqDirnMask) == kUSBIn) &&
                request->pData && bytesTransferred)
            {
                bufferMemDesc->readBytes(0, request->pData, bytesTransferred);
            }
            command->SetBufferMemoryDescriptor(NULL);
            bufferMemDesc->complete();
            bufferMemDesc->release();
        }
        if (requestMemDesc)
        {
            command->SetRequestMemoryDescriptor(NULL);
            requestMemDesc->complete();
            requestMemDesc->release();
        }
       ...
       if ( status == kIOReturnUnderrun )
	        ...
	        if (command->GetStatus() != kIOReturnSuccess)
		{
	        ...
	        me->Complete(clientCompletion, command->GetStatus(), command->GetDataRemaining());
...
IOUSBController::GetDeviceZeroDescriptor(IOUSBDeviceDescriptor *desc, UInt16 size)
{
    ...
    do
    {
        ...
        IOLog("PANTHERA:GetDeviceZeroDescriptor waiting size=%u\n", (unsigned)size);
        err = syncer->wait(false);
        syncer->release();
        IOLog("PANTHERA:GetDeviceZeroDescriptor done err=0x%x bMaxPacketSize0=%u\n", err, (unsigned)desc->bMaxPacketSize0);
...
IOUSBController::SetDeviceZeroAddress(USBDeviceAddress address) 
{
    ...
    do
    {
        ...
        IOLog("PANTHERA:SetDeviceZeroAddress waiting addr=%u\n", (unsigned)address);
        err = syncer->wait(false);
        syncer->release();
        IOLog("PANTHERA:SetDeviceZeroAddress done err=0x%x addr=%u\n", err, (unsigned)address);
...
IOUSBController::DeviceRequest(IOUSBDevRequest *request, IOUSBCompletion *completion, USBDeviceAddress address, UInt8 ep, UInt32 noDataTimeout, UInt32 completionTimeout)
{
    ...
    if (completion && ((uintptr_t)completion->action == (uintptr_t)&IOUSBSyncCompletion))
    {
        isSyncTransfer = true;
        if (_workLoop->onThread())
        {
            USBError(1, "IOUSBController(%s)[%p]::DeviceRequest sync request on workloop thread. Use async!", getName(), this);
...
IOUSBController::DeviceRequest(IOUSBDevRequestDesc *request, IOUSBCompletion *	completion, USBDeviceAddress address, UInt8 ep, UInt32 noDataTimeout, UInt32 completionTimeout)
{
    ...
    if (completion && ((uintptr_t)completion->action == (uintptr_t)&IOUSBSyncCompletion))
    {
        ...
        if (_workLoop->onThread())
        {
            USBError(1, "IOUSBController(%s)[%p]::DeviceRequestDesc sync request on workloop thread. Use async!", getName(), this);
            return kIOUSBSyncRequestOnWLThread;
        }
    }
