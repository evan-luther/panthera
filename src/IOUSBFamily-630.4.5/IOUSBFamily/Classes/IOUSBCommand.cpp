...
IOUSBCommand::free()
{
    ...
    //  variables.
    //
    if (_expansionData && _expansionData->_dmaCommand)
    {
		if (_expansionData->_dmaCommand->getMemoryDescriptor())
			_expansionData->_dmaCommand->clearMemoryDescriptor();
		_expansionData->_dmaCommand->release();
		_expansionData->_dmaCommand = NULL;
    }
    if (_expansionData)
    {
