...
static IOReturn
PantheraRunDeviceRequestAction(IOUSBDevice *device,
                               IOWorkLoop *workLoop,
                               IOCommandGate *gate,
                               IOCommandGate::Action action,
                               void *request,
                               ...
                               IOUSBCompletion *completion)
{
	...
	if (!device->isInactive() && gate && workLoop)
	{
		device->retain();
		workLoop->retain();
		gate->retain();

		kr = gate->runAction(action,
					 request,
					 (void *)(uintptr_t)noDataTimeout,
					 (void *)(uintptr_t)completionTimeout,
					 (void *)completion);
...
static IOReturn
PantheraSyncDeviceRequest(IOUSBDevice *device,
                          IOWorkLoop *workLoop,
                          IOCommandGate *gate,
                          USBDeviceAddress address,
                          IOUSBController *controller,
                          IOCommandGate::Action action,
                          ...
                          UInt32 completionTimeout)
{
	...
	if (device && controller && controller->getWorkLoop())
	{
		IOLog("PANTHERA:PantheraSyncDeviceRequest addr=%u deviceWL=%d controllerWL=%d req=0x%x wLen=%u noData=%u completion=%u\n",
		      (unsigned)address,
		      (workLoop && workLoop->onThread()) ? 1 : 0,
		      controller->getWorkLoop()->onThread() ? 1 : 0,
		      request ? (unsigned)request->bRequest : 0,
		      request ? (unsigned)request->wLength : 0,
	...
	kr = PantheraRunDeviceRequestAction(device, workLoop, gate, action, request, noDataTimeout, completionTimeout, &tap);
	if (kr == kIOReturnSuccess)
	{
		kr = syncer->wait(false);
	}
...
IOUSBDevice::start( IOService * provider )
{
	...
	IOLog("PANTHERA:IOUSBDevice start this=%p provider=%s controller=%p controllerWL=%d\n",
	      this,
	      provider ? provider->getName() : "<none>",
	      _controller,
	      (_controller && _controller->getWorkLoop() && _controller->getWorkLoop()->onThread()) ? 1 : 0);
...
IOUSBDevice::DeviceRequest(IOUSBDevRequest *request, IOUSBCompletion *completion)
{
	if (completion == NULL)
	{
		return PantheraSyncDeviceRequest(this, _WORKLOOP, _COMMAND_GATE, _address, _controller, _DeviceRequest, request, 0, 0);
	}

	return PantheraRunDeviceRequestAction(this, _WORKLOOP, _COMMAND_GATE, _DeviceRequest, request, 0, 0, completion);
}
...
IOUSBDevice::DeviceRequest(IOUSBDevRequestDesc	*request, IOUSBCompletion *completion)
{
	if (completion == NULL)
	{
		return PantheraSyncDeviceRequest(this, _WORKLOOP, _COMMAND_GATE, _address, _controller, _DeviceRequestDesc, (IOUSBDevRequest *)request, 0, 0);
	}

	return PantheraRunDeviceRequestAction(this, _WORKLOOP, _COMMAND_GATE, _DeviceRequestDesc, request, 0, 0, completion);
}
...
IOUSBDevice::DeviceRequest(IOUSBDevRequest *request, UInt32 noDataTimeout, UInt32 completionTimeout, IOUSBCompletion *completion)
{
	if (completion == NULL)
	{
		return PantheraSyncDeviceRequest(this, _WORKLOOP, _COMMAND_GATE, _address, _controller, _DeviceRequestWithTimeout, request, noDataTimeout, completionTimeout);
	}

	return PantheraRunDeviceRequestAction(this, _WORKLOOP, _COMMAND_GATE, _DeviceRequestWithTimeout, request, noDataTimeout, completionTimeout, completion);
}
...
IOUSBDevice::DeviceRequest(IOUSBDevRequestDesc *request, UInt32 noDataTimeout, UInt32 completionTimeout, IOUSBCompletion *completion)
{
	if (completion == NULL)
	{
		return PantheraSyncDeviceRequest(this, _WORKLOOP, _COMMAND_GATE, _address, _controller, _DeviceRequestDescWithTimeout, (IOUSBDevRequest *)request, noDataTimeout, completionTimeout);
	}

	return PantheraRunDeviceRequestAction(this, _WORKLOOP, _COMMAND_GATE, _DeviceRequestDescWithTimeout, request, noDataTimeout, completionTimeout, completion);
}
