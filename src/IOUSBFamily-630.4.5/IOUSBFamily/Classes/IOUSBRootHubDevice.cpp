            ...
            switch (dType) {
                case kUSBDeviceDesc:
				{
	                ...
	                case kUSBConfDesc:
	                {
						...
						{
							const UInt8 *bytes = (const UInt8 *)fullDesc->getBytesNoCopy();
							IOLog("PANTHERA:RootHub confdesc len=%u addr=%u speed=%u bytes=%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
							      (unsigned)fullDesc->getLength(),
							      (unsigned)_address,
							      (unsigned)_speed,
							      bytes && fullDesc->getLength() > 0 ? bytes[0] : 0,
							      bytes && fullDesc->getLength() > 1 ? bytes[1] : 0,
							      bytes && fullDesc->getLength() > 2 ? bytes[2] : 0,
							      bytes && fullDesc->getLength() > 3 ? bytes[3] : 0,
							      bytes && fullDesc->getLength() > 4 ? bytes[4] : 0,
							      bytes && fullDesc->getLength() > 5 ? bytes[5] : 0,
							      bytes && fullDesc->getLength() > 6 ? bytes[6] : 0,
							      bytes && fullDesc->getLength() > 7 ? bytes[7] : 0,
							      bytes && fullDesc->getLength() > 8 ? bytes[8] : 0,
							      bytes && fullDesc->getLength() > 9 ? bytes[9] : 0,
							      bytes && fullDesc->getLength() > 10 ? bytes[10] : 0,
							      bytes && fullDesc->getLength() > 11 ? bytes[11] : 0);
