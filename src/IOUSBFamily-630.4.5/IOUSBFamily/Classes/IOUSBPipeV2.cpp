...
IOUSBPipeV2::InitToEndpoint(const IOUSBEndpointDescriptor *ed, IOUSBSuperSpeedEndpointCompanionDescriptor *sscd, UInt8 speed, USBDeviceAddress address, IOUSBController * controller, IOUSBDevice * device, IOUSBInterface * interface)
{
    ...
    if ((err == kIOReturnNoBandwidth) && (_endpoint.transferType == kUSBIsoc))
    {
	    ...
	    {
	        if (interface && interface->GetInterfaceClass() == kUSBHubClass)
	        {
	            IOLog("PANTHERA:IOUSBPipeV2 InitToEndpoint failed addr=%u ep=0x%02x type=%u dir=%u mps=%u interval=%u err=0x%x\n",
	                  device ? (unsigned)device->GetAddress() : 0,
	                  (unsigned)ed->bEndpointAddress,
	                  (unsigned)_endpoint.transferType,
	                  (unsigned)_endpoint.direction,
	                  (unsigned)_endpoint.maxPacketSize,
	                  (unsigned)_endpoint.interval,
	                  (unsigned)err);
	        }
	    ...
	    if (interface && interface->GetInterfaceClass() == kUSBHubClass)
	    {
	        IOLog("PANTHERA:IOUSBPipeV2 InitToEndpoint ok addr=%u ep=0x%02x type=%u dir=%u mps=%u interval=%u\n",
	              device ? (unsigned)device->GetAddress() : 0,
	              (unsigned)ed->bEndpointAddress,
	              (unsigned)_endpoint.transferType,
	              (unsigned)_endpoint.direction,
