...
IOUSBControllerV2::ReadV2(IOMemoryDescriptor *buffer, USBDeviceAddress address, Endpoint *endpoint, IOUSBCompletionWithTimeStamp *completion, UInt32 noDataTimeout, UInt32 completionTimeout, IOByteCount reqCount)
{
    ...
    err = this->CheckForDisjointDescriptor(command, endpoint->maxPacketSize);
    if (kIOReturnSuccess == err)
	{
...
IOUSBControllerV2::ReturnIsochDoneQueue(IOUSBControllerIsochEndpoint* pEP)
{
    ...
    while(pTD)
    {
		...
		if ( pTD->_completion.action != NULL)
		{
			...
			else if (!_activeIsochTransfers && (_isochMaxBusStall != 0))
            {
...
IOUSBControllerV2::UpdateDeviceAddress(USBDeviceAddress oldDeviceAddress,
                                       ...
                                       int port)
{
    if ((oldDeviceAddress > kUSBMaxDevices) || (newDeviceAddress > kUSBMaxDevices))
    {
        return kIOReturnBadArgument;
    }
    ...
    if (speed < kUSBDeviceSpeedHigh)
    {
        if (_highSpeedHub[hubAddress] == 0)
        {
            _highSpeedHub[newDeviceAddress] = hubAddress;
            _highSpeedPort[newDeviceAddress] = port;
        }
        else
        {
            _highSpeedHub[newDeviceAddress] = _highSpeedHub[hubAddress];
            _highSpeedPort[newDeviceAddress] = _highSpeedPort[hubAddress];
        }
    }
    else
    {
        _highSpeedHub[newDeviceAddress] = 0;
        _highSpeedPort[newDeviceAddress] = 0;
    }

    if (oldDeviceAddress != newDeviceAddress)
    {
        _highSpeedHub[oldDeviceAddress] = 0;
        _highSpeedPort[oldDeviceAddress] = 0;
    }
