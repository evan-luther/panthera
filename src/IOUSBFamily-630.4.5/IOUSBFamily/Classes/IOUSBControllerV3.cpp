...
{
    return super::GetErrataBits(vendorID, deviceID, revisionID);
}
...
{
	USBTrace_Start( kUSBTController, kTPControllerPowerState, (uintptr_t)this, (int)powerStateOrdinal, (uintptr_t)whatDevice, (int)_myPowerState );
	...
	if ( whatDevice != this )
	{
		...
		USBTrace( kUSBTController, kTPControllerPowerState, (uintptr_t)this, 0, 0, 1 );
	...
	if ( isInactive() )
	{
		...
		USBTrace( kUSBTController, kTPControllerPowerState, (uintptr_t)this, 0, 0, 2 );
	...
	if (powerStateOrdinal > kUSBPowerStateOn)
	{
		...
		USBTrace( kUSBTController, kTPControllerPowerState, (uintptr_t)this, 0, 0, 3 );
	...
	USBTrace_End( kUSBTController, kTPControllerPowerState, (uintptr_t)this, _myPowerState, 0, 0);
...
IOUSBControllerV3::AcquireDeviceZero( void )
{
	...
	// Device-zero enumeration needs the controller fully awake so that the UIM timeout
	// and completion machinery see a running bus. Leaving the controller in LowPower
	// here lets synthetic/root-hub paths work while downstream enumeration stalls.
	if ((_powerStateChangingTo == kUSBPowerStateStable) || (_powerStateChangingTo > kUSBPowerStateSleep))
	{
		IOLog("PANTHERA:IOUSBControllerV3 AcquireDeviceZero request power ON state=%d changingTo=%d available=%d\n",
		      (int)_myPowerState,
		      (int)_powerStateChangingTo,
		      _controllerAvailable ? 1 : 0);
...
{
	IOReturn	kr;
	bool		pendingBootstrapAddress = false;
	UInt32		i;
	...
	{
		if (_addressPending[i])
		{
			pendingBootstrapAddress = true;
			break;
		}
	}

	if (PantheraDeviceBootstrapActive() || pendingBootstrapAddress)
	{
		IOLog("PANTHERA:IOUSBControllerV3 ReleaseDeviceZero keep power ON for bootstrap holds=%u pending=%d state=%d changingTo=%d available=%d\n",
		      (unsigned)_v3ExpansionData->_pantheraBootstrapHolds,
		      pendingBootstrapAddress ? 1 : 0,
		      (int)_myPowerState,
		      (int)_powerStateChangingTo,
		      _controllerAvailable ? 1 : 0);
	}
	else if ((_powerStateChangingTo == kUSBPowerStateStable) || (_powerStateChangingTo > kUSBPowerStateSleep))
	{
		IOLog("PANTHERA:IOUSBControllerV3 ReleaseDeviceZero back to LowPower state=%d changingTo=%d available=%d\n",
		      (int)_myPowerState,
		      (int)_powerStateChangingTo,
		      _controllerAvailable ? 1 : 0);
		changePowerStateTo(kUSBPowerStateLowPower);
...
IOReturn
IOUSBControllerV3::PantheraDeviceRequestWithTimeouts(IOUSBDevRequest *request, IOUSBCompletion *completion, USBDeviceAddress address, UInt8 epNum, UInt32 noDataTimeout, UInt32 completionTimeout)
{
	return DeviceRequest(request, completion, address, epNum, noDataTimeout, completionTimeout);
}

IOReturn
IOUSBControllerV3::PantheraDeviceRequestDescWithTimeouts(IOUSBDevRequestDesc *request, IOUSBCompletion *completion, USBDeviceAddress address, UInt8 epNum, UInt32 noDataTimeout, UInt32 completionTimeout)
{
	return DeviceRequest(request, completion, address, epNum, noDataTimeout, completionTimeout);
}

void
IOUSBControllerV3::PantheraBeginDeviceBootstrap(void)
{
	if (!_v3ExpansionData)
		return;

	_v3ExpansionData->_pantheraBootstrapHolds++;
	IOLog("PANTHERA:IOUSBControllerV3 begin bootstrap holds=%u state=%d changingTo=%d available=%d\n",
	      (unsigned)_v3ExpansionData->_pantheraBootstrapHolds,
	      (int)_myPowerState,
	      (int)_powerStateChangingTo,
	      _controllerAvailable ? 1 : 0);
...
void
IOUSBControllerV3::PantheraEndDeviceBootstrap(void)
{
	if (!_v3ExpansionData || (_v3ExpansionData->_pantheraBootstrapHolds == 0))
		return;

	_v3ExpansionData->_pantheraBootstrapHolds--;
	IOLog("PANTHERA:IOUSBControllerV3 end bootstrap holds=%u state=%d changingTo=%d available=%d\n",
	      (unsigned)_v3ExpansionData->_pantheraBootstrapHolds,
	      (int)_myPowerState,
	      (int)_powerStateChangingTo,
	      _controllerAvailable ? 1 : 0);

	if ((_v3ExpansionData->_pantheraBootstrapHolds == 0) &&
	    ((_powerStateChangingTo == kUSBPowerStateStable) || (_powerStateChangingTo > kUSBPowerStateSleep)))
	{
		changePowerStateTo(kUSBPowerStateLowPower);
	}
}
...
{
	IOReturn	kr;
	bool		devZeroRequest = ((address == 0) && (epNum == 0));
	bool		syncRequest = (completion && ((uintptr_t)completion->action == (uintptr_t)&IOUSBSyncCompletion));
	bool		bootstrapEp0Request = (PantheraDeviceBootstrapActive() && (epNum == 0) && _controllerAvailable);
	bool		syncEp0Request = (syncRequest && (epNum == 0) && _controllerAvailable);
	...
	if ((bootstrapEp0Request || syncEp0Request) && (_myPowerState >= kUSBPowerStateOn))
	{
		WakeControllerFromDoze();
	}
	...
	if (devZeroRequest || PantheraDeviceBootstrapActive() || syncEp0Request)
	{
	}
	else if ((_powerStateChangingTo == kUSBPowerStateStable) || (_powerStateChangingTo > kUSBPowerStateSleep))
	{
...
{
	IOReturn	kr;
	bool		rootHubInterruptRead = false;
	
	if (endpoint && endpoint->transferType == kUSBInterrupt && _controllerAvailable)
	{
		if (_rootHubDevice && (address == _rootHubDevice->GetAddress()))
			rootHubInterruptRead = true;
		else if (_rootHubDeviceSS && (address == _rootHubDeviceSS->GetAddress()))
			rootHubInterruptRead = true;
	}
	
	if (rootHubInterruptRead && (_myPowerState < kUSBPowerStateOn))
	{
		IOLog("PANTHERA:IOUSBControllerV3 bypass Read power gate addr=%u state=%d available=%d\n",
		      (unsigned)address,
		      (int)_myPowerState,
		      _controllerAvailable ? 1 : 0);
		kr = kIOReturnSuccess;
	...
	if (rootHubInterruptRead)
	{
		IOLog("PANTHERA:IOUSBControllerV3 direct RootHubQueueInterruptRead addr=%u req=%llu\n",
		      (unsigned)address,
		      (unsigned long long)reqCount);
		return RootHubQueueInterruptRead(buffer, (UInt32)reqCount, *completion);
	}
...
{	
	IOReturn	kr;
	bool		rootHubInterruptOpen = false;
	bool		deviceZeroControlOpen = false;
	
	if (endpoint && endpoint->transferType == kUSBInterrupt && _controllerAvailable)
	{
		if (_rootHubDevice && (address == _rootHubDevice->GetAddress()))
			rootHubInterruptOpen = true;
		else if (_rootHubDeviceSS && (address == _rootHubDeviceSS->GetAddress()))
			rootHubInterruptOpen = true;
	}

	if (endpoint && _controllerAvailable &&
	    (address == 0) &&
	    (endpoint->transferType == kUSBControl) &&
	    (endpoint->number == 0))
	{
		deviceZeroControlOpen = true;
	}
		
	if ((rootHubInterruptOpen || deviceZeroControlOpen) && (_myPowerState < kUSBPowerStateOn))
	{
		IOLog("PANTHERA:IOUSBControllerV3 bypass OpenPipe power gate addr=%u ep=%u type=%u state=%d available=%d\n",
		      (unsigned)address,
		      endpoint ? (unsigned)endpoint->number : 0,
		      endpoint ? (unsigned)endpoint->transferType : 0,
		      (int)_myPowerState,
		      _controllerAvailable ? 1 : 0);
		kr = kIOReturnSuccess;
	}
