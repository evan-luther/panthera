...
IOUSBInterface::handleOpen( IOService *forClient, IOOptionBits options, void *arg )
{
	...
	else
	{
		...
		if (result)
		{
			...
			{
				if (_bInterfaceClass == kUSBHubClass)
				{
					IOLog("PANTHERA:IOUSBInterface handleOpen name=%s addr=%u intf=%u alt=%u endpoints=%u config=%u\n",
					      getName(),
					      _device ? (unsigned)_device->GetAddress() : 0,
					      (unsigned)_bInterfaceNumber,
					      (unsigned)_bAlternateSetting,
					      (unsigned)_bNumEndpoints,
					      (unsigned)GetConfigValue());
				}
				err = CreatePipes();
				if ( err != kIOReturnSuccess) 
				{
...
#pragma mark •••••••• IOUSBInterface static methods •••••••••
...
#pragma mark •••••••• IOUSBInterface class methods •••••••••
bool 
...
IOUSBInterface::CreatePipes(void)
{
    ...
    if (_bInterfaceClass == kUSBHubClass)
    {
        IOLog("PANTHERA:IOUSBInterface CreatePipes begin name=%s addr=%u intf=%u alt=%u endpoints=%u\n",
              getName(),
              _device ? (unsigned)_device->GetAddress() : 0,
              (unsigned)_bInterfaceNumber,
              (unsigned)_bAlternateSetting,
    ...
    while ((pos = FindNextAssociatedDescriptor(pos, kUSBEndpointDesc))) 
    {
        ...
        if (_pipeList[i] == NULL)
		{
			...
			if (_bInterfaceClass == kUSBHubClass)
			{
				IOLog("PANTHERA:IOUSBInterface CreatePipes ep addr=%u ep=0x%02x attr=0x%02x mps=%u interval=%u idx=%u\n",
				      _device ? (unsigned)_device->GetAddress() : 0,
				      (unsigned)ep->bEndpointAddress,
				      (unsigned)ep->bmAttributes,
				      (unsigned)USBToHostWord(ep->wMaxPacketSize),
			...
			// 4266888 - check to make sure that the ep number is non-zero so we don't screw up the control ep
				...
				if (_bInterfaceClass == kUSBHubClass)
				{
					IOLog("PANTHERA:IOUSBInterface CreatePipes made pipe addr=%u idx=%u pipe=%p\n",
					      _device ? (unsigned)_device->GetAddress() : 0,
					      i,
					      _pipeList[i]);
				}
   ...
   // Relax the checkin on whether the # of pipes created was the same as the bNumEndpoints
    ...
    if (_bInterfaceClass == kUSBHubClass)
    {
        IOLog("PANTHERA:IOUSBInterface CreatePipes end addr=%u created=%u declared=%u failed=%u firstPipe=%p\n",
              _device ? (unsigned)_device->GetAddress() : 0,
              i,
              (unsigned)_bNumEndpoints,
              makePipeFailed ? 1U : 0U,
              _pipeList[0]);
    }
							 ...
							 bool withRetain)
{
	...
	for ( ; (i < numEndpoints) && (i < kUSBMaxPipes) ; i++) 
	{
		...
		if (_bInterfaceClass == kUSBHubClass)
		{
			IOLog("PANTHERA:IOUSBInterface FindNextPipe addr=%u idx=%d pipe=%p reqType=%u reqDir=%u\n",
			      _device ? (unsigned)_device->GetAddress() : 0,
			      i,
			      pipe,
			      request ? (unsigned)request->type : 0,
			      request ? (unsigned)request->direction : 0);
		}
		if (!pipe)
			continue;
    ...
    while (true) 
    {
        ...
        if ( (next->bDescriptorType == kUSBInterfaceDesc) && ( ( (IOUSBInterfaceDescriptor *)next)->bInterfaceNumber != _bInterfaceNumber) )
        {
	        ...
	        if (next->bDescriptorType == type || type == kUSBAnyDesc)
	        {
	            if (_bInterfaceClass == kUSBHubClass)
	            {
	                IOLog("PANTHERA:IOUSBInterface assocDesc addr=%u type=0x%02x want=0x%02x len=%u\n",
	                      _device ? (unsigned)_device->GetAddress() : 0,
	                      (unsigned)next->bDescriptorType,
	                      (unsigned)type,
	                      (unsigned)next->bLength);
	            }
	            break;
	        }
	    }
...
IOUSBInterface::GetInterfaceStatus(USBStatus *status)
{
    ...
    *status = 0;
    ...
    return _device->DeviceRequest(&request);
}
...
IOUSBInterface::SetFunctionSuspendFeature(UInt8 options)
{
    ...
    request.wValue = kUSBFeatureFunctionSuspend;
    request.wIndex = _bInterfaceNumber | (static_cast<UInt16>(options) << 8);
    request.wLength = 0;
    request.pData = NULL;
...
IOUSBInterface::EnableRemoteWake(bool enable)
{
    ...
    if (enable) {
        return SetFunctionSuspendFeature(static_cast<UInt8>(1U << kUSBFunctionRemoteWakeEnableBit));
    }
