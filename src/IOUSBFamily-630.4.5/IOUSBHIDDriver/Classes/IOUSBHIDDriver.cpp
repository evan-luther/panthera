...
IOUSBHIDDriver::handleStart(IOService * provider)
{
    ...
    UInt32          interfaceSubClass = 0;
    UInt32          interfaceProtocol = 0;
    OSObject *      obj = NULL;
    ...
    obj = _interface->copyProperty("bInterfaceSubClass");
    if (OSNumber *number = OSDynamicCast(OSNumber, obj)) {
        interfaceSubClass = number->unsigned32BitValue();
    }
    OSSafeReleaseNULL(obj);

    obj = _interface->copyProperty("bInterfaceProtocol");
    if (OSNumber *number = OSDynamicCast(OSNumber, obj)) {
        interfaceProtocol = number->unsigned32BitValue();
    }
    OSSafeReleaseNULL(obj);

    if (pantheraUSBHIDLogsEnabled()) {
        IOLog("PANTHERA:IOUSBHIDDriver handleStart addr=%u subclass=%u protocol=%u usagePage=0x%x usage=0x%x inReport=%u featReport=%u outReport=%u descLen=%u\n",
              _device ? _device->GetAddress() : 0,
              (unsigned)interfaceSubClass,
              (unsigned)interfaceProtocol,
              (unsigned)_deviceUsagePage,
              (unsigned)_deviceUsage,
              (unsigned)myHIDCaps.inputReportByteLength,
              (unsigned)myHIDCaps.featureReportByteLength,
              (unsigned)myHIDCaps.outputReportByteLength,
              (unsigned)hidDescSize);

        if (myHIDDesc && hidDescSize) {
            UInt32 dump = hidDescSize > 64 ? 64 : hidDescSize;
            UInt32 index;
            IOLog("PANTHERA:IOUSBHIDDriver reportDesc");
            for (index = 0; index < dump; index++) {
                IOLog("%s%02x", index == 0 ? " bytes=" : " ", myHIDDesc[index]);
            }
            IOLog("%s\n", hidDescSize > dump ? " ..." : "");
        }
    }
...
IOUSBHIDDriver::GetIndexedString(UInt8 index, UInt8 *vOutBuf, UInt32 *vOutSize, UInt16 lang) const
{
    ...
    strlcpy((char *)vOutBuf, strBuf, outSize);
    *vOutSize = strLen;
    return kIOReturnSuccess;
}
...
IOUSBHIDDriver::start(IOService *provider)
{
    ...
    if (pantheraUSBHIDLogsEnabled()) {
        IOLog("PANTHERA:IOUSBHIDDriver start provider=%s\n", provider ? provider->getName() : "<null>");
    }
...
{
    IOUSBHIDDriver *me = OSDynamicCast(IOUSBHIDDriver, target);
    SInt32	direction = (SInt32)(uintptr_t)param1;
