...
ExpressCardPort(IOPCIDevice *device)
{
    (void)device;
    return 0;
}
...
{
    if (_device->getProperty("built-in") && (_ERRATA64BITS & kUHCIErrataICH6PowerSequencing)) 
	{
                                   ...
                                   void *param3, void *param4)
{
	...
	if (!strncmp(functionName->getCStringNoCopy(), "SetDebugDriverPowerState", 24))
	{
		...
		else
		{
			...
			if ((_badExpressCardAttached) && (_ExpressCardPort > 0) && (_ERRATA64BITS & kUHCIErrataSupportsPortResumeEnable))
			{
...
AppleUSBUHCI::SuspendController(void)
{
	...
	for (i=0; i< 2; i++)
	{
		...
		if ((_ERRATA64BITS & kUHCIErrataUHCISupportsOvercurrent) && (value & kUHCI_PORTSC_OCI))  // Is the latched Overcurrent set?
		{
	...
	IOLog("PANTHERA:AppleUSBUHCI[%p] busState<=Suspended source=SuspendController power=%d changingTo=%d available=%d provider=%s\n",
	      this,
	      (int)_myPowerState,
...
AppleUSBUHCI::DozeController(void)
{
	...
	if (PantheraDeviceBootstrapActive())
	{
		IOLog("PANTHERA:AppleUSBUHCI skip DozeController during bootstrap holds active state=%d changingTo=%d available=%d busState=%u\n",
		      (int)_myPowerState,
		      (int)_powerStateChangingTo,
		      _controllerAvailable ? 1 : 0,
		      (unsigned)_myBusState);
		return kIOReturnSuccess;
	}
	...
	if (!portsBeingResumed)
	{
		...
		_myBusState = kUSBBusStateSuspended;
		IOLog("PANTHERA:AppleUSBUHCI[%p] busState<=Suspended source=DozeController power=%d changingTo=%d available=%d provider=%s\n",
		      this,
		      (int)_myPowerState,
		      (int)_powerStateChangingTo,
		      _controllerAvailable ? 1 : 0,
