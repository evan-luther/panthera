                                       ...
                                       UInt8				speed)
{
    ...
    if ((functionNumber == 0) && (endpointNumber == 0))
    {
        IOLog("PANTHERA:UIMCreateControlEndpoint fn=%u ep=%u mps=%u speed=%u rootFn=%u\n",
              (unsigned)functionNumber,
              (unsigned)endpointNumber,
              (unsigned)maxPacketSize,
              (unsigned)speed,
              (unsigned)_rootFunctionNumber);
    }

    if ((_rootFunctionNumber != 0) && (functionNumber == _rootFunctionNumber)) 
	{
        if ((functionNumber == 0) && (endpointNumber == 0))
        {
            IOLog("PANTHERA:UIMCreateControlEndpoint skipped as root hub fn=%u\n",
                  (unsigned)functionNumber);
        }
    ...
    if (pQH == NULL)
    {
        if ((functionNumber == 0) && (endpointNumber == 0))
        {
            IOLog("PANTHERA:UIMCreateControlEndpoint AllocateQH failed fn=%u ep=%u\n",
                  (unsigned)functionNumber,
                  (unsigned)endpointNumber);
        }
	...
	if (!pTD)
	{
        if ((functionNumber == 0) && (endpointNumber == 0))
        {
            IOLog("PANTHERA:UIMCreateControlEndpoint AllocateTD failed fn=%u ep=%u qh=%p\n",
                  (unsigned)functionNumber,
                  (unsigned)endpointNumber,
                  pQH);
        }
    ...
    if (endpointNumber == 0)
    {
        IOLog("PANTHERA:UIMCreateControlEndpoint done qh=%p fn=%u ep=%u dir=%u type=%u mps=%u\n",
              pQH,
              (unsigned)pQH->functionNumber,
              (unsigned)pQH->endpointNumber,
              (unsigned)pQH->direction,
              (unsigned)pQH->type,
              (unsigned)pQH->maxPacketSize);
    }
...
#pragma unused (bufferRounding)
    ...
    if (pQH->stalled) 
	{
		if (endpointNumber == 0)
		{
			USBError(1, "AppleUSBUHCI::UIMCreateControlTransfer - address %d endpoint 0 is incorrectly marked as stalled!", functionNumber);
		}
		else
		{
...
AppleUSBUHCI::DeleteIsochEP(IOUSBControllerIsochEndpoint* pEP)
{
    ...
    if (pEP->activeTDs)
    {
		...
		if (pEP->activeTDs)
		{
			USBError(1, "AppleUSBUHCI::DeleteIsochEP- after abort there are STILL %d active TDs", (uint32_t) pEP->activeTDs);
		}
    }
    prevEP = NULL;
    curEP = _isochEPList;
...
AppleUSBUHCI::CreateIsochTransfer(IOUSBControllerIsochEndpoint* pEP, IOUSBIsocCommand *command)
{
	...
	if (!pEP)
	{
		USBError(1, "AppleUSBUHCI::CreateIsochTransfer - no endpoint");
		return kIOReturnBadArgument;
	}
...
AppleUSBUHCI::UIMCheckForTimeouts(void)
{
    ...
    if (status & kUHCI_STS_HCH) 
	{
        ...
        USBError(1, "AppleUSBUHCI::UIMCheckForTimeouts - Host controller halted, resetting");
        Reset(true);
        Run(true);
    }
	...
	for (pQH = _lsControlQHStart; pQH && (loopCount++ < 100); pQH = OSDynamicCast(AppleUHCIQueueHead, pQH->_logicalNext))
	{
		...
		{
			USBError(1,"AppleUSBUHCI::UIMCheckForTimeouts  pQH (%p) linked to itself", pQH);
		}
		...
		if (completionTimeout)
		{
			...
			if (!firstActiveFrame)
			{
				if ((pQH->functionNumber == 0) && (pQH->endpointNumber == 0))
				{
					IOLog("PANTHERA:UHCI timeout arm completion addr0 frame=0x%x timeout=%u\n",
					      (unsigned)curFrame,
					      (unsigned)completionTimeout);
				}
			...
			if ((curFrame - firstActiveFrame) >= completionTimeout)
			{
				...
				USBError(1, "AppleUSBUHCI::Found a transaction past the completion deadline on bus 0x%x, timing out! (Addr: %d, EP: %d)", (uint32_t) _busNumber, pQH->functionNumber, pQH->endpointNumber );
				pQH->print(2);
...
AppleUSBUHCI::AllocTDChain(AppleUHCIQueueHead* pQH, IOUSBCommand *command, IOMemoryDescriptor* CBP, UInt32 bufferSize, UInt16 direction, Boolean controlTransaction)
{
	...
	{
		USBError(1, "AppleUSBUHCI::AllocTDChain - buffserSize %d with maxPacket %d", (int)bufferSize, (int)maxPacket);
		return kIOReturnNoResources;
	}
    ...
    if (pTD1 == NULL)
    {
		USBError(1, "AppleUSBUHCI::AllocTDChain - can't allocate 1st new TD");
		return kIOReturnNoMemory;
    }
	...
	if (CBP && bufferSize)
	{
		...
		if (!dmaCommand)
		{
			USBError(1, "AppleUSBUHCI::AllocTDChain - no dmaCommand");
			DeallocateTD(pTD1);
			return kIOReturnInternalError;
		}
		if (dmaCommand->getMemoryDescriptor() != CBP)
		{
			USBError(1, "AppleUSBUHCI::AllocTDChain - mismatched CBP (%p) and dmaCommand memory descriptor (%p)", CBP, dmaCommand->getMemoryDescriptor());
			DeallocateTD(pTD);
			return kIOReturnInternalError;
		}
	}
    ...
    {	    
        ...
        while (transferOffset < bufferSize)
        {
			...
			{
				USBError(1, "AppleUSBUHCI::AllocTDChain - generated segment not 32 bit -  offset (0x%qx) length (0x%qx) ", segments64.fIOVMAddr, segments64.fLength);
				return kIOReturnInternalError;
			}
			...
			if (totalPhysLength < bytesToSchedule)
			{
				...
				if (!bp)
				{
					USBError(1, "AppleUSBUHCI::AllocTDChain - could not get the alignment buffer I needed");
					return kIOReturnNoResources;
				}
			...
			else
            {
				...
				if (pTDnew == NULL)
				{
					...
					USBError(1, "AppleUSBUHCI::AllocTDChain can't allocate new TD");
				}
				else
				{
...
AppleUSBUHCI::AddIsochFramesToSchedule(IOUSBControllerIsochEndpoint* pEP)
{
	...
	if (!IOSimpleLockTryLock(_isochScheduleLock))
	{
		...
		USBError(1, "AppleUSBUHCI::AddIsochFramesToSchedule - could not obtain scheduling lock");
		return;
	}
    ...
    {		
		...
		//		else
		//		{
		//			USBError(1, "AppleUSBUHCI::AddIsochFramesToSchedule - expected frame (%qd) and see frame (%qd) - should do something here!!", currFrame, pTD->_frameNumber);
		//		}
	...
	if ((finFrame - startFrame) > 1)
		USBError(1, "AppleUSBUHCI::AddIsochFramesToSchedule - end -  startFrame(0x%qd) finFrame(0x%qd)", startFrame, finFrame);
