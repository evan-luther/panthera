...
{		
	if (pantheraUSBHIDLogsEnabled()) {
		...
		IOLog("PANTHERA:AppleUSBUHCI start provider=%s pin=%u line=%u\n",
		      provider ? provider->getName() : "<null>",
		      pci ? pci->configRead8(kIOPCIConfigInterruptPin) : 0,
		      pci ? pci->configRead8(kIOPCIConfigInterruptLine) : 0);
	}
...
AppleUSBUHCI::UIMInitialize(IOService * provider)
{
    ...
    if (!_uimInitialized) 
	{
		...
		{
			if (pantheraUSBHIDLogsEnabled()) {
				IOLog("PANTHERA:AppleUSBUHCI UIMInitialize no IOPCIDevice provider\n");
			}
		...
		if (_v3ExpansionData->_onThunderbolt || (_ERRATA64BITS & kUHCIErrataDontUseCompanionController))
		{
			USBLog(3, "AppleUSBUHCI[%p]::UIMInitialize - Thunderbolt and companion controllers disallowed. Not initializing", this);
			if (pantheraUSBHIDLogsEnabled()) {
				IOLog("PANTHERA:AppleUSBUHCI UIMInitialize blocked on thunderbolt/companion errata=0x%llx onThunderbolt=%d\n",
				      _ERRATA64BITS, _v3ExpansionData->_onThunderbolt ? 1 : 0);
			}
        ...
        } else 
		{
			...
			if (pantheraUSBHIDLogsEnabled()) {
				IOLog("PANTHERA:AppleUSBUHCI UIMInitialize mapDeviceMemoryWithIndex(0) failed\n");
			}
            return kIOReturnNoMemory;
        }
		...
		{
			if (pantheraUSBHIDLogsEnabled()) {
				IOLog("PANTHERA:AppleUSBUHCI UIMInitialize IOLockAlloc failed\n");
			}
            return kIOReturnNoMemory;
        }

        _isocBandwidth = kUSBMaxFSIsocEndpointReqCount;
		_isochMaxBusStall = kUHCIIsochMaxBusStall;						// we need a requireMaxBusStall of 10 microseconds for UHCI
        ...
        if (pantheraUSBHIDLogsEnabled()) {
            OSArray *controllers = OSDynamicCast(OSArray, _device->getProperty(gIOInterruptControllersKey));
            OSArray *specifiers = OSDynamicCast(OSArray, _device->getProperty(gIOInterruptSpecifiersKey));
            int interruptType = 0;
            IOReturn interruptTypeStatus = _device->getInterruptType(0, &interruptType);
            IOLog("PANTHERA:AppleUSBUHCI provider interrupts controllers=%u specifiers=%u getInterruptType=0x%x type=0x%x\n",
                  controllers ? (unsigned)controllers->getCount() : 0,
                  specifiers ? (unsigned)specifiers->getCount() : 0,
                  interruptTypeStatus,
                  interruptType);
        }
        ...
        IOReturn addSourceStatus = _interruptSource ? _workLoop->addEventSource(_interruptSource) : kIOReturnNoResources;

        if (!_interruptSource || (addSourceStatus != kIOReturnSuccess)) 
		{
			if (pantheraUSBHIDLogsEnabled()) {
				IOLog("PANTHERA:AppleUSBUHCI UIMInitialize interrupt source setup failed source=%p workLoop=%p status=0x%x\n",
				      _interruptSource, _workLoop, addSourceStatus);
			}
            return kIOReturnBadArgument;
        }
		...
		_myBusState = kUSBBusStateReset;
		IOLog("PANTHERA:AppleUSBUHCI[%p] busState<=Reset source=UIMInitialize power=%d available=%d provider=%s\n",
		      this,
		      (int)_myPowerState,
		      _controllerAvailable ? 1 : 0,
		      getProvider() ? getProvider()->getName() : "<none>");
    }
...
AppleUSBUHCI::SetVendorInfo(void)
{
    ...
    if (_vendorID == 0x1106) 
	{
        ...
        _ERRATA64BITS |= kUHCIErratakUHCIResetAfterBabble;
    }
}
...
AppleUSBUHCI::scavengeAnIsochTD(AppleUHCIIsochTransferDescriptor *pTD)
{
    ...
    if (pEP == NULL)
    {
		USBError(1, "AppleUSBUHCI::scavengeAnIsochTD - could not find endpoint associated with iTD (%p)", pTD->_pEndpoint);
    }
    else
    {	
		if (!pTD->_lowLatency)
			...
			{	
				...
				if (qTD->alignBuffer)
				{
					...
					else
					{
						...
						if (!command)
						{
							USBError(1, "AppleUSBUHCI::scavengeQueueHeads - ERROR - missing usbcommand!!");
						}
						else
						{
							...
							{
								USBError(1, "AppleUSBUHCI::scavengeQueueHeads - ERROR - TD (%p) missing or empty dmaCommand (%p) or (%p)", qTD, dmaCommand, command->GetDMACommand());
							}
						}
					}
				}
...
AppleUSBUHCI::UHCIUIMDoDoneQueueProcessing(AppleUHCITransferDescriptor *pHCDoneTD, OSStatus forceErr, AppleUHCITransferDescriptor *stopAt)
{
    ...
    while (pHCDoneTD != NULL)
    {
		...
		if (pHCDoneTD->callbackOnTD)
		{
			...
			{
				USBError (1, "AppleUSBUHCI::UHCIUIMDoDoneQueueProcessing pHCDoneTD->command is NULL (%p)", pHCDoneTD);
			}
			else
			{
				...
				if (completion.action)
				{
					...
					if ((pHCDoneTD->pQH->type == kUSBControl) || (pHCDoneTD->pQH->type == kUSBBulk))
					{
						...
						{
							USBError(1, "AppleUSBUHCI::UHCIUIMDoDoneQueueProcessing - _controlBulkTransactionsOut underrun!");
						}
						else
						{
...
AppleUSBUHCI::AllocateTD(AppleUHCIQueueHead *pQH)
{
    ...
    if (freeTD == NULL)
    {
		...
		if (!memBlock)
		{
			USBError(1, "AppleUSBUHCI::AllocateTD - unable to allocate a new memory block!");
			return NULL;
		}
		...
		for (i=1; i < numTDs; i++)
		{
			...
			if (!freeTD)
			{
				USBError(1, "AppleUSBUHCI::AllocateTD - hmm. ran out of TDs in a memory block");
				freeTD = _pFreeTD;
				break;
			}
...
AppleUSBUHCI::AllocateITD(void)
{
    ...
    if (freeITD == NULL)
    {
		...
		if (!memBlock)
		{
			USBError(1, "AppleUSBUHCI::AllocateITD - unable to allocate a new memory block!");
			return NULL;
		}
		// link it in to my list of ED memory blocks
		memBlock->SetNextBlock(_tdMBHead);
		...
		for (i=1; i < numTDs; i++)
		{
			...
			if (!freeITD)
			{
				USBError(1, "AppleUSBUHCI::AllocateITD - hmm. ran out of TDs in a memory block");
				freeITD = _pFreeITD;
				break;
			}
...
AppleUSBUHCI::DeallocateITD(AppleUHCIIsochTransferDescriptor *pITD)
{
	...
	if (pITD->alignBuffer)
	{
		USBError(1, "AppleUSBUHCI::DeallocateITD - UNEXPECTED alignment buffer %p", pITD->alignBuffer);
...
AppleUSBUHCI::InitializeBufferMemory()
{
	...
	if (!dmaCommand)
	{
		USBError(1, "AppleUSBUHCI::InitializeBufferMemory - could not create IODMACommand");
	...
	do
	{
		...
		{
			USBError(1, "AppleUSBUHCI::InitializeBufferMemory - could not get frame list buffer");
		...
		if (!_cbiAlignBuffer)
		{
			USBError(1, "AppleUSBUHCI::InitializeBufferMemory - could not get alignment buffer");
			status = kIOReturnNoMemory;
			break;
		}
		...
		for (i=0; i < (PAGE_SIZE/kUHCI_BUFFER_CBI_ALIGN_SIZE); i++)
		{
			...
			if (!alignBuf)
			{
				USBError(1, "AppleUSBUHCI::InitializeBufferMemory - unable to allocate expected UHCIAlignmentBuffer");
				break;
			}
		...
		{
			USBError(1, "AppleUSBUHCI::InitializeBufferMemory - could not get isoch alignment buffer");
			status = kIOReturnNoMemory;
			break;
		}
	...
	if (dmaCommand)
	{
		...
		{
			USBError(1, "AppleUSBUHCI::InitializeBufferMemory - dmaCommand still has memory descriptor (%p)", dmaCommand->getMemoryDescriptor());
			dmaCommand->clearMemoryDescriptor();
		}
		dmaCommand->release();
	}
...
AppleUSBUHCI::GetCBIAlignmentBuffer()
{
	...
	{
		USBError(1, "AppleUSBUHCI::GetCBIAlignmentBuffer - ran out of alignment buffers");
		return NULL;
	}
	queue_remove_first(&_cbiAlignmentBuffers, ap, UHCIAlignmentBuffer *, chain);
	ap->userBuffer = NULL;
...
AppleUSBUHCI::GetIsochAlignmentBuffer()
{
	...
	{
		USBError(1, "AppleUSBUHCI::GetIsochAlignmentBuffer - ran out of alignment buffers");
		return NULL;
	}
	queue_remove_first(&_isochAlignmentBuffers, ap, UHCIAlignmentBuffer *, chain);
	ap->userBuffer = NULL;
