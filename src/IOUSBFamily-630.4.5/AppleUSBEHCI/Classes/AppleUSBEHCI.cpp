...
AppleUSBEHCI::UIMInitialize(IOService * provider)
{
    ...
    do {
        ...
        if (!_v3ExpansionData->_onThunderbolt)
            _isochMaxBusStall = 25000;						// we need a requireMaxBusStall of 25 microseconds for EHCI (but not on Tbolt)
...
#pragma unused(refCon)
    ...
    _workLoop->wake(&_xhciController);
...
{
    IOACPIPlatformDevice * acpiDevice = NULL;
    OSString *             acpiPath;

    if (device)
    {
        acpiPath = (OSString *) device->copyProperty("acpi-path");
...
AppleUSBEHCI::CheckForSharedXHCIController(void)
{
	...
	if ( !isInactive() )
	{
        ...
        do
        {
            ...
            {
                sleepResult = _workLoop->sleep(&_xhciController);
