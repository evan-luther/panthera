	...
	vm_size_t             new_data_size)
{
	...
	if (__improbable(va == 0)) {
		PANTHERA_TRACE(printf("PANTHERA:ROCOPY zid=%u off=%p size=%p src=%p ra0=%p\n",
		    zid, (void *)offset, (void *)new_data_size, (void *)new_data,
...
pmap_init(void)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT enter\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT kmem_alloc pmap_struct_size=0x%lx\n",
	    (unsigned long)pmap_struct_size));
	kmem_alloc(pmap_struct_map, &addr, pmap_struct_size,
	    KMA_NOFAIL | KMA_ZERO | KMA_KOBJECT | KMA_PERMANENT,
	    VM_KERN_MEMORY_PMAP);
	PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT kmem_alloc result addr_is_zero=%d size=0x%lx\n",
	    (addr == 0), (unsigned long)pmap_struct_size));
	/* Panthera: test if allocated memory is accessible */
	PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT testing base addr read...\n"));
	volatile char test = *(volatile char *)addr;
	(void)test;
	PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT base addr read OK\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT last_pn=%u regions=%u\n", last_pn, pmap_memory_region_count));
	unsigned int i;
	pmap_memory_region_t *pmptr = pmap_memory_regions;
	for (i = 0; i < pmap_memory_region_count; i++, pmptr++) {
		PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT region[%u] type=%u base=0x%x end=0x%x\n",
		    i, pmptr->type, pmptr->base, pmptr->end));
		if (pmptr->type != kEfiConventionalMemory) {
			continue;
		}
		PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT phys_attr npages=%ld last_pn=%u base=%u end=%u\n",
		    npages, last_pn, pmptr->base, pmptr->end));
		/* Panthera: verify pmap_phys_attributes is accessible via pmap_find_phys */
		{
			ppnum_t ppn_test = pmap_find_phys(kernel_pmap, (vm_map_offset_t)(uintptr_t)pmap_phys_attributes);
			PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT phys_attr ppn=0x%x (0=unmapped)\n", ppn_test));
		}
		ppnum_t pn;
	...
	PANTHERA_TRACE(printf("PANTHERA:PMAP_INIT phys_attrib loop done\n"));
	while (vsize) {
