    ...
    const void *new_data, vm_size_t new_data_size)
{
	...
	if (__improbable(elem == NULL)) {
		PANTHERA_TRACE(printf("PANTHERA:ZRO_MUT_NULL zid=%u off=%p size=%p src=%p ra0=%p\n",
		    zid, (void *)offset, (void *)new_data_size, new_data,
...
zone_id_require_ro_panic(zone_id_t zid, void *addr)
{
	...
	if (!from_ro_map(addr, 1)) {
		panic("zone_require_ro failed: address not in a ro zone (zid: %u %s%s addr: %p ra0: %p)",
		    zid, zone_heap_name(zone), zone->z_name, addr, ra0);
	}

	if (!Z_FAST_ALIGNED(PAGE_SIZE - (elem & PAGE_MASK), p.z_align_magic)) {
		panic("zone_require_ro failed: element improperly aligned (zid: %u %s%s addr: %p ra0: %p)",
		    zid, zone_heap_name(zone), zone->z_name, addr, ra0);
	}
	...
	if (zindex >= os_atomic_load(&num_zones, relaxed) || !other->z_self) {
		panic("zone_require_ro failed: invalid zone index %d "
		    "(addr: %p, expected: %s%s, ra0: %p)", zindex,
		    addr, zone_heap_name(zone), zone->z_name, ra0);
	} else {
		panic("zone_require_ro failed: address in unexpected zone id %d (%s%s) "
		    "(addr: %p, expected: %s%s, ra0: %p)",
		    zindex, zone_heap_name(other), other->z_name,
		    addr, zone_heap_name(zone), zone->z_name, ra0);
	}
}
...
panthera_zone_create_debug_prefix(const char *name)
{
	if (strcmp(name, "Bank ledger") == 0) {
		return "PANTHERA:ZC_BANK_";
	}
	if (strcmp(name, "Per-task ledger") == 0) {
		return "PANTHERA:ZC_TASK_";
	}
	if (strcmp(name, "Per-coalition ledgers") == 0) {
		return "PANTHERA:ZC_COAL_";
	}
	if (strcmp(name, "Coalition task ledgers") == 0) {
		return "PANTHERA:ZC_CTASK_";
	}
	return NULL;
}

static inline void
panthera_zone_create_debug_mark(const char *prefix, const char *phase)
{
	if (prefix != NULL) {
		extern void panthera_debugcon_puts(const char *);
		panthera_debugcon_puts(prefix);
		panthera_debugcon_puts(phase);
	...
	void                  (^extra_setup)(zone_t))
{
	...
	const char *panthera_prefix;
	uint16_t redzone;
	zone_t z;

	panthera_prefix = panthera_zone_create_debug_prefix(name);
	panthera_zone_create_debug_mark(panthera_prefix, "IN");
	...
	panthera_zone_create_debug_mark(panthera_prefix, "FIND");
	if (__improbable(z->z_self)) {
		/* We found a zone to reuse */
		panthera_zone_create_debug_mark(panthera_prefix, "REUSE");
		return z;
	}
	...
	panthera_zone_create_debug_mark(panthera_prefix, "STATS");
	...
	panthera_zone_create_debug_mark(panthera_prefix, "SETUP");

	/*
	 * Configure debugging features
	 */
	/*
	 * Panthera: skip per-zone debug feature setup during early bring-up.
	 * These paths walk boot-args after zone_init() and currently wedge
	 * dynamic zone creation under QEMU. They are not boot-critical.
	 */
	panthera_zone_create_debug_mark(panthera_prefix, "DEBUG");
	...
	panthera_zone_create_debug_mark(panthera_prefix, "CACHE");

	zone_lock(z);
	z->z_self = z;
	zone_unlock(z);
	panthera_zone_create_debug_mark(panthera_prefix, "OUT");
