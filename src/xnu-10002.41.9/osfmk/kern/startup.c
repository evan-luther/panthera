...
extern void panthera_debugcon_puthex64_public(uint64_t value);
#define PANTHERA_STARTUP_DBG(x) panthera_debugcon_write_public(x)
#define PANTHERA_STARTUP_HEX(x) panthera_debugcon_puthex64_public((uint64_t)(uintptr_t)(x))
#else
#define PANTHERA_STARTUP_DBG(x) do { } while (0)
#define PANTHERA_STARTUP_HEX(x) do { } while (0)
#endif
...
kernel_startup_initialize_upto(startup_subsystem_id_t upto)
{
	...
	while (cur < startup_entries_end && cur->subsystem <= upto) {
		if (startup_phase >= STARTUP_SUB_KPRINTF) {
			PANTHERA_TRACE(printf("PANTHERA:STARTUP[%d, rank %d, func %p, arg %p]\n",
			    cur->subsystem, cur->rank, cur->func, cur->arg));
		}
		PANTHERA_STARTUP_DBG("PANTHERA:STARTUP_RAW func=");
		PANTHERA_STARTUP_HEX(cur->func);
		PANTHERA_STARTUP_DBG(" arg=");
		PANTHERA_STARTUP_HEX(cur->arg);
		PANTHERA_STARTUP_DBG("\n");
		startup_phase = cur->subsystem - 1;
		kernel_startup_log(cur->subsystem);
#if defined(__x86_64__)
		if (cur->subsystem == STARTUP_SUB_SYSCTL &&
		    ((cur->rank == STARTUP_RANK_MIDDLE && cur->arg == NULL) ||
		    cur->rank == STARTUP_RANK_LAST)) {
			PANTHERA_TRACE(printf("PANTHERA:STARTUP_SKIP[%d, rank %d]\n",
			    cur->subsystem, cur->rank));
			if (startup_phase >= STARTUP_SUB_KPRINTF) {
				PANTHERA_TRACE(printf("PANTHERA:STARTUP_DONE[%d, rank %d, func %p, arg %p]\n",
				    cur->subsystem, cur->rank, cur->func, cur->arg));
			}
...
#endif
		if (cur->func == NULL) {
			PANTHERA_STARTUP_DBG("PANTHERA:STARTUP_NULL skip\n");
			if (startup_phase >= STARTUP_SUB_KPRINTF) {
				PANTHERA_TRACE(printf("PANTHERA:STARTUP_DONE[%d, rank %d, func %p, arg %p]\n",
				    cur->subsystem, cur->rank, cur->func, cur->arg));
			}
	...
	if (startup_phase >= STARTUP_SUB_KPRINTF) {
		PANTHERA_TRACE(printf("PANTHERA:STARTUP_UPTO_DONE[%d]\n", upto));
	}
...
kernel_bootstrap(void)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:KB thread_machine_init_template\n"));
	kernel_bootstrap_log("thread_machine_init_template");
	thread_machine_init_template();

	PANTHERA_TRACE(printf("PANTHERA:KB clock_init\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:KB thread_group_init\n"));
	kernel_bootstrap_log("thread_group_init");
	thread_group_init();
#endif

#if CONFIG_COALITIONS
	PANTHERA_TRACE(printf("PANTHERA:KB coalitions_init\n"));
	coalitions_init();
#endif

	PANTHERA_TRACE(printf("PANTHERA:KB code_signing_init\n"));
	kernel_bootstrap_log("code_signing_init");
	code_signing_init();
	code_signing_configuration(NULL, NULL);

	PANTHERA_TRACE(printf("PANTHERA:KB task_init\n"));
	kernel_bootstrap_log("task_init");
	task_init();

	PANTHERA_TRACE(printf("PANTHERA:KB thread_init\n"));
	kernel_bootstrap_log("thread_init");
	thread_init();

	PANTHERA_TRACE(printf("PANTHERA:KB restartable_init\n"));
	kernel_bootstrap_log("restartable_init");
	restartable_init();

	PANTHERA_TRACE(printf("PANTHERA:KB workq_init\n"));
	kernel_bootstrap_log("workq_init");
	workq_init();

	PANTHERA_TRACE(printf("PANTHERA:KB turnstiles_init\n"));
	kernel_bootstrap_log("turnstiles_init");
	turnstiles_init();

	PANTHERA_TRACE(printf("PANTHERA:KB mach_init_activity_id\n"));
	kernel_bootstrap_log("mach_init_activity_id");
	mach_init_activity_id();

	PANTHERA_TRACE(printf("PANTHERA:KB host_statistics_init\n"));
	/* initialize host_statistics */
	host_statistics_init();

	PANTHERA_TRACE(printf("PANTHERA:KB exception_init\n"));
	/* initialize exceptions */
	kernel_bootstrap_log("exception_init");
	exception_init();

#if CONFIG_SCHED_SFI
	PANTHERA_TRACE(printf("PANTHERA:KB sfi_init\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:KB kernel_thread_create\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:KB thread created ok, state=%d\n", thread->state));
	...
	PANTHERA_TRACE(printf("PANTHERA:KB load_context\n"));
...
kernel_bootstrap_thread(void)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:KBST idle_thread_create\n"));
	kernel_bootstrap_thread_log("idle_thread_create");
	/*
	 * Create the idle processor thread.
	 */
	idle_thread_create(processor);
	PANTHERA_TRACE(printf("PANTHERA:KBST idle_thread_create done\n"));
	...
	sched_startup();
	PANTHERA_TRACE(printf("PANTHERA:KBST sched_startup done\n"));
	...
	thread_daemon_init();
	PANTHERA_TRACE(printf("PANTHERA:KBST thread_daemon_init done\n"));

	/*
	 * Thread callout service.
	 */
	PANTHERA_TRACE(printf("PANTHERA:KBST thread_call_startup\n"));
	kernel_startup_initialize_upto(STARTUP_SUB_THREAD_CALL);
	PANTHERA_TRACE(printf("PANTHERA:KBST thread_call_startup done\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:KBST thread_bind done\n"));
	...
	mapping_adjust();
	PANTHERA_TRACE(printf("PANTHERA:KBST mapping_adjust done\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:KBST clock_service_create done\n"));

	/*
	 *	Create the device service.
	 */
	device_service_create();
	PANTHERA_TRACE(printf("PANTHERA:KBST device_service_create done\n"));

	phys_carveout_init();
	PANTHERA_TRACE(printf("PANTHERA:KBST phys_carveout_init done\n"));

#if MACH_KDP
	PANTHERA_TRACE(printf("PANTHERA:KBST kdp_init\n"));
	kernel_bootstrap_log("kdp_init");
	kdp_init();
	PANTHERA_TRACE(printf("PANTHERA:KBST kdp_returned\n"));
	PANTHERA_TRACE(printf("PANTHERA:KBST kdp_init done\n"));
#endif

#if ALTERNATE_DEBUGGER
#if defined(__x86_64__)
	PANTHERA_TRACE(printf("PANTHERA:KBST alternate_debugger_skip\n"));
#else
...
#if HYPERVISOR
#if defined(__x86_64__)
	PANTHERA_TRACE(printf("PANTHERA:KBST hv_support_skip\n"));
#else
...
#if CONFIG_TELEMETRY
#if defined(__x86_64__)
	PANTHERA_TRACE(printf("PANTHERA:KBST bootprofile_skip\n"));
#else
	...
	PANTHERA_TRACE(printf("PANTHERA:KBST sysctl_startup\n"));
	kernel_startup_initialize_upto(STARTUP_SUB_SYSCTL);
	PANTHERA_TRACE(printf("PANTHERA:KBST sysctl_startup done\n"));
...
#ifdef  IOKIT
	PANTHERA_TRACE(printf("PANTHERA:KBST PE_init_iokit\n"));
	kernel_bootstrap_log("PE_init_iokit");
	PE_init_iokit();
	PANTHERA_TRACE(printf("PANTHERA:KBST PE_init_iokit done\n"));
#endif
	 ...
	 */
	PANTHERA_TRACE(printf("PANTHERA:KBST early_boot_startup\n"));
	kernel_startup_initialize_upto(STARTUP_SUB_EARLY_BOOT);
	PANTHERA_TRACE(printf("PANTHERA:KBST early_boot_startup done\n"));

#if SCHED_HYGIENE_DEBUG
	// Reset interrupts masked timeout before we enable interrupts
	ml_spin_debug_clear_self();
#endif
	PANTHERA_TRACE(printf("PANTHERA:KBST spllo\n"));
	(void) spllo();         /* Allow interruptions */
	PANTHERA_TRACE(printf("PANTHERA:KBST spllo done\n"));

	/*
	 * This will start displaying progress to the user, start as early as possible
	 */
	initialize_screen(NULL, kPEAcquireScreen);
	PANTHERA_TRACE(printf("PANTHERA:KBST initialize_screen done\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:KBST vm_commpage_init done\n"));
...
#ifndef BCM2837
	PANTHERA_TRACE(printf("PANTHERA:KBST trust_cache_init\n"));
	 ...
	 */
#ifdef  MACH_BSD
	PANTHERA_TRACE(printf("PANTHERA:KBST bsd_init\n"));
	bsd_init();
#endif
