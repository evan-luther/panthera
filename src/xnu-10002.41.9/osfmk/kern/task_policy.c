...
proc_init_cpumon_params(void)
{
	...
	panthera_debugcon_puts("PANTHERA:CPUMON_ENTER\n");

	/*
	 * Panthera bring-up currently uses a synthetic EFI handoff and device tree.
	 * Seed the documented defaults directly until those PE lookups are stable.
	 */
	...
	proc_max_cpumon_interval = DEFAULT_CPUMON_INTERVAL * NSEC_PER_SEC;

	if (task_policy_suppression_flags & TASK_POLICY_SUPPRESSION_IOTIER2) {
		proc_suppressed_disk_tier = THROTTLE_LEVEL_TIER2;
	} else {
		proc_suppressed_disk_tier = THROTTLE_LEVEL_TIER1;
	}

	panthera_debugcon_puts("PANTHERA:CPUMON_DONE\n");
}
