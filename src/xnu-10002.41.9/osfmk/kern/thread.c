...
thread_terminate_self(void)
{
	...
	panthera_debugcon_puts("PANTHERA:TSELF enter\n");
	...
	panthera_debugcon_puts("PANTHERA:TSELF ipc disabled\n");
	...
	panthera_debugcon_puts("PANTHERA:TSELF uthread cleanup\n");
	...
	if (threadcnt == 0 && bsd_info != NULL) {
		...
		panthera_debugcon_puts("PANTHERA:TSELF before proc_exit\n");
		proc_exit(bsd_info);
		panthera_debugcon_puts("PANTHERA:TSELF after proc_exit\n");
		bsd_info = NULL;
		/*
	...
	thread_unlock(thread);
	/* splsched */
	panthera_debugcon_puts("PANTHERA:TSELF before block\n");
...
thread_daemon_init(void)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:THD stack daemon done\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:THD exception daemon done\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:THD backtrace daemon done\n"));
}
