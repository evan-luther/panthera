...
static inline void
panthera_sched_boot_probe(const char *message)
{
	extern void panthera_debugcon_puts(const char *);

	if (sched_maintenance_thread == THREAD_NULL) {
		panthera_debugcon_puts(message);
	}
}
...
thread_invoke(
	...
	ast_t                           reason)
{
	panthera_sched_boot_probe("PANTHERA:TINV enter\n");
...
need_stack:
	...
	panthera_sched_boot_probe("PANTHERA:TINV switch\n");
	thread = machine_switch_context(self, continuation, thread);
	panthera_sched_boot_probe("PANTHERA:TINV resumed\n");
	...
	ast_t                           reason)
{
	...
	do {
		panthera_sched_boot_probe("PANTHERA:TBR select\n");
		thread_lock(self);
		new_thread = thread_select(self, processor, &reason);
		thread_unlock(self);
		panthera_sched_boot_probe("PANTHERA:TBR invoke\n");
	...
	thread_t        thread)
{
	...
	if (sched_maintenance_thread == THREAD_NULL) {
		extern void panthera_debugcon_write_public(const char *);
		extern void panthera_debugcon_puthex64_public(uint64_t);
#if __x86_64__
		cpu_data_t *cpu_data = current_cpu_datap();
		uint64_t stack_pointer;

		__asm__ volatile ("movq %%rsp, %0" : "=r" (stack_pointer));
#endif

		panthera_debugcon_write_public("PANTHERA:TCONT cont=0x");
		panthera_debugcon_puthex64_public((uint64_t)(uintptr_t)continuation);
		panthera_debugcon_write_public(" param=0x");
		panthera_debugcon_puthex64_public((uint64_t)(uintptr_t)parameter);
#if __x86_64__
		panthera_debugcon_write_public(" self=0x");
		panthera_debugcon_puthex64_public((uint64_t)(uintptr_t)self);
		panthera_debugcon_write_public(" tstack=0x");
		panthera_debugcon_puthex64_public((uint64_t)self->kernel_stack);
		panthera_debugcon_write_public(" cstack=0x");
		panthera_debugcon_puthex64_public((uint64_t)cpu_data->cpu_active_stack);
		panthera_debugcon_write_public(" ctop=0x");
		panthera_debugcon_puthex64_public((uint64_t)cpu_data->cpu_kernel_stack);
		panthera_debugcon_write_public(" rsp=0x");
		panthera_debugcon_puthex64_public(stack_pointer);
#endif
	...
	/*
	 * Early scheduler bring-up still runs before the first safe
	 * interrupt-enabled continuation on Panthera's EFI handoff path.
	 */
	if (thread == THREAD_NULL || (self->state & TH_IDLE) ||
	    sched_maintenance_thread == THREAD_NULL) {
		enable_interrupts = FALSE;
	}
	...
	processor_t             processor)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:IDLE create enter cpu=%d\n", processor->cpu_id));
	result = kernel_thread_create(idle_thread, NULL, MAXPRI_KERNEL, &thread);
	if (result != KERN_SUCCESS) {
		PANTHERA_TRACE(printf("PANTHERA:IDLE create fail kr=%d\n", result));
		return result;
	}
	PANTHERA_TRACE(printf("PANTHERA:IDLE thread created\n"));
	...
	thread_unlock(thread);
	splx(s);
	PANTHERA_TRACE(printf("PANTHERA:IDLE state set\n"));

	thread_deallocate(thread);
	PANTHERA_TRACE(printf("PANTHERA:IDLE create done\n"));
...
sched_startup(void)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:SCHED enter\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:SCHED init_thread created\n"));
	 ...
	 */
	PANTHERA_TRACE(printf("PANTHERA:SCHED before block\n"));
	thread_block(THREAD_CONTINUE_NULL);
	PANTHERA_TRACE(printf("PANTHERA:SCHED after block\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:SCHED powered_cores thread created\n"));

	thread_deallocate(thread);

	assert_thread_magic(thread);
	PANTHERA_TRACE(printf("PANTHERA:SCHED done\n"));
}
...
{
	panthera_sched_boot_probe("PANTHERA:SCHED_INIT dbg enter\n");
	PANTHERA_TRACE(printf("PANTHERA:SCHED_INIT enter\n"));
	thread_block(THREAD_CONTINUE_NULL);
	PANTHERA_TRACE(printf("PANTHERA:SCHED_INIT after block\n"));

	thread_t thread = current_thread();

	thread_set_thread_name(thread, "sched_maintenance_thread");
	PANTHERA_TRACE(printf("PANTHERA:SCHED_INIT named\n"));

	sched_maintenance_thread = thread;
	PANTHERA_TRACE(printf("PANTHERA:SCHED_INIT set maint thread\n"));
