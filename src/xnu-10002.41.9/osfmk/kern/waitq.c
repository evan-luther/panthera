...
{
	if (!waitq_valid(waitq)) {
		/* Panthera: dump caller info before panic for debugging */
		PANTHERA_TRACE(kprintf("PANTHERA:WAITQ_INVALID wq=%p caller=%p lck_valid=%d\n",
		    waitq.wq_q,
		    __builtin_return_address(0),
		    waitq.wq_q ? waitq.wq_q->waitq_interlock.lck_valid : -1));
