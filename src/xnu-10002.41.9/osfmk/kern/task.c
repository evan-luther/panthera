	...
	mach_vm_size_t    size)
{
	...
	if ((task->t_flags & TF_DYLD_ALL_IMAGE_FINAL) == 0) {
		...
		if (inputNonZero && currentNonZero) {
...
task_init(void)
{
	...
	panthera_debugcon_puts("PANTHERA:TASK_ENTER\n");
...
#if DEVELOPMENT || DEBUG
	panthera_debugcon_puts("PANTHERA:TASK_GUARD\n");
...
#if CONFIG_COREDUMP
	panthera_debugcon_puts("PANTHERA:TASK_CORES\n");
	/*
	 * Panthera: boot-arg parsing hangs on this optional coredump knob
	 * during early x86 bring-up under QEMU. Defaulting to disabled is
	 * sufficient until the PE boot-arg path is understood.
	 */
	hwm_user_cores = 0;
	panthera_debugcon_puts("PANTHERA:TASK_CORES_DONE\n");
#endif

	panthera_debugcon_puts("PANTHERA:TASK_CPUMON\n");
	proc_init_cpumon_params();
	panthera_debugcon_puts("PANTHERA:TASK_CPUMON_DONE\n");

	panthera_debugcon_puts("PANTHERA:TASK_WAKEUPS_RATE\n");
	task_wakeups_monitor_rate = TASK_WAKEUPS_MONITOR_DEFAULT_LIMIT;

	panthera_debugcon_puts("PANTHERA:TASK_WAKEUPS_INT\n");
	task_wakeups_monitor_interval = TASK_WAKEUPS_MONITOR_DEFAULT_INTERVAL;

	panthera_debugcon_puts("PANTHERA:TASK_USTACK\n");
	task_wakeups_monitor_ustackshots_trigger_pct =
	    TASK_WAKEUPS_MONITOR_DEFAULT_USTACKSHOTS_TRIGGER;

	panthera_debugcon_puts("PANTHERA:TASK_IOMON_LIMIT\n");
	task_iomon_limit_mb = IOMON_DEFAULT_LIMIT;

	panthera_debugcon_puts("PANTHERA:TASK_IOMON_INT\n");
	task_iomon_interval_secs = IOMON_DEFAULT_INTERVAL;

	panthera_debugcon_puts("PANTHERA:TASK_IO_TELEM\n");
 ...
 */
	...
	if (task_ledger_template == NULL) {
		panthera_debugcon_puts("PANTHERA:TASK_LEDGER_FALLBACK\n");
		init_task_ledgers();
		panthera_debugcon_puts("PANTHERA:TASK_LEDGER_FALLBACK_DONE\n");
	}
	...
	/*
	 * The bootstrap thread starts life before thread_ro exists.
	 * Seed its direct task pointer so current_task() can still
	 * report kernel_task until load_context() switches to the
	 * first real zoned thread.
	 */
	current_thread()->t_task = kernel_task;
#endif
	...
	task_t                  task)
{
	...
	assert(task != kernel_task);
	panthera_debugcon_puts("PANTHERA:TTERM enter\n");
	...
	task->active = FALSE;
	ipc_task_disable(task);
	panthera_debugcon_puts("PANTHERA:TTERM inactive\n");
	 ...
	 */
	queue_iterate(&task->threads, thread, thread_t, task_threads) {
		panthera_debugcon_puts("PANTHERA:TTERM thread\n");
		thread_terminate_internal(thread);
	}
	panthera_debugcon_puts("PANTHERA:TTERM threads done\n");
	...
	task_unlock(task);
	panthera_debugcon_puts("PANTHERA:TTERM unlocked\n");
...
//        task_partial_reap(task, pid);
	...
	panthera_debugcon_puts("PANTHERA:TTERM sync done\n");
	 ...
	 */
	ipc_space_terminate(task->itk_space);
	panthera_debugcon_puts("PANTHERA:TTERM ipc done\n");
	...
	panthera_debugcon_puts("PANTHERA:TTERM vm done\n");
	...
	panthera_debugcon_puts("PANTHERA:TTERM done\n");
...
#define TASK_LEGACY_DYLD_INFO_COUNT \
	...
	case TASK_VM_INFO_PURGEABLE:
	{
		...
		if (original_task_info_count > TASK_VM_INFO_COUNT) {
			 ...
			 * logging about it...
			 */
			printf("%s:%d %d[%s] task_info(flavor=%d) possibly invalid "
			    "task_info_count=%d > TASK_VM_INFO_COUNT=%d platform %d sdk "
			    "%d.%d.%d - please use TASK_VM_INFO_COUNT.\n",
			    __FUNCTION__, __LINE__, proc_pid(p), proc_name_address(p),
			    flavor, original_task_info_count, TASK_VM_INFO_COUNT,
...
#if __x86_64__ || __arm__
	if (__improbable(thread->t_tro == NULL)) {
		if (thread->t_task != TASK_NULL) {
