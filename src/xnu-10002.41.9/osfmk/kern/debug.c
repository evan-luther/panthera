...
static void
panic_debugcon_putc(char c)
{
	outb(PANTHERA_DEBUGCON_PORT, (uint8_t)c);
}

static void
panic_debugcon_puthex64(uint64_t value)
{
	static const char hex[] = "0123456789abcdef";
	int shift;

	for (shift = 60; shift >= 0; shift -= 4) {
		panic_debugcon_putc(hex[(value >> shift) & 0xfU]);
	}
}

static void
panic_debugcon_write(const char *string)
{
	if (string == NULL) {
		return;
	}
	...
	while (*string != '\0') {
		panic_debugcon_putc(*string++);
	}
}

static void
panic_debugcon_log_panic(const char *panic_format_str, va_list *panic_args, unsigned int reason, unsigned long panic_caller)
{
	panic_debugcon_write("PANTHERA:PANIC caller=0x");
	panic_debugcon_puthex64(panic_caller);
	panic_debugcon_write(" reason=0x");
	panic_debugcon_puthex64(reason);

	if (panic_format_str != NULL && panic_args != NULL) {
		va_list copy;

		panic_debugcon_write(" msg=\"");
		va_copy(copy, *panic_args);
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wformat-nonliteral"
		_doprnt(panic_format_str, &copy, panic_debugcon_putc, 16);
#pragma clang diagnostic pop
		va_end(copy);
		panic_debugcon_putc('"');
	}

	panic_debugcon_write("\n");
}

static inline void
debug_seed_early_bootstrap_gsbase(void)
{
	uint64_t gsbase = rdmsr64(MSR_IA32_GS_BASE);

	if (gsbase == EARLY_GSBASE_MAGIC || gsbase == 0) {
		uintptr_t bootstrap_cpu_data = (uintptr_t)cpu_data_master;

		wrmsr64(MSR_IA32_GS_BASE, bootstrap_cpu_data);
		wrmsr64(MSR_IA32_KERNEL_GS_BASE, bootstrap_cpu_data);
	}
}
#endif
...
phys_carveout_init(void)
{
	...
	panthera_debugcon_puts("PANTHERA:PHYS_CARVEOUT enter\n");

	if (!PE_i_can_has_debugger(NULL)) {
		panthera_debugcon_puts("PANTHERA:PHYS_CARVEOUT no_debugger\n");
		return;
	}

	panthera_debugcon_puts("PANTHERA:PHYS_CARVEOUT debugger_ok\n");
	...
	for (int i = 0; i < (sizeof(carveouts) / sizeof(struct carveout)); i++) {
		if (carveouts[i].present) {
			...
			panthera_debugcon_puts("PANTHERA:PHYS_CARVEOUT alloc_prepare\n");
			...
			panthera_debugcon_puts("PANTHERA:PHYS_CARVEOUT alloc_enter\n");
			kmem_alloc_contig(kernel_map, carveouts[i].va,
			    ...
			    VM_KERN_MEMORY_DIAG);
			panthera_debugcon_puts("PANTHERA:PHYS_CARVEOUT alloc_done\n");
	...
	panthera_debugcon_puts("PANTHERA:PHYS_CARVEOUT done\n");
}
...
#if defined(__x86_64__)
	debug_seed_early_bootstrap_gsbase();
	panic_debugcon_log_panic(panic_format_str, panic_args, reason, panic_caller);
#endif
