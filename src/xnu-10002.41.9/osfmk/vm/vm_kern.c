...
{
	PANTHERA_TRACE(printf("PANTHERA:KMEM_RANGE sp=%p calc=%p sz=0x%lx\n",
	    sp, sp->kc_calculate_sz, (unsigned long)sp->kc_size));
	assert(kmem_claim_count < KMEM_MAX_CLAIMS - KMEM_RANGE_COUNT);
	if (sp->kc_calculate_sz) {
		PANTHERA_TRACE(printf("PANTHERA:KMEM_RANGE calling kc_calculate_sz\n"));
		sp->kc_size = (sp->kc_calculate_sz)();
		PANTHERA_TRACE(printf("PANTHERA:KMEM_RANGE kc_calculate_sz returned 0x%lx\n",
		    (unsigned long)sp->kc_size));
	}
	if (sp->kc_size) {
		kmem_claims[kmem_claim_count] = *sp;
		kmem_claim_count++;
	}
	PANTHERA_TRACE(printf("PANTHERA:KMEM_RANGE done\n"));
}
...
kmem_range_init(void)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:KRI scramble\n"));
	kmem_scramble_ranges();

	PANTHERA_TRACE(printf("PANTHERA:KRI ranges\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:KRI pmap_init\n"));
	pmap_init();
	PANTHERA_TRACE(printf("PANTHERA:KRI kmem_metadata_init\n"));
	kmem_metadata_init();
	PANTHERA_TRACE(printf("PANTHERA:KRI kmem_sizeclass_init\n"));
	kmem_sizeclass_init();
	PANTHERA_TRACE(printf("PANTHERA:KRI done\n"));
...
vm_kernel_addrhash_internal(vm_offset_t addr, uint64_t salt)
{
	...
	if (__improbable(g_crypto_funcs == NULL ||
	    g_crypto_funcs->ccsha256_di == NULL ||
	    g_crypto_funcs->ccdigest_init_fn == NULL ||
	    g_crypto_funcs->ccdigest_update_fn == NULL ||
	    g_crypto_funcs->ccdigest_final_fn == NULL)) {
		/*
		 * Panthera bring-up can reach VM_KERNEL_ADDRHASH() before the
		 * late corecrypto provider is registered. Keep early boot moving
		 * with a deterministic non-cryptographic mix instead of faulting.
		 */
		uint64_t hash = ((uint64_t)addr) ^ salt;
		hash ^= hash >> 33;
		hash *= 0xff51afd7ed558ccdULL;
		hash ^= hash >> 33;
		hash *= 0xc4ceb9fe1a85ec53ULL;
		hash ^= hash >> 33;
		return (vm_offset_t)hash;
	}
