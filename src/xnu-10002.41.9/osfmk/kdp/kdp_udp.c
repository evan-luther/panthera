...
kdp_init(void)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:KDP debug_log_init\n"));
	debug_log_init();
	PANTHERA_TRACE(printf("PANTHERA:KDP debug_log_init done\n"));
...
#if     defined(__x86_64__)
	PANTHERA_TRACE(printf("PANTHERA:KDP machine_init\n"));
	kdp_machine_init();
	PANTHERA_TRACE(printf("PANTHERA:KDP machine_init done\n"));
#endif

	PANTHERA_TRACE(printf("PANTHERA:KDP timer_callout_init\n"));
	kdp_timer_callout_init();
	PANTHERA_TRACE(printf("PANTHERA:KDP timer_callout_init done\n"));
	kdp_crashdump_feature_mask = htonl(kdp_crashdump_feature_mask);
	// Figure out the initial packet size
	PANTHERA_TRACE(printf("PANTHERA:KDP setup_packet_size\n"));
	kdp_setup_packet_size();
	PANTHERA_TRACE(printf("PANTHERA:KDP setup_packet_size done\n"));
	PANTHERA_TRACE(printf("PANTHERA:KDP core_init\n"));
	kdp_core_init();
	PANTHERA_TRACE(printf("PANTHERA:KDP core_init done\n"));

#if defined(__x86_64__)
	PANTHERA_TRACE(printf("PANTHERA:KDP serial_kdp_skip\n"));
	return;
#endif

#if CONFIG_SERIAL_KDP && !defined(__x86_64__)
	if (!PE_i_can_has_debugger(NULL) && debug_boot_arg == 0) {
		PANTHERA_TRACE(printf("PANTHERA:KDP serial_kdp_skip\n"));
		return;
	}
...
#endif /* CONFIG_SERIAL_KDP && !defined(__x86_64__) */
}
