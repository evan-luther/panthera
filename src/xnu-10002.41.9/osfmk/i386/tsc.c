...
tsc_init(void)
{
	...
	if (tscFreq == busFreq) {
		 ...
		 */
		if (N_by_2_bus_ratio) {
			tsc_divisor = 1 + 2 * tscGranularity;
			if (tsc_divisor == 0) {
				kprintf("tsc_init: invalid N/2 divisor, forcing ratio 1 under emulation\n");
				tscGranularity = 0;
				tsc_divisor = 1;
			}
			tscFCvtt2n = busFCvtt2n * 2 / tsc_divisor;
		} else {
			if (tscGranularity == 0) {
				kprintf("tsc_init: zero TSC granularity, forcing ratio 1 under emulation\n");
				tscGranularity = 1;
			}
