...
rtclock_init(void)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:RTCLOCK enter cpu=%d\n", cpu_number()));
	...
	if (cpu_number() == master_cpu) {
		...
		PANTHERA_TRACE(printf("PANTHERA:RTCLOCK tscFreq=%llu cycles=%llu\n", tscFreq, cycles));
		...
		PANTHERA_TRACE(printf("PANTHERA:RTCLOCK rtc_timer_init\n"));
		rtc_timer_init();
		PANTHERA_TRACE(printf("PANTHERA:RTCLOCK clock_timebase_init\n"));
		clock_timebase_init();
		PANTHERA_TRACE(printf("PANTHERA:RTCLOCK ml_init_delay_spin_threshold\n"));
		ml_init_delay_spin_threshold(10);
	}

	PANTHERA_TRACE(printf("PANTHERA:RTCLOCK rtc_config\n"));
	/* Set fixed configuration for lapic timers */
	rtc_timer->rtc_config();
	PANTHERA_TRACE(printf("PANTHERA:RTCLOCK rtc_timer_start\n"));
	rtc_timer_start();
