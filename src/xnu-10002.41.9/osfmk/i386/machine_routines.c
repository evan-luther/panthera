...
ml_wait_max_cpus(void)
{
	...
	while (max_cpus_initialized != MAX_CPUS_SET) {
		if (machine_info.max_cpus <= 0) {
			integer_t fallback_cpus = machine_info.logical_cpu_max;

			if (fallback_cpus <= 0) {
				fallback_cpus = machine_info.physical_cpu_max;
			}
			if (fallback_cpus <= 0) {
				fallback_cpus = 1;
			}

			machine_info.max_cpus = (integer_t)MIN((unsigned int)fallback_cpus, max_ncpus);
			if (machine_info.physical_cpu_max <= 0) {
				machine_info.physical_cpu_max = machine_info.max_cpus;
			}
			if (machine_info.logical_cpu_max <= 0) {
				machine_info.logical_cpu_max = machine_info.max_cpus;
			}
			max_cpus_initialized = MAX_CPUS_SET;
			break;
		}
