...
#define PANTHERA_DEBUGCON_PORT 0xe9U

void panthera_debugcon_write_public(const char *string);
void panthera_debugcon_puthex64_public(uint64_t value);

static void
panthera_debugcon_putc(char c)
{
	outb(PANTHERA_DEBUGCON_PORT, (uint8_t)c);
}

static void
panthera_debugcon_puthex64(uint64_t value)
{
	static const char hex[] = "0123456789abcdef";
	int shift;

	if (panthera_debug_verbose == 0) {
		return;
	}

	for (shift = 60; shift >= 0; shift -= 4) {
		panthera_debugcon_putc(hex[(value >> shift) & 0xfU]);
	}
}

static void
panthera_debugcon_write(const char *string)
{
	if (panthera_debug_verbose == 0) {
		return;
	}
	...
	while (*string != '\0') {
		panthera_debugcon_putc(*string++);
	}
}

void
panthera_debugcon_write_public(const char *string)
{
	if (panthera_debug_verbose != 0) {
		panthera_debugcon_write(string);
	}
}

void
panthera_debugcon_puthex64_public(uint64_t value)
{
	if (panthera_debug_verbose != 0) {
		panthera_debugcon_puthex64(value);
	}
}
...
/* Panthera: write to QEMU debugcon port 0xe9, bypassing all kernel I/O */
void panthera_debugcon_puts(const char *s);
void
panthera_debugcon_puts(const char *s)
{
	if (panthera_debug_verbose == 0) {
		return;
	}
	...
	while (*s) {
		__asm__ volatile("outb %0, $0xe9" :: "a"((unsigned char)*s));
		s++;
	}
}
...
Idle_PTs_init(void)
{
	...
	/*
	 * Panthera: convert kernelBootArgs from identity-mapped low address
	 * to physmap high address. The identity map (PML4[0]) will be removed
	 * later, making the low address invalid. Must do this AFTER set_cr3
	 * so the physmap is active.
	 */
	/*
	 * Panthera: kernelBootArgs was set via ml_static_ptovirt() which
	 * creates address VM_MIN_KERNEL_ADDRESS + physical. This mapping
	 * may not survive in IdlePML4 if boot_args is above physfree.
	 * Convert to a physmap address (physmap_base + physical) which
	 * covers all physical memory. Also update PE_state.bootArgs.
	 */
	{
		/* ml_static_ptovirt returns (VM_MIN_KERNEL_ADDRESS + phys).
		 * Extract physical by subtracting VM_MIN_KERNEL_ADDRESS. */
		uintptr_t ba_phys = (uintptr_t)kernelBootArgs - VM_MIN_KERNEL_ADDRESS;
		boot_args *ba_new = (boot_args *)PHYSMAP_PTOV(ba_phys);
		kernelBootArgs = ba_new;
		PE_state.bootArgs = ba_new;
	}
}
...
vstart(vm_offset_t boot_args_start)
{
	...
	if (is_boot_cpu) {
		...
		/* Panthera: verify physmap via debugcon (port 0xe9) */
		{
			extern uint64_t physmap_base, physmap_max;
			panthera_debugcon_puts("PANTHERA:PHYSMAP_CHECK\n");
			volatile uint64_t *test_ptr = (volatile uint64_t *)(physmap_base + 0x1000);
			volatile uint64_t test_val = *test_ptr;
			(void)test_val;
			panthera_debugcon_puts("PANTHERA:PHYSMAP_READ_OK\n");
		}
...
i386_init(void)
{
	...
	/* Panthera: fallback if ACPI enumeration returns 0 under QEMU */
...
doublemap_init(uint8_t randL3)
{
	...
	panthera_debugcon_write("PANTHERA:DBLMAP hdescb=0x");
	panthera_debugcon_puthex64(hdescb);
	panthera_debugcon_write(" base=0x");
	panthera_debugcon_puthex64(dblmap_base);
	panthera_debugcon_write(" dist=0x");
	panthera_debugcon_puthex64(dblmap_dist);
	panthera_debugcon_write(" hi64_syscall=0x");
	panthera_debugcon_puthex64((uint64_t)(uintptr_t)hi64_syscall);
	panthera_debugcon_write("\n");
