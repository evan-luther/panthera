...
/*
 * Panthera: the open XNU drop expects private firehose runtime helpers that
 * are not shipped in the public SDK/toolchain. Fall back to the boot chunk so
 * kernel logging remains functional enough for bring-up.
 */
    ...
    uint16_t pubsize, uint16_t privsize, uint8_t **privptr)
{
	...
	long offset = firehose_chunk_tracepoint_try_reserve(fbc, stamp, stream, 0,
	    pubsize, privsize, privptr);
	if (offset <= 0) {
		return NULL;
	}

	return firehose_chunk_tracepoint_begin(fbc, stamp, pubsize,
	           thread_tid(current_thread()), offset);
}
...
__firehose_buffer_tracepoint_flush(firehose_tracepoint_t ft,
    firehose_tracepoint_id_u ftid)
{
	if (ft == NULL || firehose_boot_chunk == NULL) {
		return;
	}
...
{
	if (size != NULL) {
		*size = __firehose_buffer_kernel_chunk_count * FIREHOSE_CHUNK_SIZE;
	}
	return (firehose_buffer_t)kernel_firehose_addr;
}

bool
__firehose_merge_updates(__unused firehose_push_reply_t update)
{
	return false;
}
