...
#define PANTHERA_OSR_DBG(x) panthera_debugcon_write_public(x)
#define PANTHERA_OSR_HEX(x) panthera_debugcon_puthex64_public((uint64_t)(uintptr_t)(x))
#else
#define PANTHERA_OSR_DBG(x) do { } while (0)
#define PANTHERA_OSR_HEX(x) do { } while (0)
#endif
...
OSlibkernInit(void)
{
	...
	PANTHERA_OSR_DBG("PANTHERA:OSR enter\n");
	PANTHERA_OSR_DBG("PANTHERA:OSR OSMetaClassBase initialize\n");
	OSMetaClassBase::initialize();
	PANTHERA_OSR_DBG("PANTHERA:OSR OSMetaClassBase initialize done\n");

	g_kernel_kmod_info.address = (vm_address_t) &_mh_execute_header;

	PANTHERA_OSR_DBG("PANTHERA:OSR OSRuntimeInitializeCPP\n");
	...
	PANTHERA_OSR_DBG("PANTHERA:OSR OSRuntimeInitializeCPP done\n");

	gKernelCPPInitialized = true;
	PANTHERA_OSR_DBG("PANTHERA:OSR done\n");
	...
	uintptr_t                  textEnd)
{
	...
	PANTHERA_OSR_DBG("PANTHERA:OSR sectscan enter\n");
	...
	for (section = firstsect(segment);
		...
		PANTHERA_OSR_DBG("PANTHERA:OSR sectscan match\n");
		PANTHERA_OSR_DBG("PANTHERA:OSR sect addr=");
		PANTHERA_OSR_HEX(section->addr);
		PANTHERA_OSR_DBG("\n");
		PANTHERA_OSR_DBG("PANTHERA:OSR sect size=");
		PANTHERA_OSR_HEX(section->size);
		PANTHERA_OSR_DBG("\n");
		...
		PANTHERA_OSR_DBG("PANTHERA:OSR structors loop\n");
		for (;
		    (firstIndex < num_structors)
			...
			if ((structor = structors[firstIndex])) {
				...
				if (firstIndex == 0) {
					PANTHERA_OSR_DBG("PANTHERA:OSR structor0=");
					PANTHERA_OSR_HEX(value);
					PANTHERA_OSR_DBG("\n");
				}
				(*structor)();
				if (firstIndex == 0) {
					PANTHERA_OSR_DBG("PANTHERA:OSR structor0 done\n");
				}
		...
		PANTHERA_OSR_DBG("PANTHERA:OSR structors loop done\n");
		if (metaHandle) {
			result = OSMetaClass::checkModLoad(metaHandle);
		}
		break;
	} /* for (section...) */
	PANTHERA_OSR_DBG("PANTHERA:OSR sectscan done\n");
	return result;
}
	...
	OSKext                   * theKext)
{
	 ...
	 */
	PANTHERA_OSR_DBG("PANTHERA:OSR preModLoad\n");
	metaHandle = OSMetaClass::preModLoad(kmodInfo->name);
	PANTHERA_OSR_DBG("PANTHERA:OSR preModLoad done\n");
	assert(metaHandle);
	if (!metaHandle) {
		goto finish;
	}
	...
	for (segment = firstsegfromheader(header);
		...
		PANTHERA_OSR_DBG("PANTHERA:OSR segment\n");
		PANTHERA_OSR_DBG("PANTHERA:OSR seg vmaddr=");
		PANTHERA_OSR_HEX(segment->vmaddr);
		PANTHERA_OSR_DBG("\n");
		load_success = OSRuntimeCallStructorsInSection(
			theKext, kmodInfo, metaHandle, segment,
			sectionNames[kOSSectionNameInitializer],
			textStart, textEnd);
		PANTHERA_OSR_DBG("PANTHERA:OSR segment done\n");
	} /* for (segment...) */
	...
	PANTHERA_OSR_DBG("PANTHERA:OSR postModLoad done\n");
