...
extern "C" void panthera_debugcon_write_public(const char *string);
extern "C" void panthera_debugcon_puthex64_public(uint64_t value);
	...
	void              * user_data)
{
	...
	char logBuffer[256];
	va_list copyList;
	bool pantheraTraceKxld = false;
	const char *kextID = NULL;

	if (theKext) {
		kextID = theKext->getIdentifierCString();
		pantheraTraceKxld =
		    !strcmp(kextID, "com.apple.driver.AppleI386GenericPlatform") ||
		    !strcmp(kextID, "com.apple.filesystems.hfs.encodings.kext") ||
		    !strcmp(kextID, "com.apple.filesystems.hfs.kext");
	}
	...
	if (pantheraTraceKxld || level == kKxldLogErr || level == kKxldLogWarn) {
		va_copy(copyList, argList);
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wformat-nonliteral"
		vsnprintf(logBuffer, sizeof(logBuffer), format, copyList);
#pragma clang diagnostic pop
		va_end(copyList);
		panthera_debugcon_write_public("PANTHERA:KXLD[");
		panthera_debugcon_puthex64_public((uint64_t)(uint32_t)subsystem);
		panthera_debugcon_write_public("/");
		panthera_debugcon_puthex64_public((uint64_t)(uint32_t)level);
		panthera_debugcon_write_public("] ");
		panthera_debugcon_write_public(kextID ? kextID : "(null)");
		panthera_debugcon_write_public(": ");
		panthera_debugcon_write_public(logBuffer);
		panthera_debugcon_write_public("\n");
	}
...
#if CONFIG_KXLD && (defined(__i386__) || defined(__x86_64__))
	/*
	 * Panthera relies on runtime kext linking during early x86 bring-up.
	 * Keep the kernel __LINKEDIT segment resident so KXLD can still
	 * resolve kernel symbols after boot-time handoff.
	 */
	sKeepSymbols = true;
#endif
#if CONFIG_DTRACE
	...
	OSString     * aPath)
{
	...
	if (scratchBool == kOSBooleanTrue) {
		...
		flags.loaded = 1;            // Panthera: mark as loaded so dependency->load() skips re-load
		flags.started = 1;

		/* Panthera: create synthetic kmod_info for kernel-resource kexts
		 * so that dependent kexts can reference them during load.
		 */
		if (!kmod_info) {
			kmod_info = (kmod_info_t *)kalloc_type(kmod_info_t, (zalloc_flags_t)(Z_WAITOK | Z_ZERO));
			if (kmod_info) {
				kmod_info->info_version = KMOD_INFO_VERSION;
				strlcpy(kmod_info->name, getIdentifierCString(), sizeof(kmod_info->name));
				OSString *kv = OSDynamicCast(OSString, getPropertyForHostArch(kCFBundleVersionKey));
				if (kv) {
					strlcpy(kmod_info->version, kv->getCStringNoCopy(), sizeof(kmod_info->version));
				}
			}
		}
	...
	OSArray            * personalityNames)
{
	 ...
	 *
	 * Interface/kernel-resource pseudo-kexts still need to flow through
	 * loadExecutable() so they get the synthetic kmod_info used by loaded
	 * kext dependency tracking.
	 */
	if (!declaresExecutable() && !isInterface()) {
	...
	PANTHERA_TRACE(printf("PANTHERA:KLOAD %s loading %d dependencies\n", getIdentifierCString(), count));
	for (i = 0; i < count; i++) {
		 ...
		 */
		PANTHERA_TRACE(printf("PANTHERA:KLOAD   dep[%d] %s (loaded=%d started=%d interface=%d)\n",
		       i, dependency->getIdentifierCString(),
		       dependency->isLoaded(), dependency->isStarted(), dependency->isInterface()));
		result = dependency->load(dependenciesStartOpt,
		    dependenciesStartMatchingOpt,
		    /* personalityNames */ NULL);
		PANTHERA_TRACE(printf("PANTHERA:KLOAD   dep[%d] %s load result=0x%x\n",
		       i, dependency->getIdentifierCString(), result));
		if (result != KERN_SUCCESS) {
			OSKextLog(this,
			    kOSKextLogErrorLevel |
			    kOSKextLogLoadFlag | kOSKextLogDependenciesFlag,
	...
	PANTHERA_TRACE(printf("PANTHERA:KLOAD %s loadExecutable\n", getIdentifierCString()));
	result = loadExecutable();
	PANTHERA_TRACE(printf("PANTHERA:KLOAD %s loadExecutable result=0x%x\n", getIdentifierCString(), result));
	if (result != KERN_SUCCESS) {
		goto finish;
	}

	PANTHERA_TRACE(printf("PANTHERA:KLOAD %s post-loadExecutable setup\n", getIdentifierCString()));
...
loaded:
	if (isExecutable() && !flags.started) {
		if (startOpt == kOSKextExcludeNone) {
			PANTHERA_TRACE(printf("PANTHERA:KEXT calling start() for %s\n", getIdentifierCString()));
			result = start();
			PANTHERA_TRACE(printf("PANTHERA:KEXT start() returned 0x%x for %s\n", result, getIdentifierCString()));
			if (result != kOSReturnSuccess) {
				OSKextLog(this,
				    kOSKextLogErrorLevel | kOSKextLogLoadFlag,
...
OSKext::loadExecutable()
{
	...
	/*
	 * Panthera boots through booter-provided startup kexts rather than a full
	 * prelinked collection, so early boot needs to link those kexts before any
	 * userspace kext client exists. Keep the entitlement gate for late
	 * user-space loading, but bypass it during the pre-userspace boot window.
	 */
	if (sUserLoadsActive &&
	    FALSE == IOCurrentTaskHasEntitlement(kOSKextCollectionManagementEntitlement)) {
		OSKextLog(this,
		    kOSKextLogErrorLevel | kOSKextLogLoadFlag,
	 ...
	 * should exist.
	 */
	panthera_debugcon_write_public("PANTHERA:KXLD load ");
	panthera_debugcon_write_public(getIdentifierCString());
	panthera_debugcon_write_public("\n");
	...
	panthera_debugcon_write_public("PANTHERA:KXLD load done ");
	panthera_debugcon_write_public(getIdentifierCString());
	panthera_debugcon_write_public(" result=0x");
	panthera_debugcon_puthex64_public((uint64_t)(uint32_t)kxldResult);
	panthera_debugcon_write_public("\n");
...
OSKext::start(bool startDependenciesFlag)
{
	...
	PANTHERA_TRACE(printf("PANTHERA:KSTART %s OSRuntimeInitializeCPP\n", getIdentifierCString()));
	result = OSRuntimeInitializeCPP(this);
	PANTHERA_TRACE(printf("PANTHERA:KSTART %s OSRuntimeInitializeCPP result=0x%x\n", getIdentifierCString(), result));
	if (result == KERN_SUCCESS) {
		PANTHERA_TRACE(printf("PANTHERA:KSTART %s calling startfunc at %p\n", getIdentifierCString(), (void*)startfunc));
		result = startfunc(kmod_info, kmodStartData);
		PANTHERA_TRACE(printf("PANTHERA:KSTART %s startfunc result=0x%x\n", getIdentifierCString(), result));
	}
