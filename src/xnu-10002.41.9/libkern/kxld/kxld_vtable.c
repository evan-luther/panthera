...
static void
panthera_kxld_vtable_detail(const char *stage, const char *name, uint64_t value)
{
	panthera_debugcon_write_public("PANTHERA:KXLD vtable ");
	panthera_debugcon_write_public(stage);
	panthera_debugcon_write_public(" ");
	panthera_debugcon_write_public(name ? name : "(null)");
	panthera_debugcon_write_public(" value=0x");
	panthera_debugcon_puthex64_public(value);
	panthera_debugcon_write_public("\n");
}
#else
static void
panthera_kxld_vtable_detail(const char *stage __unused, const char *name __unused,
...
static u_long get_vtable_symbol_size(const KXLDSym *vtable_sym,
    ...
    const KXLDObject *object, const KXLDDict *defined_cxx_symbols)
{
	...
	vtable_sect = kxld_object_get_section_by_index(object,
	    vtable_sym->sectnum);
	require_action(vtable_sect, finish,
	    rval = KERN_FAILURE;
	    panthera_kxld_vtable_detail("no_sect", vtable_sym->name, 0));
	vtable_size = get_vtable_symbol_size(vtable_sym, vtable_sect, symtab);
	require_action(vtable_size != 0, finish,
	    rval = KERN_FAILURE;
	    panthera_kxld_vtable_detail("size_zero_sym", vtable_sym->name,
	    (uint64_t)vtable_sym->base_addr);
	    panthera_kxld_vtable_detail("size_zero_sectbase", vtable_sym->name,
	    (uint64_t)vtable_sect->base_addr);
	    panthera_kxld_vtable_detail("size_zero_sectsize", vtable_sym->name,
	    (uint64_t)vtable_sect->size);
	    panthera_kxld_vtable_detail("size_zero_sectnum", vtable_sym->name,
	    (uint64_t)vtable_sym->sectnum);
	    panthera_kxld_vtable_detail("size_zero", vtable_sym->name, 0));
	...
	if (kxld_object_is_linked(object)) {
		rval = init_by_entries(vtable, relocator, defined_cxx_symbols,
		    vtable_size);
		if (rval) {
			panthera_kxld_vtable_detail("init_by_entries", vtable_sym->name,
			    (uint64_t)(uint32_t)rval);
			goto finish;
		}
...
*******************************************************************************/
static u_long
get_vtable_symbol_size(const KXLDSym *vtable_sym, const KXLDSect *sect,
    const KXLDSymtab *symtab)
{
	kxld_addr_t start_offset = 0;
	kxld_addr_t end_offset = 0;
	u_int i = 0;
	u_int nsymbols = 0;
	...
	start_offset = kxld_sym_get_section_offset(vtable_sym, sect);
	end_offset = sect->size;
	nsymbols = kxld_symtab_get_num_symbols(symtab);

	for (i = 0; i < nsymbols; ++i) {
		KXLDSym *candidate = kxld_symtab_get_symbol_by_index(symtab, i);
		kxld_addr_t candidate_offset = 0;

		if (!candidate || candidate == vtable_sym) {
			continue;
		}
		if (!kxld_sym_is_vtable(candidate) || candidate->sectnum != vtable_sym->sectnum) {
			continue;
		}

		candidate_offset = kxld_sym_get_section_offset(candidate, sect);
		if (candidate_offset > start_offset && candidate_offset < end_offset) {
			end_offset = candidate_offset;
		}
	}

	return (end_offset > start_offset) ? (u_long)(end_offset - start_offset) : 0;
}
...
init_by_entries(KXLDVTable *vtable, const KXLDRelocator *relocator,
    const KXLDDict *defined_cxx_symbols, u_long vtable_size)
{
	kern_return_t rval = KERN_FAILURE;
	...
	(void) get_vtable_base_sizes(relocator->is_32_bit,
	    &vtable_entry_size, &vtable_header_size);
	require_action(vtable_size >= vtable_header_size, finish,
	    rval = KERN_FAILURE);

	/* Count the number of entries (the vtable is null-terminated) */

	entry_offset = vtable_header_size;
	while (entry_offset + vtable_entry_size <= vtable_size) {
    ...
    KXLDObject *object)
{
	...
	require_action(vtable->entries.nitems >= super_vtable->entries.nitems, finish,
	    rval = KERN_FAILURE;
	    panthera_kxld_vtable_detail("short_child_count", vtable->name,
	    (uint64_t)vtable->entries.nitems);
	    panthera_kxld_vtable_detail("short_parent_count", super_vtable->name,
	    (uint64_t)super_vtable->entries.nitems);
