...
static void
panthera_kxld_relocate_failure(const char *stage, kern_return_t rval)
{
	panthera_debugcon_write_public("PANTHERA:KXLD relocate ");
...
static void
panthera_kxld_vtable_failure(const char *stage, const char *name, kern_return_t rval)
{
...
#else
static void
panthera_kxld_relocate_failure(const char *stage __unused, kern_return_t rval __unused)
{
}

static void
panthera_kxld_vtable_failure(const char *stage __unused, const char *name __unused,
    ...
    const KXLDDict *defined_cxx_symbols)
{
	...
	if (rval) {
		panthera_kxld_relocate_failure("index_cxx_symbols", rval);
		goto finish;
	}

	rval = kxld_object_index_symbols_by_name(kext->kext);
	if (rval) {
		panthera_kxld_relocate_failure("index_symbols", rval);
		goto finish;
	}

	rval = kxld_object_relocate(kext->kext, link_address);
	if (rval) {
		panthera_kxld_relocate_failure("object_relocate", rval);
		goto finish;
	}

	rval = resolve_symbols(kext, defined_symbols, obsolete_symbols);
	if (rval) {
		panthera_kxld_relocate_failure("resolve_symbols", rval);
		goto finish;
	}

	rval = create_vtables(kext, defined_cxx_symbols, /* defined_symbols */ NULL);
	if (rval) {
		panthera_kxld_relocate_failure("create_vtables", rval);
		goto finish;
	}
	...
	if (isSplitKext == FALSE) {
		...
		if (rval) {
			panthera_kxld_relocate_failure("patch_vtables", rval);
			goto finish;
		}
	}
	...
	if (rval) {
		panthera_kxld_relocate_failure("validate_symbols", rval);
		goto finish;
	}

	rval = kxld_object_process_relocations(kext->kext, patched_vtables);
	if (rval) {
		panthera_kxld_relocate_failure("process_relocations", rval);
		goto finish;
	}
    ...
    const KXLDDict *defined_symbols)
{
	...
	while ((sym = kxld_symtab_iterator_get_next(&iter))) {
		...
		rval = kxld_vtable_init(vtable, vtable_sym, kext->kext,
		    defined_cxx_symbols);
		if (rval) {
			panthera_kxld_vtable_failure("init", vtable_sym ? vtable_sym->name : NULL, rval);
			goto finish;
		}
		...
		if (!kxld_object_is_linked(kext->kext)) {
			if (meta_vtable_sym) {
				...
				if (rval) {
					panthera_kxld_vtable_failure("meta_init",
					    meta_vtable_sym ? meta_vtable_sym->name : NULL, rval);
					goto finish;
				}
			} else {
