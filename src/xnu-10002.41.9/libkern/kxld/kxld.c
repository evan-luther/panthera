...
static void
panthera_kxld_stage_failure(const char *stage, kern_return_t rval)
{
	panthera_debugcon_write_public("PANTHERA:KXLD stage ");
	panthera_debugcon_write_public(stage);
	panthera_debugcon_write_public(" rval=0x");
	panthera_debugcon_puthex64_public((uint64_t)(uint32_t)rval);
	panthera_debugcon_write_public("\n");
}

static void
panthera_kxld_dependency_trace(const char *phase, const char *name, kern_return_t rval)
{
	panthera_debugcon_write_public("PANTHERA:KXLD dep ");
	panthera_debugcon_write_public(phase);
	panthera_debugcon_write_public(" ");
...
#else
static void
panthera_kxld_stage_failure(const char *stage __unused, kern_return_t rval __unused)
{
}

static void
panthera_kxld_dependency_trace(const char *phase __unused, const char *name __unused,
    kern_return_t rval __unused)
{
}
#endif
	...
	kxld_addr_t       * kmod_info_kern)
{
	...
	rval = init_context(context, ndependencies);
	if (rval) {
		panthera_kxld_stage_failure("init_context", rval);
		goto finish;
	}
	...
	rval = init_kext_objects(context, file, size, name,
	    dependencies, ndependencies);
	if (rval) {
		panthera_kxld_stage_failure("init_kext_objects", rval);
		goto finish;
	}

	linked_object = allocate_kext(context, callback_data,
	    &vmaddr, &vmsize, &linked_object_alloc);
	require_action(linked_object, finish,
	    rval = KERN_RESOURCE_SHORTAGE;
	    panthera_kxld_stage_failure("allocate_kext", rval));
	...
	rval = kxld_kext_relocate(context->kext,
	    ...
	    &context->defined_cxx_symbols_by_value);
	if (rval) {
		panthera_kxld_stage_failure("kxld_kext_relocate", rval);
		goto finish;
	}
	...
	rval = kxld_kext_export_linked_object(context->kext,
	    ...
	    kmod_info_kern);
	if (rval) {
		panthera_kxld_stage_failure("export_linked_object", rval);
		goto finish;
	}
    ...
    u_int ndependencies)
{
	...
	for (i = 0; i < ndependencies; ++i) {
		...
		require_action(kext_object, finish,
		    rval = KERN_FAILURE;
		    panthera_kxld_dependency_trace("kext_object", dependencies[i].kext_name, rval));
		...
		if (dependencies[i].interface) {
			...
			require_action(interface_object, finish,
			    rval = KERN_FAILURE;
			    panthera_kxld_dependency_trace("interface_object", dependencies[i].interface_name, rval));
		}

		rval = kxld_kext_init(kext, kext_object, interface_object);
		if (rval) {
			panthera_kxld_dependency_trace("kext_init", dependencies[i].kext_name, rval);
			goto finish;
		}
		...
		if (dependencies[i].is_direct_dependency) {
			rval = kxld_kext_export_symbols(kext,
			    ...
			    &context->defined_cxx_symbols_by_value);
			if (rval) {
				panthera_kxld_dependency_trace("export_symbols_direct",
				    dependencies[i].kext_name, rval);
				goto finish;
			}
		} else {
			rval = kxld_kext_export_symbols(kext,
			    ...
			    &context->defined_cxx_symbols_by_value);
			if (rval) {
				panthera_kxld_dependency_trace("export_symbols_indirect",
				    dependencies[i].kext_name, rval);
				goto finish;
			}
		}
	...
	for (i = 0; i < context->dependencies.nitems; ++i) {
		...
		rval = kxld_kext_export_vtables(kext,
		    ...
		    &context->vtables_by_name);
		if (rval) {
			panthera_kxld_dependency_trace("export_vtables",
			    dependencies[i].kext_name, rval);
			goto finish;
		}
	}
	...
	require_action(kext_object, finish,
	    rval = KERN_FAILURE;
	    panthera_kxld_dependency_trace("target_object", name, rval));

	rval = kxld_kext_init(context->kext, kext_object, /* interface */ NULL);
	if (rval) {
		panthera_kxld_dependency_trace("target_init", name, rval);
		goto finish;
	}
	...
	rval = kxld_kext_export_symbols(context->kext,
	    ...
	    &context->defined_cxx_symbols_by_value);
	if (rval) {
		panthera_kxld_dependency_trace("target_export_symbols", name, rval);
		goto finish;
	}
