...
IOKitBSDInit( void )
{
	PANTHERA_TRACE(printf("PANTHERA:IOBSD publish IOBSD\n"));
	IOService::publishResource("IOBSD");
	PANTHERA_TRACE(printf("PANTHERA:IOBSD publish IOBSD done\n"));
	...
	PANTHERA_TRACE(printf("PANTHERA:IOBSD return\n"));
...
IOTaskHasStringEntitlement(task_t task, const char *entitlement, const char *value)
{
	...
	/* Panthera: AMFI kext may not be loaded during bring-up */
	if (!amfi || !amfi->OSEntitlements.queryEntitlementStringWithProc) {
		return false;
	}
...
IOTaskHasEntitlement(task_t task, const char *entitlement)
{
	...
	/* Panthera: AMFI kext may not be loaded during bring-up */
	if (!amfi || !amfi->OSEntitlements.queryEntitlementBooleanWithProc) {
		return false;
	}
...
IOTaskGetEntitlement(task_t task, const char *entitlement)
{
	...
	/* Panthera: AMFI kext may not be loaded during bring-up */
	if (!amfi || !amfi->OSEntitlements.copyEntitlementAsOSObjectWithProc) {
		return NULL;
	}
