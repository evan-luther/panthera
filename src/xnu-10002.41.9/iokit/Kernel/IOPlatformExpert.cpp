...
enum {
	kPantheraPlatformNubCPU      = 1u << 0,
	kPantheraPlatformNubPIC      = 1u << 1,
	kPantheraPlatformNubIOAPIC   = 1u << 2,
	kPantheraPlatformNubClock    = 1u << 3,
	kPantheraPlatformNubPS2      = 1u << 4,
	kPantheraPlatformNubDisplay  = 1u << 5,
	kPantheraPlatformNubBIOS     = 1u << 6,
	kPantheraPlatformNubPCI      = 1u << 7,
	kPantheraPlatformNubAll      = (1u << 8) - 1u,
};

enum {
	kPantheraCMOSAddressPort   = 0x70,
	kPantheraCMOSDataPort      = 0x71,
	kPantheraRTCRegSeconds     = 0x00,
	kPantheraRTCRegMinutes     = 0x02,
	kPantheraRTCRegHours       = 0x04,
	kPantheraRTCRegWeekday     = 0x06,
	kPantheraRTCRegDayOfMonth  = 0x07,
	kPantheraRTCRegMonth       = 0x08,
	kPantheraRTCRegYear        = 0x09,
	kPantheraRTCRegStatusA     = 0x0a,
	kPantheraRTCRegStatusB     = 0x0b,
	kPantheraRTCRegCentury     = 0x32,
};
...
typedef struct {
	...
	uint8_t century;
	uint8_t statusB;
} panthera_cmos_rtc_snapshot_t;

static uint8_t
panthera_cmos_read(uint8_t reg)
{
	ml_port_io_write8(kPantheraCMOSAddressPort, (uint8_t)(0x80u | reg));
	return ml_port_io_read8(kPantheraCMOSDataPort);
}

static bool
panthera_cmos_updating(void)
{
	return (panthera_cmos_read(kPantheraRTCRegStatusA) & 0x80u) != 0;
}

static uint8_t
panthera_rtc_bcd_to_int(uint8_t value)
{
	return (uint8_t)(((value >> 4) * 10u) + (value & 0x0fu));
}

static bool
panthera_read_cmos_rtc_snapshot(panthera_cmos_rtc_snapshot_t *snapshot)
{
	panthera_cmos_rtc_snapshot_t first = {};
	panthera_cmos_rtc_snapshot_t second = {};
	...
	for (attempts = 0; attempts < 1000; attempts++) {
		while (panthera_cmos_updating()) {
			IOSleep(1);
		}

		first.second = panthera_cmos_read(kPantheraRTCRegSeconds);
		first.minute = panthera_cmos_read(kPantheraRTCRegMinutes);
		first.hour = panthera_cmos_read(kPantheraRTCRegHours);
		first.weekday = panthera_cmos_read(kPantheraRTCRegWeekday);
		first.day = panthera_cmos_read(kPantheraRTCRegDayOfMonth);
		first.month = panthera_cmos_read(kPantheraRTCRegMonth);
		first.year = panthera_cmos_read(kPantheraRTCRegYear);
		first.century = panthera_cmos_read(kPantheraRTCRegCentury);
		first.statusB = panthera_cmos_read(kPantheraRTCRegStatusB);

		while (panthera_cmos_updating()) {
			IOSleep(1);
		}

		second.second = panthera_cmos_read(kPantheraRTCRegSeconds);
		second.minute = panthera_cmos_read(kPantheraRTCRegMinutes);
		second.hour = panthera_cmos_read(kPantheraRTCRegHours);
		second.weekday = panthera_cmos_read(kPantheraRTCRegWeekday);
		second.day = panthera_cmos_read(kPantheraRTCRegDayOfMonth);
		second.month = panthera_cmos_read(kPantheraRTCRegMonth);
		second.year = panthera_cmos_read(kPantheraRTCRegYear);
		second.century = panthera_cmos_read(kPantheraRTCRegCentury);
		second.statusB = panthera_cmos_read(kPantheraRTCRegStatusB);
...
static bool
panthera_normalize_cmos_rtc_snapshot(panthera_cmos_rtc_snapshot_t *snapshot,
    int *fullYear)
{
	bool binaryMode;
	bool hour24Mode;
	...
	if (snapshot == NULL || fullYear == NULL) {
		return false;
	}

	binaryMode = (snapshot->statusB & 0x04u) != 0;
	hour24Mode = (snapshot->statusB & 0x02u) != 0;
	pm = (snapshot->hour & 0x80u) != 0;

	if (!binaryMode) {
		snapshot->second = panthera_rtc_bcd_to_int(snapshot->second);
		snapshot->minute = panthera_rtc_bcd_to_int(snapshot->minute);
		snapshot->hour = (uint8_t)(snapshot->hour & 0x7fu);
		snapshot->hour = panthera_rtc_bcd_to_int(snapshot->hour);
		snapshot->weekday = panthera_rtc_bcd_to_int(snapshot->weekday);
		snapshot->day = panthera_rtc_bcd_to_int(snapshot->day);
		snapshot->month = panthera_rtc_bcd_to_int(snapshot->month);
		snapshot->year = panthera_rtc_bcd_to_int(snapshot->year);
		snapshot->century = panthera_rtc_bcd_to_int(snapshot->century);
	} else {
		snapshot->hour &= 0x7fu;
	}
	...
	if (!hour24Mode) {
		...
		if (pm) {
			snapshot->hour = (uint8_t)(snapshot->hour + 12u);
		}
	}

	year = snapshot->year;
	century = snapshot->century;
	...
	if (snapshot->second > 59 || snapshot->minute > 59 || snapshot->hour > 23 ||
	    snapshot->day < 1 || snapshot->day > 31 || snapshot->month < 1 || snapshot->month > 12 ||
	    year < 1970) {
		return false;
	}
...
static long long
panthera_days_from_civil(int year, unsigned int month, unsigned int day)
{
	int adjustedYear = year - (month <= 2 ? 1 : 0);
	int era = (adjustedYear >= 0 ? adjustedYear : adjustedYear - 399) / 400;
	unsigned int yearOfEra = (unsigned int)(adjustedYear - era * 400);
	unsigned int monthPrime = (unsigned int)((int)month + (month > 2 ? -3 : 9));
	unsigned int dayOfYear = (153u * monthPrime + 2u) / 5u + day - 1u;
	unsigned int dayOfEra = yearOfEra * 365u + yearOfEra / 4u - yearOfEra / 100u + dayOfYear;

	return (long long)era * 146097ll + (long long)dayOfEra - 719468ll;
}

static bool
panthera_read_cmos_rtc_seconds(clock_sec_t *secs)
{
	panthera_cmos_rtc_snapshot_t snapshot = {};
	int year;
	long long days;
	long long result;
	...
	if (!panthera_read_cmos_rtc_snapshot(&snapshot)) {
		return false;
	}

	if (!panthera_normalize_cmos_rtc_snapshot(&snapshot, &year)) {
		return false;
	}

	days = panthera_days_from_civil(year, snapshot.month, snapshot.day);
	if (days < 0) {
		return false;
	}

	result = days * 86400ll +
	    (long long)snapshot.hour * 3600ll +
	    (long long)snapshot.minute * 60ll +
	    (long long)snapshot.second;
	if (result < 0) {
		return false;
	}

	*secs = (clock_sec_t)result;
	return true;
}
...
typedef struct {
	...
	UInt8 reserved[3];
} panthera_pci_bus_info_t;

static uint32_t
panthera_platform_nub_mask_for_name(const char *name)
{
	if (0 == strcmp(name, "cpu")) {
		return kPantheraPlatformNubCPU;
	}
	if (0 == strcmp(name, "8259-pic")) {
		return kPantheraPlatformNubPIC;
	}
	if (0 == strcmp(name, "io-apic")) {
		return kPantheraPlatformNubIOAPIC;
	}
	if (0 == strcmp(name, "intel-clock")) {
		return kPantheraPlatformNubClock;
	}
	if (0 == strcmp(name, "ps2controller")) {
		return kPantheraPlatformNubPS2;
	}
	if (0 == strcmp(name, "display")) {
		return kPantheraPlatformNubDisplay;
	}
	if (0 == strcmp(name, "bios")) {
		return kPantheraPlatformNubBIOS;
	}
	if (0 == strcmp(name, "pci")) {
		return kPantheraPlatformNubPCI;
	}
	return 0;
}

static OSDictionary *
panthera_create_x86_platform_nub_dict(const char *name)
{
	OSDictionary *dict = OSDictionary::withCapacity(2);
	OSString *ioNameString = dict ? OSString::withCString(name) : NULL;
	OSData *dtNameData = dict ? OSData::withBytes(name, (unsigned int)strlen(name) + 1) : NULL;
	if (!dict || !ioNameString || !dtNameData) {
		OSSafeReleaseNULL(dtNameData);
		OSSafeReleaseNULL(ioNameString);
		OSSafeReleaseNULL(dict);
		return NULL;
	}

	dict->setObject("IOName", ioNameString);
	ioNameString->release();
	dict->setObject("name", dtNameData);
	dtNameData->release();

	if (0 == strcmp(name, "cpu")) {
		OSData *deviceType = OSData::withBytes("cpu", sizeof("cpu"));
		if (!deviceType) {
			dict->release();
			return NULL;
		}
		dict->setObject("device_type", deviceType);
		deviceType->release();
	}

	if (0 == strcmp(name, "pci")) {
		panthera_pci_bus_info_t info = {};
		OSData *pciBusInfo = NULL;
		...
		pciBusInfo = OSData::withBytes(&info, sizeof(info));
		if (!pciBusInfo) {
			dict->release();
			return NULL;
		}
		dict->setObject("pci-bus-info", pciBusInfo);
		pciBusInfo->release();

		OSData *deviceType = OSData::withBytes("pci", sizeof("pci"));
		if (!deviceType) {
			dict->release();
			return NULL;
		}
	...
	if (0 == strcmp(name, "bios")) {
		OSData *deviceType = OSData::withBytes("rom", sizeof("rom"));
		if (!deviceType) {
			dict->release();
			return NULL;
		}
	...
	if (0 == strcmp(name, "ps2controller")) {
		OSData *compatible = OSData::withBytes("ps2controller", sizeof("ps2controller"));
		if (!compatible) {
			dict->release();
			return NULL;
		}
		dict->setObject("compatible", compatible);
		compatible->release();
	}

	if (0 == strcmp(name, "intel-clock")) {
		OSData *deviceType = OSData::withBytes("clock", sizeof("clock"));
		if (!deviceType) {
			dict->release();
			return NULL;
		}
	...
	if (0 == strcmp(name, "io-apic")) {
		OSData *deviceType = OSData::withBytes("interrupt-controller", sizeof("interrupt-controller"));
		OSString *interruptController = NULL;
		OSNumber *physicalAddress = NULL;
		OSNumber *destinationApicID = NULL;
		OSNumber *baseVectorNumber = NULL;
		if (!deviceType) {
			dict->release();
			return NULL;
		}
		...
		interruptController = OSString::withCString("8259-pic");
		physicalAddress = OSNumber::withNumber((uint64_t) 0xfec00000, 64);
		destinationApicID = OSNumber::withNumber((uint64_t) 0, 32);
		baseVectorNumber = OSNumber::withNumber((uint64_t) 0x40, 32);
		if (!interruptController || !physicalAddress || !destinationApicID || !baseVectorNumber) {
			OSSafeReleaseNULL(baseVectorNumber);
			OSSafeReleaseNULL(destinationApicID);
			OSSafeReleaseNULL(physicalAddress);
			OSSafeReleaseNULL(interruptController);
			dict->release();
			return NULL;
		}
		dict->setObject("InterruptControllerName", interruptController);
		dict->setObject("Physical Address", physicalAddress);
		dict->setObject("Destination APIC ID", destinationApicID);
		dict->setObject("Base Vector Number", baseVectorNumber);
		interruptController->release();
		physicalAddress->release();
		destinationApicID->release();
		baseVectorNumber->release();
	}
	...
	if (0 == strcmp(name, "8259-pic")) {
		OSString *interruptController = OSString::withCString("8259-pic");
...
static void
panthera_seed_x86_platform_nubs(IOPlatformExpert *platform, uint32_t presentMask)
{
	...
	for (unsigned int i = 0; i < sizeof(names) / sizeof(names[0]); i++) {
		if (presentMask & panthera_platform_nub_mask_for_name(names[i])) {
			continue;
		}
		if (0 == strcmp(names[i], "display")) {
			kprintf("PANTHERA:PE fallback seed display platform=%s presentMask=0x%x\n",
			    platform->getMetaClass() ? platform->getMetaClass()->getClassName() : "<null>",
			    presentMask);
		}
		OSDictionary *dict = panthera_create_x86_platform_nub_dict(names[i]);
		if (!dict) {
			if (0 == strcmp(names[i], "display")) {
				kprintf("PANTHERA:PE fallback display dict creation failed\n");
			}
			PANTHERA_TRACE(printf("PANTHERA:PE fallback dict failed for %s\n", names[i]));
			continue;
		}
		...
		if (!nub) {
			if (0 == strcmp(names[i], "display")) {
				kprintf("PANTHERA:PE fallback createNub(display) returned null\n");
			}
			PANTHERA_TRACE(printf("PANTHERA:PE fallback createNub failed for %s\n", names[i]));
			continue;
		}
		if (0 == strcmp(names[i], "display")) {
			kprintf("PANTHERA:PE fallback created display class=%s name=%s provider=%s\n",
			    nub->getMetaClass() ? nub->getMetaClass()->getClassName() : "<null>",
			    nub->getName() ? nub->getName() : "<null>",
			    (nub->getProvider() && nub->getProvider()->getName()) ? nub->getProvider()->getName() : "<null>");
		}

		PANTHERA_TRACE(printf("PANTHERA:PE fallback register %s\n", nub->getName()));
		bool attached = nub->attach(platform);
		if (0 == strcmp(names[i], "display")) {
			kprintf("PANTHERA:PE fallback attach(display)=%d\n", attached);
		}
		nub->registerService();
		if (0 == strcmp(names[i], "display")) {
			kprintf("PANTHERA:PE fallback registered display boot-display=%d\n",
			    (nub->getProperty("AAPL,boot-display") != NULL));
		}
		nub->release();
	}
}
...
IOPlatformExpert::start( IOService * provider )
{
	...
	PANTHERA_TRACE(printf("PANTHERA:PE IOPlatformExpert::start(%p) class=%s\n", provider,
	       provider ? (provider->getMetaClass() ? provider->getMetaClass()->getClassName() : "?") : "nil"));

	if (!super::start(provider)) {
		PANTHERA_TRACE(printf("PANTHERA:PE IOPlatformExpert::start super::start FAILED\n"));
		return false;
	}
	...
	PANTHERA_TRACE(printf("PANTHERA:PE IOPlatformExpert::start calling configure\n"));
	bool configOk = configure(provider);
	PANTHERA_TRACE(printf("PANTHERA:PE IOPlatformExpert::start configure returned %d\n", configOk));
#if defined(__x86_64__)
	if (configOk) {
		unsigned int childCount = 0;
		uint32_t x86ChildMask = 0;
		OSIterator *childIter = getChildIterator(gIOServicePlane);
		if (childIter) {
			while (OSObject *childObj = childIter->getNextObject()) {
				childCount++;
				IORegistryEntry *child = OSDynamicCast(IORegistryEntry, childObj);
				if (child) {
					x86ChildMask |= panthera_platform_nub_mask_for_name(child->getName());
					PANTHERA_TRACE(printf("PANTHERA:PE child[%u]=%s class=%s\n",
					    childCount - 1,
					    child->getName(),
					    child->getMetaClass() ? child->getMetaClass()->getClassName() : "?"));
				}
			}
			childIter->release();
		}
		PANTHERA_TRACE(printf("PANTHERA:PE childCount=%u\n", childCount));
		PANTHERA_TRACE(printf("PANTHERA:PE x86ChildMask=0x%x\n", x86ChildMask));
		if ((x86ChildMask & kPantheraPlatformNubAll) != kPantheraPlatformNubAll) {
			PANTHERA_TRACE(printf("PANTHERA:PE x86 nubs incomplete, seeding fallback nubs\n"));
			panthera_seed_x86_platform_nubs(this, x86ChildMask);
		}
	}
#endif
	return configOk;
...
#if defined(__x86_64__)
	clock_sec_t secs = 0;
	if (panthera_read_cmos_rtc_seconds(&secs)) {
		return (long)secs;
	}
#endif
	return 0;
