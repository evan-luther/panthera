	...
	bool doNubMatching)
{
	...
	if (!persons) {
		PANTHERA_TRACE(printf("PANTHERA:CAT addDrivers: drivers is not an OSArray!\n"));
		goto finish;
	}
	PANTHERA_TRACE(printf("PANTHERA:CAT addDrivers: %d personalities, doNubMatching=%d\n",
	       persons->getCount(), doNubMatching));
...
IOCatalogue::isModuleLoaded(OSDictionary * driver, OSObject ** kextRef) const
{
	...
	if (moduleName) {
		PANTHERA_TRACE(printf("PANTHERA:CAT loading kext for matching: %s\n", moduleName->getCStringNoCopy()));
		ret = OSKext::loadKextWithIdentifier(moduleName, kextRef);
		PANTHERA_TRACE(printf("PANTHERA:CAT loadKext result=0x%x for %s\n", ret, moduleName->getCStringNoCopy()));
		if (kOSKextReturnDeferred == ret) {
			PANTHERA_TRACE(printf("PANTHERA:CAT loadKext DEFERRED: %s\n", moduleName->getCStringNoCopy()));
