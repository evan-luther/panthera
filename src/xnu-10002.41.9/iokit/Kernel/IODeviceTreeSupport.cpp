...
#define PANTHERA_IODT_DBG(x) panthera_debugcon_write_public(x)
#else
#define PANTHERA_IODT_DBG(x) do { } while (0)
#endif
...
IODeviceTreeAlloc( void * dtTop )
{
	...
	DTEntry                     dtChild;
	DTEntry                     mapEntry = NULL;
	OSArray *                   stack;
	OSData *                    prop;
	OSDictionary *              allInts;
	vm_offset_t *               dtMap = NULL;
...
#if defined(__x86_64__)
	PANTHERA_IODT_DBG("PANTHERA:IODT map skip\n");
	foundDTNode = false;
#else
	...
	PANTHERA_IODT_DBG("PANTHERA:IODT make root\n");
	parent = MakeReferenceTable((DTEntry)dtTop, freeDT );
	PANTHERA_IODT_DBG("PANTHERA:IODT make root done\n");

	stack = OSArray::withObjects((const OSObject **) &parent, 1, 10 );
	PANTHERA_IODT_DBG("PANTHERA:IODT iter init\n");
	SecureDTInitEntryIterator((DTEntry)dtTop, &iter );
	PANTHERA_IODT_DBG("PANTHERA:IODT iter init done\n");
	...
	PANTHERA_IODT_DBG("PANTHERA:IODT done\n");
