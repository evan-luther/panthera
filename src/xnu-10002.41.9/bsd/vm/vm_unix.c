...
scdir_ok:
		...
		/*
		 * Panthera development cache is host-built and intentionally unsigned.
		 * After the ownership, restricted-flag, and directory checks above,
		 * allow the shared cache mappings through without code-signature coverage.
		 */
		bool panthera_skip_shared_cache_cs_check = false;
#if XNU_TARGET_OS_OSX && defined(__x86_64__)
		panthera_skip_shared_cache_cs_check = true;
#endif

		/* check that the mappings are properly covered by code signatures */
		if (!cs_system_enforcement() || panthera_skip_shared_cache_cs_check) {
