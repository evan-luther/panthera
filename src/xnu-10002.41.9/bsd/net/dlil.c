    ...
    const struct sockaddr_dl *target_hw, const struct sockaddr *target_proto)
{
	struct if_proto *proto;
	errno_t result = 0;
	static uint32_t panthera_arp_trace_count = 0;
	static int panthera_arp_trace_checked = 0;
	static int panthera_arp_trace_enabled = 0;
	uint32_t trace_this;

	if (!panthera_arp_trace_checked) {
		int value = 0;
		panthera_arp_trace_enabled =
		    PE_parse_boot_argn("panthera_arptrace", &value, sizeof(value)) &&
		    value != 0;
		panthera_arp_trace_checked = 1;
	}

	trace_this = panthera_arp_trace_enabled &&
	    panthera_arp_trace_count++ < 16;

	if (trace_this) {
		printf("PANTHERA:ARP internal enter if=%s op=%u flags=0x%x target_family=%u\n",
		    if_name(ifp), arpop, ifp->if_flags, target_proto->sa_family);
	}

	if ((ifp->if_flags & IFF_NOARP) != 0) {
		result = ENOTSUP;
		if (trace_this) {
			printf("PANTHERA:ARP internal noarp result=%d\n", result);
		}
		goto done;
	}
	...
	if (proto == NULL) {
		result = ENOTSUP;
		if (trace_this) {
			printf("PANTHERA:ARP internal no_proto result=%d\n", result);
		}
	} else {
		...
		if (arpp == NULL) {
			result = ENOTSUP;
			if (trace_this) {
				printf("PANTHERA:ARP internal no_send_arp result=%d\n",
				    result);
			}
		} else {
			switch (arpop) {
			...
			if (trace_this) {
				printf("PANTHERA:ARP internal sent result=%d\n", result);
			}
		}
		if_proto_free(proto);
	}
