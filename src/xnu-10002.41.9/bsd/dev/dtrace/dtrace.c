...
{
	printf("dtrace_postinit: skipping dtrace_attach (Panthera bring-up)\n");
	/*
	 * dtrace_attach() hangs during Panthera bring-up — the vmem_create
	 * with VM_SLEEP blocks indefinitely.  DTrace is not needed for boot,
	 * so skip it entirely until the VM subsystem is fully stable.
	 */
	return;
}
#undef DTRACE_MAJOR
