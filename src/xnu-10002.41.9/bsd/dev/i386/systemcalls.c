...
static void
panthera_exit_syscall_trace(const char *s)
{
	while (*s) {
...
unix_syscall64(x86_saved_state_t *state)
{
	...
	syscode = (code < nsysent) ? code : SYS_invalid;
	if (code == SYS_exit) {
		panthera_exit_syscall_trace("PANTHERA:USC64 exit dispatch\n");
	}
	DEBUG_KPRINT_SYSCALL_UNIX(
	...
	AUDIT_SYSCALL_ENTER(code, p, uthread);
	if (code == SYS_exit) {
		panthera_exit_syscall_trace("PANTHERA:USC64 before sy_call exit\n");
	}
	error = (*(callp->sy_call))((void *) p, vt, &(uthread->uu_rval[0]));
	if (code == SYS_exit) {
		panthera_exit_syscall_trace("PANTHERA:USC64 after sy_call exit\n");
	}
	AUDIT_SYSCALL_EXIT(code, p, uthread, error);
