...
static SECURITY_READ_ONLY_LATE(uint32_t) csr_boot_flags = 0;
static SECURITY_READ_ONLY_LATE(csr_config_t) csr_boot_active_config = 0;
...
csr_bootstrap(void)
{
	...
	csr_boot_flags = args->flags;
	if (csr_boot_flags & kBootArgsFlagCSRActiveConfig) {
		csr_boot_active_config = args->csrActiveConfig & CSR_VALID_FLAGS;
	}

	if (csr_boot_flags & kBootArgsFlagCSRBoot) {
...
csr_check(csr_config_t mask)
{
	if (mask & CSR_ALLOW_DEVICE_CONFIGURATION) {
		return (csr_boot_flags & kBootArgsFlagCSRConfigMode) ? 0 : EPERM;
	}
