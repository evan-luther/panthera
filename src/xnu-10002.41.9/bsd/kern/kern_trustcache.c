...
trust_cache_runtime_init(void)
{
	...
	if (img4if == NULL) {
		printf("trust cache runtime disabled: image4 interface not available\n");
		trust_cache_rt = NULL;
		trust_cache_mut_rt = NULL;
		return;
	}

	/* AMFI interface needs to be available */
	if (amfi == NULL) {
		printf("trust cache runtime disabled: amfi interface not available\n");
		trust_cache_rt = NULL;
		trust_cache_mut_rt = NULL;
		return;
	} else if (amfi->TrustCache.version < 2) {
		printf("trust cache runtime disabled: stale amfi interface: %u\n",
		    amfi->TrustCache.version);
		trust_cache_rt = NULL;
		trust_cache_mut_rt = NULL;
		return;
	}
