...
/*
 * Panthera: Define sysctl node without the linker set entry to avoid
 * "Use sysctl_register_oid_early" panic when built into the kernel.
 * Registered dynamically in _pthread_init() via sysctl_register_oid().
 */
static struct sysctl_oid sysctl__kern_pthread_mutex_default_policy = {
	.oid_parent = &sysctl__kern_children,
	.oid_number = OID_AUTO,
	.oid_kind = CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED | CTLFLAG_OID2,
	.oid_arg1 = &pthread_mutex_default_policy,
	.oid_arg2 = 0,
	.oid_name = "pthread_mutex_default_policy",
...
_bsdthread_create(struct proc *p,
		user_addr_t user_func, user_addr_t user_funcarg,
		...
		user_addr_t *retval)
{
	 ...
	 * Panthera: if bsdthread_register was never called, use user_func directly
	 * as the thread entry point instead of the _thread_start trampoline.
	 */
	user_addr_t threadstart_addr = pthread_kern->proc_get_threadstart(p);
#if defined(__i386__) || defined(__x86_64__)
	if (proc_is64bit_data(p)) {
		x86_thread_state64_t state;
		__builtin_memset(&state, 0, sizeof(state));
		state.rsp = (uint64_t)user_stack;
		state.rip = (uint64_t)threadstart_addr;
		/* Always use the _thread_start trampoline register layout */
		state.rdi = (uint64_t)user_pthread;
		state.rsi = (uint64_t)th_thport;
		state.rdx = (uint64_t)user_func;
		state.rcx = (uint64_t)user_funcarg;
		state.r8  = (uint64_t)user_stack;
		state.r9  = (uint64_t)flags;
		    ...
		    int32_t *retval)
{
	...
	/* Panthera: if pthsize is 0 (minimal bsdthread_register from dyld),
	 * supply defaults for LP64 based on libpthread-519's struct layout */
	if (pthsize == 0 && proc_is64bit(p)) {
		pthsize = 8192; /* MAX_PTHREAD_SIZE / 8 — generous */
		data.tsd_offset = 224; /* offsetof(struct pthread_s, tsd) */
	}
	...
	if (pthread_kern->proc_set_mach_thread_self_tsd_offset) {
		/* Panthera: default mach_thread_self to TSD slot 3 (offset 24) */
		if (data.mach_thread_self_offset == 0 && proc_is64bit(p)) {
			data.mach_thread_self_offset = 24; /* __TSD_MACH_THREAD_SELF * 8 */
		}
