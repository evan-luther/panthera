		...
		mach_port_name_t threadport, uint32_t *retval)
{
	...
	if (threadport != 0) {
		th = port_name_to_thread((mach_port_name_t)threadport, PORT_INTRANS_OPTIONS_NONE);
		if (th == THREAD_NULL) {
			return ESRCH;
		}
	}
		...
		thread_continue_t continuation, block_hint_t block_hint)
{
	...
	/* kwe_uth removed — uth is derived from kwe via container_of */
	kwe->kwe_thread = th;
		...
		uint32_t mgen, int fit)
{
	...
	if (kq->ksynq_count == 0) {
		...
		if (kwe->kwe_state == KWE_THREAD_PREPOST) {
			...
			if (tmp != NULL && ((void*)__container_of(tmp, struct uthread, uu_save.uus_kwe)) != NULL &&
					pthread_kern->uthread_is_cancelled(((void*)__container_of(tmp, struct uthread, uu_save.uus_kwe)))) {
				TAILQ_INSERT_TAIL(&kq->ksynq_kwelist, kwe, kwe_list);
				res = 0;
			}
		}
	...
	TAILQ_FOREACH_SAFE(q_kwe, &kq->ksynq_kwelist, kwe_list, r_kwe) {
		...
		if (q_kwe->kwe_state == KWE_THREAD_PREPOST |
			...
			if (q_kwe->kwe_kwqqueue == kwq) {
				if (!pthread_kern->uthread_is_cancelled((void*)__container_of(q_kwe, struct uthread, uu_save.uus_kwe))) {
...
retry:
	TAILQ_FOREACH_SAFE(kwe, &kq->ksynq_kwelist, kwe_list, newkwe) {
		...
		if (kwe->kwe_state == KWE_THREAD_INWAIT) {
			...
			if (!pthread_kern->uthread_is_cancelled(((void*)__container_of(kwe, struct uthread, uu_save.uus_kwe)))) {
