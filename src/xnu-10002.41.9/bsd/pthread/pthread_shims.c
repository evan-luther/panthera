...
/* Panthera: built-in pthread kext registration (no external kext needed) */
extern void pthread_kext_builtin_init(void);

void
pthread_init(void)
{
	if (!pthread_functions) {
		pthread_kext_builtin_init();
	}
	if (!pthread_functions) {
		printf("pthread: built-in registration failed\n");
		return;
	}
