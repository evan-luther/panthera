...
#ifndef VOUCHER_MACH_MSG_STATE_UNCHANGED
...
mach_msg_legacy_options_after_interruption(mach_msg_option_t option)
{
#ifdef MACH_RCV_SYNC_WAIT
	if ((option & MACH_SEND_MSG) && (option & MACH_RCV_MSG)) {
	...
	mach_msg_header_t *rcv_msg)
{
	...
	mach_msg_header_t *receive_msg = rcv_msg ? rcv_msg : msg;

	mr = mach_msg_overwrite_trap(msg, option & ~LIBMACH_OPTIONS, send_size,
	    rcv_limit, rcv_name, timeout, (mach_msg_priority_t)notify,
	    receive_msg, rcv_limit);

	if ((option & MACH_SEND_INTERRUPT) == 0) {
		while (mr == MACH_SEND_INTERRUPTED) {
			mr = mach_msg_overwrite_trap(msg, option & ~LIBMACH_OPTIONS,
			    send_size, rcv_limit, rcv_name, timeout,
			    (mach_msg_priority_t)notify, receive_msg, rcv_limit);
		}
	}
	...
	if ((option & MACH_RCV_INTERRUPT) == 0) {
		while (mr == MACH_RCV_INTERRUPTED) {
			mr = mach_msg_overwrite_trap(msg,
			    mach_msg_legacy_options_after_interruption(option), 0,
			    rcv_limit, rcv_name, timeout,
			    (mach_msg_priority_t)notify, receive_msg, rcv_limit);
		}
	}
	...
	__unused mach_msg_size_t rcv_scatter_size)
{
	...
	if (option64 & MACH64_MSG_VECTOR) {
		vecs[MACH_MSGV_IDX_MSG] = (mach_msg_vector_t){
			.msgv_data = (mach_vm_address_t)(uintptr_t)msg,
			.msgv_rcv_addr = (mach_vm_address_t)(uintptr_t)rcv_msg, /* if 0, just use msg as rcv address */
			.msgv_send_size = send_size,
			.msgv_rcv_size = rcv_limit,
		};
		vecs[MACH_MSGV_IDX_AUX] = (mach_msg_vector_t){
			.msgv_data = (mach_vm_address_t)(uintptr_t)aux,
			.msgv_rcv_addr = 0,
