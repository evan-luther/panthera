...
extern char * PE_boot_args(
	void);
extern void panthera_cache_boot_args(
...
#if XNU_KERNEL_PRIVATE
extern int panthera_debug_verbose;
#endif

#ifndef PANTHERA_TRACE
#if XNU_KERNEL_PRIVATE
#define PANTHERA_TRACE(stmt) \
	do { \
...
#define PANTHERA_TRACE(stmt) \
	do { \
		int __panthera_verbose = 0; \
		if (PE_parse_boot_argn("panthera_verbose", &__panthera_verbose, sizeof(__panthera_verbose)) && \
		    __panthera_verbose != 0) { \
			stmt; \
		} \
	} while (0)
#endif
