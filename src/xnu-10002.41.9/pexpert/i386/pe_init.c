...
extern void panthera_cache_boot_args(const char *command_line);
#if defined(__x86_64__)
extern void panthera_debugcon_write_public(const char *string);
#define PANTHERA_PEI_DBG(x) panthera_debugcon_write_public(x)
#else
#define PANTHERA_PEI_DBG(x) do { } while (0)
#endif
/* private globals */
PE_state_t  PE_state;
SECURITY_READ_ONLY_LATE(uint32_t) PE_srd_fused = 0;
int panthera_debug_verbose = 0;
...
static uint8_t            panthera_device_tree_copy[16384];
...
PE_init_iokit(void)
{
	...
	PANTHERA_PEI_DBG("PANTHERA:PEI enter\n");
	PE_init_printf(TRUE);
	PANTHERA_PEI_DBG("PANTHERA:PEI printf\n");

#if defined(__x86_64__)
	PANTHERA_PEI_DBG("PANTHERA:PEI bootargs_skip\n");
#else
...
#if defined(__x86_64__)
	PANTHERA_PEI_DBG("PANTHERA:PEI dt_bootgfx_skip\n");
#else
	if (kSuccess == SecureDTLookupEntry(NULL, "/chosen/memory-map", &entry)) {
		PANTHERA_PEI_DBG("PANTHERA:PEI chosen_memory_map\n");
		...
		if (kSuccess == SecureDTGetProperty(entry, "FailedImage", (void const **) &map, &size)) {
			...
			if (norootClut_lzss == NULL) {
				PANTHERA_PEI_DBG("PANTHERA:PEI missing_failedclut\n");
			}
		}
	} else {
		PANTHERA_PEI_DBG("PANTHERA:PEI chosen_memory_map_missing\n");
	}
#endif
	PANTHERA_PEI_DBG("PANTHERA:PEI dt_done\n");
	...
	PANTHERA_PEI_DBG("PANTHERA:PEI vc_progress_done\n");
	 ...
	 */
	PANTHERA_PEI_DBG("PANTHERA:PEI InitIOKit\n");
	InitIOKit(PE_state.deviceTreeHead);
	PANTHERA_PEI_DBG("PANTHERA:PEI InitIOKit done\n");
	PANTHERA_PEI_DBG("PANTHERA:PEI zalloc_iokit_lockdown\n");
	zalloc_iokit_lockdown();
	PANTHERA_PEI_DBG("PANTHERA:PEI zalloc_iokit_lockdown done\n");
	PANTHERA_PEI_DBG("PANTHERA:PEI StartIOKitMatching\n");
	StartIOKitMatching();
	PANTHERA_PEI_DBG("PANTHERA:PEI StartIOKitMatching done\n");
}
...
PE_init_platform(boolean_t vm_initialized, void * _args)
{
	...
	void *device_tree_head = (void *)ml_static_ptovirt(args->deviceTreeP);
	...
	if (PE_state.initialized == FALSE) {
		...
		panthera_cache_boot_args(args->CommandLine);
		panthera_debug_verbose = 0;
		(void) PE_parse_boot_argn("panthera_verbose", &panthera_debug_verbose, sizeof(panthera_debug_verbose));
		if (args->deviceTreeLength != 0 &&
		    args->deviceTreeLength <= sizeof(panthera_device_tree_copy) &&
		    device_tree_head != NULL) {
			bcopy(device_tree_head, panthera_device_tree_copy, args->deviceTreeLength);
			PE_state.deviceTreeHead = panthera_device_tree_copy;
		} else {
			PE_state.deviceTreeHead = device_tree_head;
		}
		PE_state.deviceTreeSize     = args->deviceTreeLength;
		PANTHERA_KPRINTF("PANTHERA:PE video baseAddr=0x%llx w=%d h=%d d=%d disp=%d\n",
		    args->Video.v_baseAddr, args->Video.v_width,
		    args->Video.v_height, args->Video.v_depth, args->Video.v_display);
...
{
	PANTHERA_TRACE(printf("PANTHERA:PE_create_console v_display=%d v_baseAddr=0x%llx w=%d h=%d d=%d\n",
	    PE_state.video.v_display,
	    PE_state.video.v_baseAddr,
...
PE_i_can_has_debugger(uint32_t *debug_flags)
{
	...
	panthera_debugcon_puts("PANTHERA:PE_DEBUGGER enter\n");
...
#if CONFIG_CSR
	panthera_debugcon_puts("PANTHERA:PE_DEBUGGER csr_check\n");
	if (csr_check(CSR_ALLOW_KERNEL_DEBUGGER) != 0) {
		panthera_debugcon_puts("PANTHERA:PE_DEBUGGER denied\n");
		if (debug_flags) {
			*debug_flags = 0;
		}
		return FALSE;
	...
	panthera_debugcon_puts("PANTHERA:PE_DEBUGGER allowed\n");
	return TRUE;
}
