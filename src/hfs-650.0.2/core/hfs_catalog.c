...
static void buildthreadkey(HFSCatalogNodeID parentID, CatalogKey *key);

static void buildrecord(struct cat_attr *attrp, cnid_t cnid, u_int32_t encoding, CatalogRecord *crp, u_int32_t *recordSize);
...
static int buildthread(void *keyp, void *recp, int directory);
...
cat_insertfilethread(struct hfsmount *hfsmp, struct cat_desc *descp)
{
	...
	if ((file_rec.flags & kHFSThreadExistsMask) == 0) {
		...
		thread_data.itemSize = buildthread(&iterator[0].key, &thread_rec, 0);
		buildthreadkey(file_rec.fileID, (CatalogKey *)&iterator[1].key);
	...
	struct cat_desc *out_descp)
{
	 ...
	 */
	datalen = buildthread((void*)&bto->key, &bto->data, S_ISDIR(attrp->ca_mode));
	...
	/* Caller asserts the following:
		 ...
		 */		 
	buildthreadkey(new_fileid, (CatalogKey *) &bto->iterator.key);
	 ...
	 */
	buildrecord(attrp, new_fileid, encoding, &bto->data, &datalen);
	btdata.bufferAddress = &bto->data;
	btdata.itemSize = datalen;
	btdata.itemCount = 1;
	...
	struct cat_desc * out_cdp )
{
	 ...
	 *
	 */
	if (hfskeycompare(to_key, iter->key) == 0)
		goto exit;	
	if (hfspluskeycompare(to_key, iter->key) == 0)
		goto exit;	
#endif
...
catrec_update(const CatalogKey *ckp, CatalogRecord *crp, struct update_state *state)
{
	...
	case kHFSPlusFolderRecord: {
		...
		bcopy(&attrp->ca_finderinfo[0], &dir->userInfo, sizeof(dir->userInfo));
		bcopy(&attrp->ca_finderinfo[0] + sizeof(dir->userInfo), &dir->finderInfo, sizeof(dir->finderInfo));
		/*
	...
	case kHFSPlusFileRecord: {
		...
		bcopy(&attrp->ca_finderinfo[0], &file->userInfo, sizeof(file->userInfo));
		bcopy(&attrp->ca_finderinfo[0] + sizeof(file->userInfo), &file->finderInfo, sizeof(file->finderInfo));
		/*
		 * Update the BSD Info if it was already initialized on
		 * disk or if the runtime values have been modified.
		 *
 ...
 */
static void
buildrecord(struct cat_attr *attrp, cnid_t cnid, u_int32_t encoding,
            CatalogRecord *crp, u_int32_t *recordSize)
{
	...
	if (type == S_IFDIR) {
		...
		bcopy(attrp->ca_finderinfo, &crp->hfsPlusFolder.userInfo, sizeof(crp->hfsPlusFolder.userInfo));
		bcopy(attrp->ca_finderinfo + sizeof(crp->hfsPlusFolder.userInfo), &crp->hfsPlusFolder.finderInfo, sizeof(crp->hfsPlusFolder.finderInfo));
		...
		*recordSize = sizeof(HFSPlusCatalogFolder);
		...
		bcopy(attrp->ca_finderinfo, &crp->hfsPlusFile.userInfo, sizeof(crp->hfsPlusFile.userInfo));
		bcopy(attrp->ca_finderinfo + sizeof(crp->hfsPlusFile.userInfo), &crp->hfsPlusFile.finderInfo, sizeof(crp->hfsPlusFile.finderInfo));
