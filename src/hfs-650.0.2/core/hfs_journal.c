...
size_up_tbuffer(journal *jnl, int tbuffer_size, int phys_blksz)
{
	...
	if (def_tbuffer_size == 0) {
		...
		/*
		 * Panthera does not expose the late sysctl-based memory query that
		 * Apple uses here for journal buffer sizing. Falling back to the
		 * default sizing keeps HFS bring-up deterministic and is sufficient
		 * for early root-volume boot.
		 */
