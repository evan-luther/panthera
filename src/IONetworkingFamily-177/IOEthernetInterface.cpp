...
IOReturn IOEthernetInterface::enableController(IONetworkController * ctr)
{
    ...
    do {
        ...
        {
            IOLog("PANTHERA:NETIO enableController doEnable fail if=%s ret=0x%x\n",
                  getName(), ret);
        ...
        // Panthera bring-up does not use Wake-On-LAN. Avoid mutating WOL
        // filters while bringing the controller online; this path can block
        // the minimal boot network probe before address assignment.

        // Restore current filter selection.

        // Unicast and broadcast are always required for Ethernet and the
        // default controller implementation treats them as a no-op. Mark the
        // defaults active directly during Panthera bring-up; explicit filter
        // ioctls still go through enableFilter()/disableFilter().
        SET_ACTIVE_FILTERS(gIONetworkFilterGroup,
        ...
        // Wake-On-LAN publication is deferred during Panthera bring-up.
    ...
    {
        IOLog("PANTHERA:NETIO enableController disabling after error if=%s ret=0x%x\n",
              getName(), ret);
        ctr->doDisable(this);
    }
...
int IOEthernetInterface::syncSIOCSIFFLAGS(IONetworkController * ctr)
{
         ...
         ( flags & ( IFF_PROMISC | IFF_ALLMULTI ) ) )
    {
        ...
        if ( flags & IFF_PROMISC )
            rc = enableFilter(ctr, gIONetworkFilterGroup,
                              kIOPacketFilterPromiscuous);
        else if ( GET_ACTIVE_FILTERS(gIONetworkFilterGroup) &
                  kIOPacketFilterPromiscuous )
            rc = disableFilter(ctr, gIONetworkFilterGroup,
                               kIOPacketFilterPromiscuous);
        else
            rc = kIOReturnSuccess;
...
SInt IOEthernetInterface::syncSIOCSIFADDR(IONetworkController * ctr)
{
    ...
    // Interface is implicitly brought up by the BSD SIOCSIFADDR path before
    // it reaches this callback. Avoid calling ifnet_set_flags() again from
    // inside the ioctl path; Panthera's minimal networking bring-up can
    // otherwise re-enter flag handling before the address ioctl returns.

    if ( (getFlags() & IFF_RUNNING) == 0 )
    {
        if ( (ret = enableController(ctr)) == kIOReturnSuccess )
            setFlags(IFF_RUNNING, IFF_RUNNING);
    }
