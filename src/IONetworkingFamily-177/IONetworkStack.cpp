    ...
    IONotifier *    notifier __unused )
{
    ...
    else if (isNew && autoRegister)
    {
        IOReturn result = registerNetworkInterface(netif, 0, false);
        PANTHERA_NET_TRACE("PANTHERA:NET auto-register %s result=0x%x\n",
            netif->getName(), result);
    }
    ...
    bool *                  attachToBSD )
{
    ...
    *attachToBSD = false;
    ...
    do {
        ...
        PANTHERA_NET_TRACE("PANTHERA:NET reserved interface %s%u for %s\n",
            prefix, unit, netif->getName());
...
IOReturn IONetworkStack::attachNetworkInterfaceToBSD( IONetworkInterface * netif )
{
    ...
    PANTHERA_NET_TRACE("PANTHERA:NET assigning BSD name %s to %s\n",
    ...
    if ((result = netif->attachToDataLinkLayer(0, 0)) == kIOReturnSuccess)
    {
        ...
        PANTHERA_NET_TRACE("PANTHERA:NET BSD attach success %s ifnet=%p\n",
            ifname, netif->getIfnet());
    ...
    {
        PANTHERA_NET_TRACE("PANTHERA:NET BSD attach failed %s result=0x%x\n",
            ifname, result);
    }
    ...
    else
    {
        ...
        PANTHERA_NET_TRACE("PANTHERA:NET interface abandoned %s state=0x%x\n",
            ifname, (uint32_t) NIF_VAR(netif));
        UNLOCK();
    }
