...
#define PANTHERA_NET_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
                                          ...
                                          void *                arg1)
{
    ...
    switch ( cmd )
    {
        ...
        case SIOCSIFFLAGS:
            ...
            {
                status = ctr->doEnable(this);
                PANTHERA_NET_TRACE("PANTHERA:NET ioctl SIOCSIFFLAGS up if=%s status=0x%x\n",
                                   getName(), status);
            }
            else
            {
                status = ctr->doDisable(this);
                PANTHERA_NET_TRACE("PANTHERA:NET ioctl SIOCSIFFLAGS down if=%s status=0x%x\n",
                                   getName(), status);
            }
            ret = (status == kIOReturnSuccess) ? 0 : EIO;
            break;
...
int IONetworkInterface::if_output( ifnet_t ifp, mbuf_t m )
{
	...
	int         result;
	static UInt32 panthera_if_output_trace_count = 0;
	bool        trace_this;

	IONetworkInterface * self = IFNET_TO_THIS(ifp);
    
    assert(ifp == self->_backingIfnet);
	trace_this = panthera_if_output_trace_count++ < 16;
	...
	if (trace_this) {
		PANTHERA_NET_TRACE("PANTHERA:NET if_output if=%s len=%u target=%p\n",
		    self->getName(), (unsigned int)mbuf_pkthdr_len(m), self->_outTarget);
	}
	result = ((self->_outTarget)->*(self->_outAction))(m, 0);
	if (trace_this) {
		PANTHERA_NET_TRACE("PANTHERA:NET if_output if=%s result=%d\n",
		    self->getName(), result);
	}
	return result;
}
                                                    ...
                                                    void *       parameter )
{
    ...
    if (!_driver)
    {
        PANTHERA_NET_TRACE("PANTHERA:NET %s BSD attach failed: no driver\n", getName());
    ...
    {
        PANTHERA_NET_TRACE("PANTHERA:NET %s initIfnetParams failed\n", getName());
    ...
    if (error)
    {
        PANTHERA_NET_TRACE("PANTHERA:NET %s ifnet_allocate_extended error=%d\n",
            getName(), error);
    ...
    if (prop)
    {
        ...
        if (!ll_addr)
        {
            PANTHERA_NET_TRACE("PANTHERA:NET %s BSD attach failed: bad MAC address\n",
                getName());
    ...
    if (!error) {
        ...
        PANTHERA_NET_TRACE("PANTHERA:NET %s ifnet_attach ok ifnet=%p\n",
            getName(), _backingIfnet);
        /*
         * Panthera's minimal boot path configures BSD networking directly and
         * does not yet have the normal user-space IOKit client open that
         * enables legacy controllers. Bring the controller up once the ifnet
         * is attached so the driver's transmit queue can drain.
         */
        if (_driver)
        {
            IOReturn enableResult = _driver->doEnable(this);
            PANTHERA_NET_TRACE("PANTHERA:NET %s attach enable status=0x%x\n",
                               getName(), enableResult);
        }
    } else
    {
        PANTHERA_NET_TRACE("PANTHERA:NET %s ifnet_attach error=%d\n", getName(), error);
