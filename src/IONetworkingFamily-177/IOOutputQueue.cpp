...
#define PANTHERA_NETQ_TRACE(...) PANTHERA_TRACE(IOLog(__VA_ARGS__))
...
UInt32 IOBasicOutputQueue::enqueue(mbuf_t m, void * param)
{
    bool success;
    bool running;
    UInt32 state_snapshot;
    UInt32 queued_snapshot;
    static UInt32 panthera_enqueue_trace_count = 0;
    bool trace_this = panthera_enqueue_trace_count++ < 16;
    ...
    running = STATE_IS( kStateRunning );
    state_snapshot = _state;
    queued_snapshot = IOMbufQueueGetSize(&(_inQueues[priority]));
    if (trace_this) {
        PANTHERA_NETQ_TRACE("PANTHERA:NET txq enqueue q=%p len=%u pri=%u ok=%u state=0x%x running=%u queued=%u\n",
                            this, m ? (unsigned int)mbuf_pkthdr_len(m) : 0, priority,
                            success ? 1U : 0U, state_snapshot, running ? 1U : 0U,
                            queued_snapshot);
    }
    ...
    if ( running )
    {
        STATE_SET( kStateOutputActive );
        if (trace_this) {
            PANTHERA_NETQ_TRACE("PANTHERA:NET txq enqueue q=%p dequeue_now state=0x%x\n",
                                this, _state);
        }
        dequeue();
    }
    else if (trace_this) {
        PANTHERA_NETQ_TRACE("PANTHERA:NET txq enqueue q=%p held state=0x%x\n",
                            this, _state);
    }
...
void IOBasicOutputQueue::output(IOMbufQueue * queue, UInt32 * state)
{
    mbuf_t pkt;
    UInt32        status;
    static UInt32 panthera_output_trace_count = 0;
    bool trace_this;

    do {
        pkt = IOMbufQueueDequeue(queue);
        assert(pkt);
        trace_this = panthera_output_trace_count++ < 16;
        if (trace_this) {
            IOLog("PANTHERA:NET txq output q=%p pkt=%p len=%u target=%p\n",
                  this, pkt, (unsigned int)mbuf_pkthdr_len(pkt), _target);
        }
        ...
        if (trace_this) {
            IOLog("PANTHERA:NET txq output q=%p status=0x%x\n",
                  this, status);
        }
...
bool IOBasicOutputQueue::start()
{
    ...
    _serviceCount++;
    PANTHERA_NETQ_TRACE("PANTHERA:NET txq start q=%p state=0x%x service=%u\n",
                        this, _state, _serviceCount);
...
bool IOBasicOutputQueue::stop()
{
    ...
    PANTHERA_NETQ_TRACE("PANTHERA:NET txq stop q=%p was=%u state=0x%x\n",
                        this, wasRunning ? 1U : 0U, _state);
                                     ...
                                     UInt32 *             state)
{
    mbuf_t pkt;
    UInt32        status;
    static UInt32 panthera_gated_trace_count = 0;
    bool trace_this;

    do {
        pkt = IOMbufQueueDequeue(queue);
        assert(pkt);
        trace_this = panthera_gated_trace_count++ < 16;
        if (trace_this) {
            PANTHERA_NETQ_TRACE("PANTHERA:NET gated output q=%p pkt=%p len=%u target=%p\n",
                                self, pkt, (unsigned int)mbuf_pkthdr_len(pkt), self->_target);
        }
        ...
        if (trace_this) {
            PANTHERA_NETQ_TRACE("PANTHERA:NET gated output q=%p status=0x%x state=0x%x\n",
                                self, status, state ? *state : 0);
        }
...
{
    IOReturn ret;
    static UInt32 panthera_gate_attempt_trace_count = 0;
    bool trace_this = panthera_gate_attempt_trace_count++ < 16;

    ret = _gate->attemptAction((IOCommandGate::Action)
                               &IOGatedOutputQueue::gatedOutput,
                               (void *) this,
                               (void *) queue,
                               (void *) state);
    if (trace_this) {
        PANTHERA_NETQ_TRACE("PANTHERA:NET gated attempt q=%p ret=0x%x state=0x%x\n",
                            this, ret, state ? *state : 0);
    }
...
{
    PANTHERA_NETQ_TRACE("PANTHERA:NET gated schedule q=%p param=0x%lx\n",
                        this, (unsigned long)(uintptr_t)param);
