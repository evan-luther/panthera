...
#if PANTHERA
static int panthera_launchd_trace_enabled = -1;

static void
panthera_launchd_puts(const char *s)
{
	size_t len = 0;
	while (s[len] != '\0') {
		len++;
...
static void
panthera_launchd_init_trace_flag(void)
{
	if (panthera_launchd_trace_enabled == -1) {
		panthera_launchd_trace_enabled =
		    (getenv("PANTHERA_LAUNCHD_TRACE") != NULL) ? 1 : 0;
	}
}

static const char *
panthera_job_exec_hint(job_t j)
{
	if (j->prog) {
		return j->prog;
	}
...
static bool
panthera_launchd_should_trace_job(job_t j)
{
	(void)j;
	return panthera_launchd_trace_enabled == 1;
}

static void
panthera_launchd_trace_job(job_t j, const char *event)
{
	char msg[512];
	if (!panthera_launchd_should_trace_job(j)) {
		return;
	}
	snprintf(msg, sizeof(msg),
	    "PANTHERA:launchd %s label=%s exec=%s pid=%d\n",
	    event,
	    j->label,
	    panthera_job_exec_hint(j),
	    j->p);
	panthera_launchd_puts(msg);
}

static void
panthera_launchd_trace_exec(job_t j, const char *event, const char *path, int error)
{
	char msg[512];
	if (!panthera_launchd_should_trace_job(j)) {
		return;
	...
	snprintf(msg, sizeof(msg),
	    "PANTHERA:launchd %s label=%s exec=%s errno=%d\n",
	    event, j->label, path ? path : "(null)", error);
	panthera_launchd_puts(msg);
}

static void
panthera_launchd_trace_fd(job_t j, const char *event, int target_fd, const char *path, int error)
{
	char msg[512];
	if (!panthera_launchd_should_trace_job(j)) {
		return;
	...
	snprintf(msg, sizeof(msg),
	    "PANTHERA:launchd %s label=%s target=%d path=%s errno=%d\n",
	    event, j->label, target_fd, path ? path : "(null)", error);
	panthera_launchd_puts(msg);
}

static void
panthera_launchd_trace_reap(job_t j, int wait_result, int wait_error)
{
	char msg[512];
	int status = j ? j->last_exit_status : 0;
	if (!panthera_launchd_should_trace_job(j)) {
		return;
	}

	snprintf(msg, sizeof(msg),
	    "PANTHERA:launchd reap label=%s exec=%s pid=%d wait=%d errno=%d raw=0x%x exited=%d exit=%d signaled=%d signal=%d did_exec=%d\n",
	    (j && j->label) ? j->label : "(null)",
	    (j && j->prog) ? j->prog : ((j && j->argv && j->argv[0]) ? j->argv[0] : "(null)"),
	    j ? j->p : 0,
	    wait_result,
	    wait_error,
	    status,
...
#if PANTHERA
job_t
panthera_job_import_deferred(launch_data_t pload)
{
	job_t j = jobmgr_import2(root_jobmgr, pload);
...
job_reap(job_t j)
{
	...
	#ifdef PANTHERA
					panthera_launchd_trace_reap(j, r, wait_error);
	#endif
...
#if PANTHERA
static bool
panthera_job_label_is(job_t j, const char *label)
{
	return (j != NULL && j->label != NULL && strcmp(j->label, label) == 0);
}

static void
panthera_dispatch_job_label(jobmgr_t jm, const char *label)
{
	jobmgr_t jmi, jmn;
	job_t ji, jn;

	SLIST_FOREACH_SAFE(jmi, &jm->submgrs, sle, jmn) {
		panthera_dispatch_job_label(jmi, label);
	}

	LIST_FOREACH_SAFE(ji, &jm->jobs, sle, jn) {
		if (panthera_job_label_is(ji, label)) {
			job_dispatch(ji, false);
		}
	}
}
...
static void
panthera_dispatch_all_except_label(jobmgr_t jm, const char *label)
{
	jobmgr_t jmi, jmn;
	job_t ji, jn;
	...
	SLIST_FOREACH_SAFE(jmi, &jm->submgrs, sle, jmn) {
		panthera_dispatch_all_except_label(jmi, label);
	}
...
void
panthera_dispatch_loaded_jobs(void)
{
	if (!pid1_magic || root_jobmgr == NULL) {
		return;
	}

	panthera_launchd_init_trace_flag();
	panthera_launchd_puts("PANTHERA:launchd dispatch imported boot jobs\n");
	panthera_dispatch_job_label(root_jobmgr, "com.apple.configd");
	panthera_dispatch_all_except_label(root_jobmgr, "com.apple.configd");
}
#endif
...
#if PANTHERA
	if (kickstart) {
		panthera_launchd_trace_job(j, "dispatch-kickstart");
	}
#endif
...
jobmgr_callback(void *obj, struct kevent *kev)
{
	...
	case EVFILT_SIGNAL:
		...
		case SIGUSR1:
			...
			if (!network_up) {
				network_up = true;
				jobmgr_log(jm, LOG_DEBUG | LOG_CONSOLE,
				    "PANTHERA:launchd NetworkState up");
...
job_start(job_t j)
{
...
#if PANTHERA
	panthera_launchd_trace_job(j, "start-enter");
#endif

	if (unlikely(job_active(j))) {
#if PANTHERA
		panthera_launchd_trace_job(j, "start-active-return");
#endif
	...
	td /= NSEC_PER_SEC;
#if PANTHERA
	panthera_launchd_trace_job(j, "start-after-time");
#endif
...
#if PANTHERA
	panthera_launchd_trace_job(j, sipc ? "start-sipc" : "start-no-sipc");
#endif

	if (sipc) {
#if PANTHERA
		panthera_launchd_trace_job(j, "start-socketpair-sipc-enter");
#endif
		(void)job_assumes_zero_p(j, socketpair(AF_UNIX, SOCK_STREAM, 0, spair));
#if PANTHERA
		panthera_launchd_trace_job(j, "start-socketpair-sipc-done");
#endif
	}

#if PANTHERA
	panthera_launchd_trace_job(j, "start-socketpair-exec-enter");
#endif
	(void)job_assumes_zero_p(j, socketpair(AF_UNIX, SOCK_STREAM, 0, execspair));
#if PANTHERA
	panthera_launchd_trace_job(j, "start-socketpair-exec-done");
	panthera_launchd_trace_job(j, "start-runtime-fork-enter");
#endif
	...
	case 0:
#if PANTHERA
		/* Panthera: skip _vproc_post_fork_ping(). It sends a synchronous MIG
		 * message back to launchd, but during panthera_load_jobs() the main
		 * thread is not yet in the mach_msg receive loop, so the child would
		 * deadlock waiting for a reply. The ping is only needed for audit
		 * session and special port setup, which Panthera does not use. */
#else
...
#if PANTHERA
		panthera_launchd_trace_job(j, "child-wait-uncork");
#endif
		read(_fd(execspair[1]), &c, sizeof(c));
#if PANTHERA
		panthera_launchd_trace_job(j, "child-after-uncork");
#endif
	...
	default:
		...
		(void)job_assumes_zero(j, runtime_close(execspair[1]));
#if PANTHERA
		panthera_launchd_trace_job(j, "forked");
#endif
		if (sipc) {
			(void)job_assumes_zero(j, runtime_close(spair[1]));
...
#if PANTHERA
			panthera_launchd_trace_job(j, "uncork-before");
#endif
			job_uncork_fork(j);
		}
		break;
...
job_start_child(job_t j)
{
...
#if PANTHERA
	panthera_launchd_trace_job(j, "child-start");
#endif
	job_setup_attributes(j);
#if PANTHERA
	panthera_launchd_trace_job(j, "child-after-attributes");
#endif

	bool use_xpcproxy = false;
	struct waiting4attach *w4a = NULL;
#if PANTHERA
	panthera_launchd_trace_job(j, "child-waiting4attach-enter");
#endif
	if (!LIST_EMPTY(&j->mgr->attaches)) {
#if PANTHERA
		panthera_launchd_trace_job(j, "child-waiting4attach-scan");
#endif
		w4a = waiting4attach_find(j->mgr, j);
	}
#if PANTHERA
	panthera_launchd_trace_job(j, "child-waiting4attach-done");
#endif
	if (w4a) {
...
#if PANTHERA
	panthera_launchd_trace_job(j, "child-argv-enter");
#endif
	if (use_xpcproxy) {
...
#if PANTHERA
	panthera_launchd_trace_job(j, "child-argv-done");
#endif
	...
	/* Panthera executes directly after fork.  The spawn attribute setup below
	 * exists for Darwin's posix_spawn(SETEXEC) path and is not consumed by
	 * execve(), so running it here only exposes Panthera to unsupported spawn
	 * policy calls before the real exec. */
	panthera_launchd_trace_exec(j, "execve-enter", file2exec, 0);
	execve(file2exec, (char *const *)argv, environ);
	panthera_launchd_trace_exec(j, "execve-failed", file2exec, errno);
	_exit(errno);
#endif
...
#if !TARGET_OS_EMBEDDED
	{
		...
		if (!j->legacy_timers) {
			kern_return_t qkr = task_policy_set(mach_task_self(), TASK_BASE_QOS_POLICY, (task_policy_t)&qosinfo, TASK_QOS_POLICY_COUNT);
			(void)job_assumes_zero_p(j, qkr);
		}
	}
#endif
...
#if PANTHERA
	/* Panthera: use plain execve() instead of posix_spawn with
	 * POSIX_SPAWN_SETEXEC + complex spawn attributes. Panthera's XNU
	 * does not support posix_spawnattr_setprocesstype_np,
	 * posix_spawnattr_setcpumonitor_default, or the task QoS
	 * spawn attributes, and rejects the spawn with ENAMETOOLONG. */
	(void)posix_spawnattr_destroy(&spattr);
	panthera_launchd_trace_exec(j, "execve-enter", file2exec, 0);
	execve(file2exec, (char *const *)argv, environ);
	panthera_launchd_trace_exec(j, "execve-failed", file2exec, errno);
#else
...
job_setup_attributes(job_t j)
{
...
#if PANTHERA
	panthera_launchd_trace_job(j, "attrs-enter");
#endif
...
#if PANTHERA
	panthera_launchd_trace_job(j, "attrs-after-user");
#endif
	...
	if (j->stdin_fd) {
#if PANTHERA
		panthera_launchd_trace_fd(j, "attrs-stdin-dup-enter", STDIN_FILENO, "<stdin_fd>", 0);
#endif
		(void)job_assumes_zero_p(j, dup2(j->stdin_fd, STDIN_FILENO));
#if PANTHERA
		panthera_launchd_trace_fd(j, "attrs-stdin-dup-return", STDIN_FILENO, "<stdin_fd>", errno);
#endif
	} else {
#if PANTHERA
		panthera_launchd_trace_fd(j, "attrs-stdin-path-enter", STDIN_FILENO,
		    j->stdinpath ? j->stdinpath : "<null>", 0);
#endif
		job_setup_fd(j, STDIN_FILENO, j->stdinpath, O_RDONLY|O_CREAT);
#if PANTHERA
		panthera_launchd_trace_fd(j, "attrs-stdin-path-return", STDIN_FILENO,
		    j->stdinpath ? j->stdinpath : "<null>", errno);
#endif
	}
#if PANTHERA
	panthera_launchd_trace_fd(j, "attrs-stdout-path-enter", STDOUT_FILENO,
	    j->stdoutpath ? j->stdoutpath : "<null>", 0);
#endif
	job_setup_fd(j, STDOUT_FILENO, j->stdoutpath, O_WRONLY|O_CREAT|O_APPEND);
#if PANTHERA
	panthera_launchd_trace_fd(j, "attrs-stdout-path-return", STDOUT_FILENO,
	    j->stdoutpath ? j->stdoutpath : "<null>", errno);
	panthera_launchd_trace_fd(j, "attrs-stderr-path-enter", STDERR_FILENO,
	    j->stderrpath ? j->stderrpath : "<null>", 0);
#endif
	job_setup_fd(j, STDERR_FILENO, j->stderrpath, O_WRONLY|O_CREAT|O_APPEND);
#if PANTHERA
	panthera_launchd_trace_fd(j, "attrs-stderr-path-return", STDERR_FILENO,
	    j->stderrpath ? j->stderrpath : "<null>", errno);
#endif
#if PANTHERA
	panthera_launchd_trace_job(j, "attrs-after-fd");
#endif
...
#if PANTHERA
	panthera_launchd_trace_job(j, "attrs-after-env");
#endif
...
#if PANTHERA
	panthera_launchd_trace_job(j, "attrs-done");
#endif
}
...
#if PANTHERA
	panthera_launchd_trace_fd(j, "setup-fd-open-enter", target_fd, path, 0);
#endif
	if ((fd = open(path, flags|O_NOCTTY, DEFFILEMODE)) == -1) {
#if PANTHERA
		panthera_launchd_trace_fd(j, "setup-fd-open-failed", target_fd, path, errno);
#endif
		job_log_error(j, LOG_WARNING, "open(\"%s\", ...)", path);
		return;
	}
#if PANTHERA
	panthera_launchd_trace_fd(j, "setup-fd-open-ok", target_fd, path, 0);
#endif

	(void)job_assumes_zero_p(j, dup2(fd, target_fd));
#if PANTHERA
	panthera_launchd_trace_fd(j, "setup-fd-dup-ok", target_fd, path, 0);
#endif
...
{
#if PANTHERA
	/* Panthera does not run a functional Mach exception server yet.
	 * Installing crash exception ports causes noisy EXC_CORPSE_NOTIFY
	 * delivery failures for otherwise healthy daemon exits. */
	(void)j;
	(void)target_task;
	return;
#else
...
#if PANTHERA
	panthera_launchd_trace_job(j, "uncork");
#endif
...
#if PANTHERA
	/* Panthera: proc_bsdinfo not fully defined, use pid only */
	(void)snprintf(jm->name_init, NAME_MAX, "com.apple.xpc.domain.pid.%d", ldc->pid);
	(void)snprintf(jm->owner, sizeof(jm->owner), "pid%d", ldc->pid);
#else
...
jobmgr_init(bool sflag)
{
	...
	os_assert((root_jobmgr = jobmgr_new(NULL, MACH_PORT_NULL, MACH_PORT_NULL, sflag, root_session_type, skip_init, MACH_PORT_NULL)) != NULL);
#if PANTHERA
	if (pid1_magic) {
		root_jobmgr->session_initialized = true;
	}
#endif
