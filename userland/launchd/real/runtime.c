...
#if PANTHERA
	/* Panthera: do_file_init() skipped for PID 1, so do timebase init here.
	 * This is safe because launchd_runtime_init() is called from main()
	 * after dyld has finished initialization. */
	if (pid1_magic && tbi.numer == 0) {
...
{
	/* Panthera: panthera_boot_init() called from main(), before we get here */
	launchd_runtime2(max_msg_size);
	/* launchd_runtime2 is an infinite loop — should never return.
	 * If it does, sleep forever to prevent PID 1 exit. */
	for (;;) sleep(60);
}
...
runtime_fork(mach_port_t bsport)
{
	...
	__OS_COMPILETIME_ASSERT__(SIG_ERR == (typeof(SIG_ERR))-1);
	/*
	 * Invariant: The parent (launchd / PID 1) must never set ignored signal
	 * dispositions to SIG_DFL. Doing so in the parent creates a race window
	 * where a signal (such as SIGUSR1) arriving during job spawn terminates
	 * PID 1. Block the ignored-signal set across fork so the child inherits
	 * a blocked mask, and reset dispositions to SIG_DFL exclusively in the
	 * child while blocked before clearing the child's mask.
	 */
...
#if PANTHERA
	/* Panthera: skip constructor for PID 1. The shared cache may not
	 * be mappable yet (uid 501), and calling library functions here
	 * would crash. We do timebase init in launchd_runtime_init(). */
	if (getpid() == 1) {
		pid1_magic = true;
		return;
	}
