...
// Panthera shim: wraps Apple's unwind-cxx.h to fix typedef conflict with modern SDK.
// The SDK's unwind.h already defines _Unwind_Exception_Class as uint64_t.
// We define the other GCC-specific types the source needs, then include the
// rest of the original header via a trampoline.
...
// The SDK's unwind.h defines _Unwind_Exception_Class as uint64_t.
// Define the GCC-specific types that the SDK does NOT provide.
typedef unsigned _Unwind_Word __attribute__((__mode__(__word__)));
typedef signed   _Unwind_Sword __attribute__((__mode__(__word__)));
typedef unsigned _Unwind_Ptr __attribute__((__mode__(__pointer__)));
typedef unsigned _Unwind_Internal_Ptr __attribute__((__mode__(__pointer__)));
// _Unwind_Exception_Class: already defined by <unwind.h> as uint64_t — do not redefine
