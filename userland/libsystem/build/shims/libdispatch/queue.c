...
#ifndef PTHREAD_WQ_QUANTUM_EXPIRY_NARROW
#define PTHREAD_WQ_QUANTUM_EXPIRY_NARROW 0
#endif
...
_dispatch_runloop_root_queue_create_4CF(const char *label, unsigned long flags)
{
	...
	dispatch_queue_t defq = _dispatch_get_default_queue(true);
	dq->do_targetq = defq;
...
{
	_dispatch_root_queues_init();
#if HAVE_PTHREAD_MAIN_NP && PANTHERA_LIBDISPATCH_ENABLE_WORKGROUP_OBJECTS != 0
	if (pthread_main_np()) {
#endif
...
#pragma clang diagnostic pop
	...
	if (r != 0) {
#if defined(PANTHERA_LIBDISPATCH_ENABLE_WORKGROUP_OBJECTS) && \
		PANTHERA_LIBDISPATCH_ENABLE_WORKGROUP_OBJECTS == 0
#else
...
#if defined(__APPLE__) && defined(SYS_thread_selfid)
DISPATCH_ALWAYS_INLINE
static inline pid_t
_gettid(void)
{
	// XNU's SYS_gettid (286) is gettid(uid_t*, gid_t*) — NOT a thread ID syscall.
	// thread_selfid (syscall 372) returns the Mach thread ID as uint64_t.
...
#else
#error "No thread ID mechanism available"
#endif
