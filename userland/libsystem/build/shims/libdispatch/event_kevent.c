...
#ifndef PANTHERA_LIBDISPATCH_TRACE
#define PANTHERA_LIBDISPATCH_TRACE 0
#endif
static inline void
_panthera_kevent_raw_write(const void *buf, size_t len)
{
	if (!buf || len == 0) return;
...
static inline void
_panthera_kevent_ckpt(const char *msg)
{
	if (!msg) return;
	size_t len = 0;
	while (msg[len]) len++;
	_panthera_kevent_raw_write(msg, len);
}

static inline void
_panthera_kevent_ckpt_int_hex(const char *p1, long long v1, const char *p2, uintptr_t v2, const char *suffix)
{
	char buf[256];
	int pos = 0;
	if (p1) {
		while (*p1 && pos < (int)sizeof(buf) - 64) {
			buf[pos++] = *p1++;
		}
	}
	char num[32];
	int numpos = 0;
	unsigned long long uval = (v1 < 0) ? (0ULL - (unsigned long long)v1) : (unsigned long long)v1;
	if (uval == 0) {
		num[numpos++] = '0';
	} else {
		while (uval > 0 && numpos < (int)sizeof(num)) {
			num[numpos++] = '0' + (char)(uval % 10);
			uval /= 10;
		}
	}
	if (v1 < 0 && pos < (int)sizeof(buf) - 32) buf[pos++] = '-';
	while (numpos > 0 && pos < (int)sizeof(buf) - 32) {
		buf[pos++] = num[--numpos];
	}
	if (p2) {
		while (*p2 && pos < (int)sizeof(buf) - 32) {
			buf[pos++] = *p2++;
		}
	}
	...
	buf[pos++] = '0';
	buf[pos++] = 'x';
	for (int i = (int)(sizeof(uintptr_t) * 2) - 1; i >= 0 && pos < (int)sizeof(buf) - 2; i--) {
		buf[pos++] = hex[(v2 >> (i * 4)) & 0xf];
	}
	if (suffix) {
		while (*suffix && pos < (int)sizeof(buf) - 1) {
			buf[pos++] = *suffix++;
		}
	}
	_panthera_kevent_raw_write(buf, (size_t)pos);
}

static inline void
_panthera_kevent_log_event(const char *prefix, const dispatch_kevent_s *ke, int idx)
{
	if (!ke) {
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: kevent is NULL\n");
		return;
	}
	char buf[512];
	int pos = 0;
	if (prefix) {
		while (*prefix && pos < (int)sizeof(buf) - 256) {
			buf[pos++] = *prefix++;
		}
	}
	const char *s = " idx=";
	...
	if (uidx == 0) {
		...
		while (uidx > 0 && numpos < (int)sizeof(num)) {
			num[numpos++] = '0' + (char)(uidx % 10);
			uidx /= 10;
		}
	}
	while (numpos > 0) buf[pos++] = num[--numpos];

	s = " filter=";
	while (*s) buf[pos++] = *s++;
	long long filt = (long long)ke->filter;
	unsigned long long ufilt = (filt < 0) ? (0ULL - (unsigned long long)filt) : (unsigned long long)filt;
	numpos = 0;
	if (ufilt == 0) {
		num[numpos++] = '0';
	} else {
		while (ufilt > 0 && numpos < (int)sizeof(num)) {
			num[numpos++] = '0' + (char)(ufilt % 10);
			ufilt /= 10;
		}
	}
	if (filt < 0) buf[pos++] = '-';
	...
	s = " flags=0x";
	while (*s) buf[pos++] = *s++;
	uint16_t fl = ke->flags;
	for (int i = 3; i >= 0; i--) buf[pos++] = hex[(fl >> (i * 4)) & 0xf];

	s = " ident=0x";
	while (*s) buf[pos++] = *s++;
	uint64_t id = (uint64_t)ke->ident;
	for (int i = 15; i >= 0; i--) buf[pos++] = hex[(id >> (i * 4)) & 0xf];

	s = " fflags=0x";
	while (*s) buf[pos++] = *s++;
	uint32_t ff = ke->fflags;
	for (int i = 7; i >= 0; i--) buf[pos++] = hex[(ff >> (i * 4)) & 0xf];

	s = " data=0x";
	while (*s) buf[pos++] = *s++;
	uint64_t dt = (uint64_t)ke->data;
	for (int i = 15; i >= 0; i--) buf[pos++] = hex[(dt >> (i * 4)) & 0xf];

	s = " udata=0x";
	while (*s) buf[pos++] = *s++;
	uint64_t ud = (uint64_t)ke->udata;
	for (int i = 15; i >= 0; i--) buf[pos++] = hex[(ud >> (i * 4)) & 0xf];

#if DISPATCH_USE_KEVENT_QOS
	s = " qos=0x";
	while (*s) buf[pos++] = *s++;
	uint32_t qo = (uint32_t)ke->qos;
	for (int i = 7; i >= 0; i--) buf[pos++] = hex[(qo >> (i * 4)) & 0xf];
#else
	s = " qos=0x0";
	while (*s) buf[pos++] = *s++;
#endif
...
_dispatch_kevent_merge_ev_flags(dispatch_unote_t du, uint32_t flags)
{
	...
	if (flags & EV_DELETE) {
		...
		_dispatch_unote_state_set(du, du_state);
			...
			(dux_type(du._du)->dst_flags & EV_DISPATCH))) {
		/*
		 * EV_DISPATCH registrations are disabled by event delivery and must be
		 * re-enabled by libdispatch after the handler runs. Panthera's kevent
		 * path can deliver the event without echoing EV_DISPATCH in the
		 * returned flags; preserve the Darwin source contract by deriving the
		 * disarmed state from the registered source type.
		 */
...
{
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_merge: du=", 0, " ptr=", (uintptr_t)du._du, "\n");
	dispatch_unote_action_t action = dux_type(du._du)->dst_action;
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_merge: action=", (long long)action, " flags=", (uintptr_t)ke->flags, "\n");
...
{
	int16_t filter = ke->filter;

	if (filter == EVFILT_USER) {
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_drain: EVFILT_USER\n");
		_dispatch_kevent_mgr_debug("received", ke);
		return;
	}
#if DISPATCH_USE_KEVENT_WORKLOOP
	if (filter == EVFILT_WORKLOOP) {
		_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_drain: EVFILT_WORKLOOP before poke_drain tailcall filter=", (long long)filter, " udata=", (uintptr_t)ke->udata, "\n");
		return _dispatch_kevent_workloop_poke_drain(ke);
	}
#endif // DISPATCH_USE_KEVENT_WORKLOOP
	_dispatch_kevent_debug("received", ke);
	if (unlikely(ke->flags & EV_ERROR)) {
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_drain: EV_ERROR\n");
		if (filter == EVFILT_PROC && ke->data == ESRCH) {
	...
	if (filter == EVFILT_TIMER) {
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_drain: EVFILT_TIMER\n");
		return _dispatch_kevent_timer_drain(ke);
	}

#if HAVE_MACH
	if (filter == EVFILT_MACHPORT && (_dispatch_kevent_mach_msg_size(ke) ||
			_dispatch_kevent_has_machmsg_rcv_error(ke))) {
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_drain: EVFILT_MACHPORT msg\n");
		return _dispatch_kevent_mach_msg_drain(ke);
	}
#endif

	if (_dispatch_kevent_unote_is_muxed(ke)) {
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_drain: muxed\n");
		return _dispatch_kevent_merge_muxed(ke);
	}
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_drain: unote udata=", 0, " ptr=", (uintptr_t)ke->udata, "\n");
...
_dispatch_kq_init(void *context)
{
	...
	_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_init: enter\n");
	_dispatch_fork_becomes_unsafe();
	...
	*kq_initialized = true;
	...
	_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_init: before kevent_workqueue_init\n");
	_dispatch_kevent_workqueue_init();
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_init: after kevent_workqueue_init enabled=", (long long)_dispatch_kevent_workqueue_enabled, " val=", (uintptr_t)_dispatch_kevent_workqueue_enabled, "\n");
	if (_dispatch_kevent_workqueue_enabled) {
		int r;
		int kqfd = _dispatch_kq_fd();
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_init: workqueue kqfd=", (long long)kqfd, " hex=", (uintptr_t)kqfd, "\n");
...
retry:
		_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_init: before kevent_qos\n");
		r = kevent_qos(kqfd, &ke, 1, NULL, 0, NULL, NULL,
				KEVENT_FLAG_WORKQ|KEVENT_FLAG_IMMEDIATE);
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_init: after kevent_qos r=", (long long)r, " errno=", (uintptr_t)(r == -1 ? errno : 0), "\n");
		if (unlikely(r == -1)) {
			int err = errno;
			switch (err) {
			case EINTR:
...
#if DISPATCH_USE_MGR_THREAD
	_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_init: mgr_thread path\n");
		...
		uint32_t flags)
{
	...
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: enter wlh=", 0, " n=", (uintptr_t)n, "\n");
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: flags=", (long long)flags, " pred=", (uintptr_t)_dispatch_kq_poll_pred, "\n");
	_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_poll: before dispatch_once _dispatch_kq_poll_pred\n");
	dispatch_once_f(&_dispatch_kq_poll_pred, &kq_initialized, _dispatch_kq_init);
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: after dispatch_once kq_init=", (long long)kq_initialized, " val=", (uintptr_t)kq_initialized, "\n");
	if (unlikely(kq_initialized)) {
		...
		_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_poll: calling memorypressure/voucher debug init\n");
...
retry:
	if (unlikely(wlh == NULL)) {
		_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_poll: wlh is NULL crash\n");
		DISPATCH_INTERNAL_CRASH(wlh, "Invalid wlh");
	} else if (wlh == DISPATCH_WLH_ANON) {
		int kqfd = _dispatch_kq_fd();
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: WLH_ANON kqfd=", (long long)kqfd, " flags=", (uintptr_t)flags, "\n");
#if DISPATCH_USE_KEVENT_QOS
		if (_dispatch_kevent_workqueue_enabled) {
			flags |= KEVENT_FLAG_WORKQ;
		}
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: before kevent_qos flags=", (long long)flags, " kqfd=", (uintptr_t)kqfd, "\n");
		r = kevent_qos(kqfd, ke, n, ke_out, n_out, buf, avail, flags);
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: after kevent_qos r=", (long long)r, " errno=", (uintptr_t)(r == -1 ? errno : 0), "\n");
#else
		(void)buf;
		(void)avail;
		const struct timespec timeout_immediately = {}, *timeout = NULL;
		if (flags & KEVENT_FLAG_IMMEDIATE) timeout = &timeout_immediately;
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: before kevent kqfd=", (long long)kqfd, " flags=", (uintptr_t)flags, "\n");
		r = kevent(kqfd, ke, n, ke_out, n_out, timeout);
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: after kevent r=", (long long)r, " errno=", (uintptr_t)(r == -1 ? errno : 0), "\n");
#endif
#if DISPATCH_USE_KEVENT_WORKLOOP
	} else {
		...
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: before kevent_id wlh=", 0, " flags=", (uintptr_t)flags, "\n");
		r = kevent_id((uintptr_t)wlh, ke, n, ke_out, n_out, buf, avail, flags);
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_poll: after kevent_id r=", (long long)r, " errno=", (uintptr_t)(r == -1 ? errno : 0), "\n");
#endif // DISPATCH_USE_KEVENT_WORKLOOP
	}
	if (unlikely(r == -1)) {
		int err = errno;
		...
		case 0:
			/*
			 * Panthera workaround: the kernel kevent_id path can return -1
			 * without setting errno. The deferred kevent updates have already
			 * been applied by the kernel (or are no-ops), so treat this as no
			 * events delivered rather than crashing.
			 */
			return 0;
		case ENOMEM:
		...
		uint32_t flags)
{
	...
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_drain: enter wlh=", 0, " n=", (uintptr_t)n, "\n");
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_drain: flags=", (long long)flags, " poll_for_events=", (uintptr_t)poll_for_events, "\n");
	...
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_drain: after poll n=", (long long)n, " hex=", (uintptr_t)n, "\n");
	if (n == 0) {
		r = 0;
	...
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_drain: done r=", (long long)r, " hex=", (uintptr_t)r, "\n");
	return r;
}
...
{
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_update_one: enter wlh=", 0, " filt=", (uintptr_t)(ke ? (uintptr_t)(uint16_t)ke->filter : 0), "\n");
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_update_one: ident=", (long long)(ke ? ke->ident : 0), " flags=", (uintptr_t)(ke ? ke->flags : 0), "\n");
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_update_one: fflags=", (long long)(ke ? ke->fflags : 0), " udata=", (uintptr_t)(ke ? ke->udata : 0), "\n");
...
{
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_deferred_update: enter wlh=", 0, " ke_filt=", (uintptr_t)(ke ? (uintptr_t)(uint16_t)ke->filter : 0), "\n");
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_deferred_update: ke_ident=", (long long)(ke ? ke->ident : 0), " ke_udata=", (uintptr_t)(ke ? ke->udata : 0), "\n");
	_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_deferred_update: before ddi_get\n");
	dispatch_deferred_items_t ddi = _dispatch_deferred_items_get();
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_deferred_update: after ddi_get ddi=", 0, " ptr=", (uintptr_t)ddi, "\n");
	if (ddi) {
		_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_kq_deferred_update: ddi_wlh=", 0, " ddi_maxevents=", (uintptr_t)ddi->ddi_maxevents, "\n");
	}

	if (ddi && ddi->ddi_wlh == wlh && ddi->ddi_maxevents) {
		_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_deferred_update: branch deferred_slot\n");
		...
		*dk = *ke;
		...
		_panthera_kevent_ckpt("[libdispatch] _dispatch_kq_deferred_update: branch update_one\n");
		...
		uint16_t action_flags)
{
	...
	bool force_immediate_rearm = du->du_is_direct &&
			(dux_type(du)->dst_flags & EV_DISPATCH) &&
			!(action_flags & (EV_ADD | EV_DELETE)) &&
			(action_flags & EV_ENABLE);
	int r = 0;
	...
	if (!force_immediate_rearm && ddi && ddi->ddi_wlh == wlh) {
		...
		if (!(action_flags & EV_ADD) && (action_flags & EV_ENABLE)) {
			...
			deferred_delete = (action_flags & EV_DELETE) != 0;
			goto done;
		}
...
_dispatch_unote_unregister_direct(dispatch_unote_t du, uint32_t flags)
{
	...
	bool deferred_owner_delete = false;
	if (likely(du_wlh != DISPATCH_WLH_ANON && ddi && ddi->ddi_wlh == du_wlh)) {
		...
		// Unregistering from a workloop is safe, but Panthera's deferred
		// kevent path can acknowledge deletion asynchronously with
		// EV_DELETE/EV_VANISHED.
#endif
		action |= EV_ENABLE;
		flags |= DUU_DELETE_ACK;
		deferred_owner_delete = true;
	}
...
{
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_stash: enter wlh=", 0, " ptr=", (uintptr_t)wlh, "\n");
	dispatch_queue_t dq = (dispatch_queue_t)wlh;
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_stash: existing stashed_dq=", 0, " ptr=", (uintptr_t)ddi->ddi_stashed_dou._dq, "\n");
	...
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_stash: exit qos=", (long long)ddi->ddi_stashed_qos, " rq=", (uintptr_t)ddi->ddi_stashed_rq, "\n");
}
...
{
	_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: enter\n");
	dispatch_deferred_items_t ddi = _dispatch_deferred_items_get();
	dispatch_wlh_t wlh = (dispatch_wlh_t)ke->udata;

	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: ddi=", (long long)(ddi != NULL), " wlh=", (uintptr_t)wlh, "\n");
	if (ddi) {
		_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: ddi_wlh_needs_delete=", (long long)ddi->ddi_wlh_needs_delete, " ddi_wlh=", (uintptr_t)ddi->ddi_wlh, "\n");
	}

	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: fflags=", (long long)ke->fflags, " hex=", (uintptr_t)ke->fflags, "\n");
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: flags=", (long long)ke->flags, " hex=", (uintptr_t)ke->flags, "\n");

#if DISPATCH_USE_KEVENT_WORKLOOP
	if (ke->fflags & NOTE_WL_SYNC_IPC) {
		_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: branch NOTE_WL_SYNC_IPC data=", (long long)ke->data, " flags=", (uintptr_t)ke->flags, "\n");
		dispatch_assert((ke->flags & EV_ERROR) && ke->data == ENOENT);
		return _dispatch_kevent_wlh_debug("ignoring", ke);
	}
#endif // DISPATCH_USE_KEVENT_WORKLOOP

	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: checking NOTE_WL_THREAD_REQUEST fflags=", (long long)ke->fflags, " hex=", (uintptr_t)ke->fflags, "\n");
	dispatch_assert(ke->fflags & NOTE_WL_THREAD_REQUEST);
	if (ke->flags & EV_ERROR) {
		_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: branch EV_ERROR data=", (long long)ke->data, " flags=", (uintptr_t)ke->flags, "\n");
		uint64_t dq_state = ke->ext[EV_EXTIDX_WL_VALUE];

		_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: before drain_error dq_state=", 0, " val=", (uintptr_t)dq_state, "\n");
		_dispatch_kevent_workloop_drain_error(ke,
				DISPATCH_KEVENT_WORKLOOP_ALLOW_ESTALE);
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: after drain_error\n");

		if (!_dq_state_is_base_wlh(dq_state)) {
			_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: EV_ERROR !base_wlh flags=", (long long)ke->flags, " hex=", (uintptr_t)ke->flags, "\n");
		...
		_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: EV_ERROR base_wlh action=", (long long)action, " hex=", (uintptr_t)action, "\n");
		if (action == DISPATCH_WORKLOOP_ASYNC) {
			_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: EV_ERROR retry drain before stash\n");
			_dispatch_kevent_wlh_debug("retry drain", ke);
			return _dispatch_kevent_workloop_stash(wlh, ke, ddi);
		} else {
			_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: EV_ERROR deferred_update action=", (long long)action, " wlh=", (uintptr_t)wlh, "\n");
		...
		//
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: branch ddi_wlh_needs_delete ignoring\n");
		return _dispatch_kevent_wlh_debug("ignoring", ke);
	} else {
		//
		// This is a new thread request, it is carrying a +1 reference.
		//
		_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_kevent_workloop_poke_drain: branch new thread request before stash\n");
...
{
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_event_loop_poke: enter wlh=", 0, " dq_state=", (uintptr_t)dq_state, "\n");
	_panthera_kevent_ckpt_int_hex("[libdispatch] _dispatch_event_loop_poke: flags=", (long long)flags, " is_mgr=", (uintptr_t)(wlh == DISPATCH_WLH_MANAGER ? 1 : 0), "\n");
	if (wlh == DISPATCH_WLH_MANAGER) {
		...
		_panthera_kevent_ckpt("[libdispatch] _dispatch_event_loop_poke: before deferred_update mgr\n");
...
{
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_event_loop_merge: entry nevents=", (long long)nevents, " events=", (uintptr_t)events, "\n");
	dispatch_deferred_items_t ddi = _dispatch_deferred_items_get();
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_event_loop_merge: after deferred_items_get ddi=", (long long)(ddi != NULL), " wlh=", (uintptr_t)(ddi ? ddi->ddi_wlh : 0), "\n");
	dispatch_wlh_t wlh = ddi->ddi_wlh;
	...
	_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_event_loop_merge: after memcpy sizeof_kev=", (long long)sizeof(kev), " kev_ptr=", (uintptr_t)kev, "\n");
	if (nevents > 0) {
		_panthera_kevent_log_event("PANTHERA_DISPATCH: _dispatch_event_loop_merge: event[0]", &kev[0], 0);
	}
	ddi->ddi_maxevents = DISPATCH_DEFERRED_ITEMS_EVENT_COUNT - 2;

	for (int i = 0; i < nevents; i++) {
		_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_event_loop_merge: before drain i=", (long long)i, " udata=", (uintptr_t)kev[i].udata, "\n");
		_dispatch_kevent_drain(&kev[i]);
		_panthera_kevent_ckpt_int_hex("PANTHERA_DISPATCH: _dispatch_event_loop_merge: after drain i=", (long long)i, " udata=", (uintptr_t)kev[i].udata, "\n");
	}

	if (wlh == DISPATCH_WLH_ANON) {
		if (ddi->ddi_stashed_dou._do && ddi->ddi_nevents) {
			_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_event_loop_merge: before anon event_loop_drain\n");
			...
			_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_event_loop_merge: after anon event_loop_drain\n");
		}
#if DISPATCH_USE_KEVENT_WORKLOOP
		...
		if (dth && dth[0].dth_dirty_bits) {
			_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_event_loop_merge: before workloop drain_timers\n");
			_dispatch_event_loop_drain_timers(dth, DISPATCH_TIMER_WLH_COUNT);
			_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_event_loop_merge: after workloop drain_timers\n");
		}
#endif // DISPATCH_USE_KEVENT_WORKLOOP
	}
	_panthera_kevent_ckpt("PANTHERA_DISPATCH: _dispatch_event_loop_merge: exit\n");
}
