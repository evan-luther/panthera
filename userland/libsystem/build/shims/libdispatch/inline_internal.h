...
static inline void
_panthera_qinit_trace(const char *msg)
{
#if PANTHERA_LIBDISPATCH_TRACE
		...
		uint16_t width, uint64_t initial_state_bits)
{
	...
	_panthera_qinit_trace("[libdispatch] queue_init enter\n");

	dispatch_assert((initial_state_bits & ~(DISPATCH_QUEUE_ROLE_MASK |
			DISPATCH_QUEUE_INACTIVE)) == 0);

	if (initial_state_bits & DISPATCH_QUEUE_INACTIVE) {
		_panthera_qinit_trace("[libdispatch] queue_init inactive path\n");
		dq->do_ref_cnt += 2; // rdar://8181908 see _dispatch_lane_resume
		_panthera_qinit_trace("[libdispatch] queue_init after inactive ref +2\n");
#if USE_OBJC
		_panthera_qinit_trace("[libdispatch] queue_init before source class check\n");
		if ((const void *)dq->do_vtable == (const void *)DISPATCH_VTABLE(source)) {
			_panthera_qinit_trace("[libdispatch] queue_init source extra ref\n");
			dq->do_ref_cnt++; // released when DSF_DELETED is set
			_panthera_qinit_trace("[libdispatch] queue_init after source extra ref\n");
		}
#else
	...
	_panthera_qinit_trace("[libdispatch] queue_init before next/flags/state\n");
	...
	_panthera_qinit_trace("[libdispatch] queue_init return\n");
	return dqu;
}
